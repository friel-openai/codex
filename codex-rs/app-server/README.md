...
- `thread/settings/update` — experimental; queue a partial update to a loaded thread’s next-turn settings without starting a turn or adding transcript items. Omitted fields leave settings unchanged; `serviceTier: null` clears the tier; deprecated `multiAgentMode` is ignored because Frodex uses proactive multi-agent behavior for every reasoning effort; `sandboxPolicy` and `permissions` cannot be combined. Returns `{}` when the update is accepted and emits `thread/settings/updated` with the full effective settings only if they actually change. `turn/start` settings overrides emit the same notification when they change the stored settings.
...
- `recencyAt` is initialized when the thread is created and advances when a turn starts. Ordinary background output and other persisted mutations do not advance it. Persisted activity from a spawned descendant may advance the top-level root thread's `recencyAt` at most once per hour so background subagent work can reappear in recency-sorted lists.
...
- `archived` — when `true`, list archived threads only; when `false`, list non-archived threads. Omission defaults to non-archived threads for ordinary lists. A current-agent relation request (`parentThreadId` or `ancestorThreadId`) includes both archive states when `archived` is omitted because current membership is authoritative.
...
Enable `capabilities.experimentalApi` during initialization, then use `thread/list` with `ancestorThreadId` to page through every agent in the owning loaded registry's current subtree. The ancestor itself is excluded, and each result's `parentThreadId` remains its canonical immediate parent. Use `parentThreadId` instead to list only current direct children. Closed agents, historical spawn edges, and subagents inherited only through forked conversation history are excluded. A thread that does not belong to a loaded agent registry returns an empty page. Sending both filters is invalid. Review and Guardian threads are not included because they do not participate in the agent registry. Each relation-filtered result includes `agentStatus` with the same lifecycle status used by the collaboration tools. Relation-filtered requests always batch display metadata from the state DB and never scan rollout files, regardless of `useStateDbOnly`. A cold terminal member without a persisted row still returns its canonical id, parent, agent path, source, and `agentStatus`; its title, nickname, role, and preview are empty, while required provider and cwd fields use server defaults. Clients comparing current membership should use id, parent, agent path, and `agentStatus`; explicit display-metadata filters apply the fallback values. When `modelProviders`, `sourceKinds`, or `archived` is omitted, relationship-filtered requests include every provider, source kind, or archive state, respectively. This prevents the archive metadata of an explicitly resumed agent from silently hiding current membership; explicit `archived: true` or `archived: false` restricts current members to that archive state. Other explicit filters retain the ordinary `thread/list` behavior, including the interactive-only default for an empty `sourceKinds` list.
...
{ "id": 21, "result": {
    "data": [
        { "id": "00000000-0000-0000-0000-000000000101", "parentThreadId": "00000000-0000-0000-0000-000000000100", "agentStatus": "running", "status": { "type": "active", "activeFlags": [] } },
        { "id": "00000000-0000-0000-0000-000000000102", "parentThreadId": "00000000-0000-0000-0000-000000000101", "agentStatus": "completed", "status": { "type": "idle" } }
    ],
    "nextCursor": null,
...
- `interAgentCommunication` — `{id, communication}` containing typed inter-agent delivery metadata restored by `thread/read`, `thread/resume`, and `thread/fork`. Live streams deliver the model-input form through `rawResponseItem/completed`.
