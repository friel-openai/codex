...
impl ThreadRequestProcessor {
    ...
    async fn thread_resume_inner(
        ...
        let materialize_response_history = include_turns || initial_turns_page.is_some();
        let response_history = thread_history.clone();
        let response_history_items = if materialize_response_history {
        ...
        let thread_history = match thread_history {
            InitialHistory::Resumed(mut resumed_history)
                if materialize_response_history
                    && resumed_history
                        .history
                        .iter()
                        .any(|item| matches!(item, RolloutItem::RolloutReference(_))) =>
            {
                // Core materializes resumed references for replay. Reuse the identical work already
                // done for this response; if that task fell back to references, core will retry.
                resumed_history.history = response_history_items.clone().into();
    ...
    async fn thread_fork_inner(
        ...
        .await;
        let materialized_history_items =
            Arc::new(if let Some(last_turn_id) = last_turn_id.as_deref() {
                truncate_rollout_after_turn_id(&materialized_history_items, last_turn_id)
                    .map_err(|err| core_thread_write_error("truncate thread for fork", err))?
            } else {
                materialized_history_items
            });
        let history_items = Arc::clone(&materialized_history_items);
        ...
        let NewThread {
            ...
            .fork_thread_from_history(
                ...
                InitialHistory::Resumed(ResumedHistory {
                    ...
                    // The bounded history is already materialized and truncated. Referencing the
                    // source rollout here would restore the suffix removed by `lastTurnId`.
                    rollout_path: if last_turn_id.is_some() {
                        None
                    } else {
