            ...
            let (history_items, snapshot) =
                match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                    (Some(last_turn_id), None) => (
                        ...
                        ForkSnapshot::TruncateBeforeNthUserMessage(
                            user_message_count_through_turn_id(&source_history_items, last_turn_id)
                                .map_err(|err| {
                                    core_thread_write_error("locate thread fork boundary", err)
                                })?,
                        ),
                    ),
                    (None, Some(before_turn_id)) => (
                        ...
                        ForkSnapshot::TruncateBeforeNthUserMessage(
                            user_message_count_before_turn_id(
                    ...
                    (None, None) => (Arc::clone(&source_history_items), ForkSnapshot::Interrupted),
                    (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
                };
            let new_thread = self
                .thread_manager
