...
impl ThreadRequestProcessor {
    ...
    async fn thread_fork_inner(
        ...
        if paginated_source && include_turns && session_configured.rollout_path.is_some() {
            let mut turns = self.paginated_thread_full_turns(thread_id).await?;
            if let Some(last_turn_id) = last_turn_id.as_deref()
                && let Some(index) = turns.iter().position(|turn| turn.id == last_turn_id)
            {
                turns.truncate(index + 1);
            } else if let Some(before_turn_id) = before_turn_id.as_deref()
                && let Some(index) = turns.iter().position(|turn| turn.id == before_turn_id)
            {
