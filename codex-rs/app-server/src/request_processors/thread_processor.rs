...
impl ThreadRequestProcessor {
    ...
    async fn thread_turns_list_response_inner(
                ...
                && matches!(thread.agent_status().await, AgentStatus::Running)
            {
                let active_turn = {
                    ...
                    thread_state
                        .lock()
                        .await
                        .active_turn_snapshot_with_items_view(
        ...
        let active_turn = if loaded_thread.is_some() {
            ...
            state.active_turn_snapshot_with_items_view(items_view.unwrap_or(TurnItemsView::Summary))
        } else {
            None
        };
    ...
    async fn unprojected_paginated_thread_turns_list_response(
        ...
        let active_turn = if loaded_thread.is_some() {
            let thread_state = self.thread_state_manager.thread_state(thread_id).await;
            let state = thread_state.lock().await;
            state.active_turn_snapshot_with_items_view(items_view)
        } else {
            None
        };
        ...
        tokio::task::spawn_blocking(move || {
            ...
            while let Some(outcome) = scanner.scan_next_rollout_line()? {
                ...
                if started_turns >= page_size {
                    let has_older_reference = match scanner.scan_next_rollout_line()? {
    ...
    async fn load_reference_backed_turn_window(
        ...
        loop {
            let materialized = materializer.materialize(ordinary_reference_limit).await?;
    ...
    async fn thread_resume_inner(
        ...
        let (mut thread_history, resume_source_thread) = resume_result?;
    ...
    async fn thread_fork_inner(
            ...
            let prepared =
                if let Some(local_store) = local_store {
                    ...
                    match (model_context, include_turns) {
                        ...
                        (Some((model_context, expected_position)), false) => local_store
        ...
        // Response history is cut at the requested turn and can still contain an older version.
        // Use the live parent or prepared model context, which retains the source's resolved
        // version even when the cutoff removed its TurnContext and permissions are overridden.
        let source_multi_agent_version = loaded_parent
            .as_ref()
            .and_then(|parent| parent.multi_agent_version())
            .or_else(|| {
                let source_context = prepared_fork
                    .as_ref()
                    .map(|prepared| &prepared.model_context)
                    .or(latest_context.as_ref())
                    .unwrap_or(&source_history_items);
                InitialHistory::Resumed(ResumedHistory {
                    conversation_id: source_thread_id,
                    history: Arc::clone(source_context),
        ...
        if let Some(multi_agent_version) = source_multi_agent_version
            && (last_turn_id.is_some() || before_turn_id.is_some())
            && let Some(prepared_fork) = prepared_fork.as_mut()
        {
            // Update only the fork's in-memory metadata; the source rollout stays unchanged.
            for item in Arc::make_mut(&mut prepared_fork.model_context) {
