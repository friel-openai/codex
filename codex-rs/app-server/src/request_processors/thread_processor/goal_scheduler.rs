...
impl ThreadRequestProcessor {
    ...
    async fn activate_goal_supervisor_schedule_inner(
        ...
        let thread_list_state_permit = self.acquire_thread_list_state_permit().await?;
        ...
        // Config loading can call back into Desktop. Shared resume releases this permit during
        // host work, rechecks the source afterward, and restores the thread's owned settings.
        ...
        let thread = self
            ...
            .await
            .map_err(|err| internal_error(format!("error resuming goal thread: {err}")))?;
