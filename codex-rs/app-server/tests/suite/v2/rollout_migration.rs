...
async fn automatic_migration_accepts_paginated_numeric_records_and_restarts() -> Result<()> {
    ...
    .to_string();
    assert_eq!(
        serde_json::to_value(codex_rollout::parse_rollout_line(&token_count)?)?,
        serde_json::to_value(
...
async fn large_unmarked_paginated_history_uses_compatibility_reader_and_restarts() -> Result<()> {
    const OUTPUT_RECORD_COUNT: usize = 80;
    ...
    // Exercise the byte scan limit without constructing millions of JSON numbers.
    let output_chunk = "x".repeat(OUTPUT_CHUNK_BYTES);
    let mut file = fs::OpenOptions::new().append(true).open(&path)?;
    for (ordinal, index) in (next_ordinal..).zip(0..OUTPUT_RECORD_COUNT) {
        let line = RolloutLine {
            timestamp: "2025-01-03T12:00:00Z".to_string(),
            ordinal: Some(ordinal),
            item: RolloutItem::EventMsg(EventMsg::AgentMessageContentDelta(
                AgentMessageContentDeltaEvent {
                    thread_id: thread_id.to_string(),
                    turn_id: "large-active-turn".to_string(),
                    item_id: format!("large-output-{index}"),
                    delta: output_chunk.clone(),
                },
            )),
        };
    ...
    assert!(
        source_len > 64 * 1024 * 1024,
        "fixture must exceed the certified-root reverse-scan limit"
    );
    let source_sha256 = file_prefix_sha256(&path, source_len)?;
    ...
    // Resume checkpoints effective settings without rewriting any historical bytes.
    assert_eq!(file_prefix_sha256(&path, source_len)?, source_sha256);
    let mut file = fs::File::open(&path)?;
    file.seek(SeekFrom::Start(source_len))?;
    let mut appended = String::new();
    file.read_to_string(&mut appended)?;
    assert!(appended.ends_with('\n'));
    for line in appended.lines() {
        let RolloutItem::EventMsg(EventMsg::ThreadSettingsApplied(event)) =
            codex_rollout::parse_rollout_line(line)?.item
        else {
            panic!("resume must only append the effective thread settings");
        };
...
fn file_prefix_sha256(path: &Path, length: u64) -> Result<String> {
    let mut file = fs::File::open(path)?.take(length);
    ...
    assert_eq!(
        file.limit(),
        0,
        "rollout must retain the full original prefix"
    );
