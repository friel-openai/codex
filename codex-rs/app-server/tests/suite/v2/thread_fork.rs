...
#[test_case::test_case(true; "warm_parent")]
#[test_case::test_case(false; "cold_parent")]
#[tokio::test]
async fn bounded_paginated_fork_preserves_latest_multi_agent_version(
    resume_parent: bool,
) -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    let fixture_home = TempDir::new()?;
    for home in [&codex_home, &fixture_home] {
        MockResponsesConfig::new(&server.uri())
            .disable_feature(Feature::MultiAgentV2)
            .write(home.path())?;
    }
    let mut mcp = TestAppServer::builder()
        .with_codex_home(fixture_home.path())
        .build_initialized()
        .await?;
    let parent = mcp
    ...
    for text in ["included versioned source", "excluded versioned source"] {
        let completed = mcp
    ...
    // Old session metadata can omit the version while a later TurnContext selects V2.
    // The requested fork prefix retains V1, so version recovery must use the full source.
    let recorded_path = parent.path.as_ref().expect("parent rollout");
    let (mut source_lines, ..) =
        RolloutRecorder::load_rollout_lines(recorded_path.as_path()).await?;
    let mut context_versions = Vec::new();
    for line in &mut source_lines {
        match &mut line.item {
            RolloutItem::SessionMeta(meta) => meta.meta.multi_agent_version = None,
            RolloutItem::TurnContext(context) => {
                let version = if context.turn_id.as_ref() == Some(&turn_ids[0]) {
                    MultiAgentVersion::V1
                } else {
                    assert_eq!(context.turn_id.as_ref(), Some(&turn_ids[1]));
                    MultiAgentVersion::V2
                };
                context.multi_agent_version = Some(version);
                context_versions.push(version);
            }
            _ => {}
        }
    }
    assert_eq!(
        context_versions,
        vec![MultiAgentVersion::V1, MultiAgentVersion::V2]
    );
    let fixture = source_lines
        .iter()
        .map(serde_json::to_string)
        .collect::<Result<Vec<_>, _>>()?
        .join("\n");
    // Do not reuse byte-offset projections recorded before the fixture rewrite.
    let source_path = codex_home
        .path()
        .join(recorded_path.as_path().strip_prefix(fixture_home.path())?);
    std::fs::create_dir_all(source_path.parent().expect("rollout directory"))?;
    std::fs::write(source_path.as_path(), format!("{fixture}\n"))?;
    ...
    assert_eq!(
        read_session_meta_line(source_path.as_path())
            .await?
            .meta
            .multi_agent_version,
        None
    );
    let source_before_forks = std::fs::read_to_string(source_path.as_path())?;
    let mut children = Vec::new();
    for (last_turn_id, before_turn_id) in [
        (Some(turn_ids[0].clone()), None),
        (None, Some(turn_ids[1].clone())),
    ] {
        for exclude_turns in [false, true] {
            let ThreadForkResponse { thread: child, .. } = mcp
                ...
                .await?;
            assert_eq!(
                child.turns.iter().map(|turn| &turn.id).collect::<Vec<_>>(),
                if exclude_turns {
                    Vec::new()
                } else {
                    vec![&turn_ids[0]]
            ...
            assert_eq!(
                ...
                "fork version must survive lastTurnId={last_turn_id:?}, beforeTurnId={before_turn_id:?}, excludeTurns={exclude_turns}"
            );
            assert_eq!(
                std::fs::read_to_string(source_path.as_path())?,
                source_before_forks,
                "forking must not rewrite source version metadata or history"
            );
            mcp.start_turn_and_wait_for_completion(TurnStartParams {
                thread_id: child.id.clone(),
                input: vec![UserInput::Text {
                    text: "first fork request".to_string(),
                    text_elements: Vec::new(),
                }],
                ..Default::default()
            })
    ...
    mcp = TestAppServer::builder()
        ...
        .build_initialized()
        .await?;
    // One restart covers persistence; every fork above already checked its stored version.
    for child in children.into_iter().take(1) {
        let _: ThreadResumeResponse = mcp
            .request(|request_id| ClientRequest::ThreadResume {
                request_id,
                params: ThreadResumeParams {
        ...
        mcp.start_turn_and_wait_for_completion(TurnStartParams {
            thread_id: child.id,
            input: vec![UserInput::Text {
                text: "resumed fork request".to_string(),
                text_elements: Vec::new(),
            }],
            ..Default::default()
        })
    ...
    for request in child_requests {
        ...
        let namespaces = body["tools"]
            ...
            .collect::<Vec<_>>();
        assert_eq!(namespaces, vec!["collaboration"]);
        let input = serde_json::to_string(&body["input"])?;
        assert!(input.contains("included versioned source"));
        assert!(!input.contains("excluded versioned source"));
    }
    Ok(())
}
...
async fn thread_fork_preserves_reference_backed_paginated_history() -> Result<()> {
    ...
    let expected_forked_from_ordinal_exclusive =
        RolloutRecorder::load_rollout_lines(source_path.as_path())
            .await?
            .0
            .into_iter()
    ...
    assert_eq!(
        ...
        Some(expected_forked_from_ordinal_exclusive)
    );
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    let source_id = ThreadId::from_string(source_thread_id.as_str())?;
    // The source is itself a fork. Its ancestor boundary must not become the new child's cutoff.
    let mut source_lines = RolloutRecorder::load_rollout_lines(source_path.as_path())
        .await?
        .0;
    let ancestor_id = ThreadId::new();
    let ancestor_path = rollout_path(
        codex_home.path(),
        "2025-01-05T11-00-00",
        ancestor_id.to_string().as_str(),
    );
    let mut ancestor_line = source_lines[0].clone();
    let RolloutItem::SessionMeta(ancestor_meta) = &mut ancestor_line.item else {
        panic!("paginated source starts with session metadata");
    };
    ancestor_meta.meta.id = ancestor_id;
    let mut ancestor_bytes = serde_json::to_vec(&ancestor_line)?;
    ancestor_bytes.push(b'\n');
    std::fs::write(ancestor_path, &ancestor_bytes)?;
    let ancestor_position = HistoryPosition {
        thread_id: ancestor_id,
        end_ordinal_exclusive: 1,
        end_byte_offset: ancestor_bytes.len() as u64,
    };
    let RolloutItem::SessionMeta(source_meta) = &mut source_lines[0].item else {
        panic!("paginated source starts with session metadata");
    };
    source_meta.meta.history_base = Some(ancestor_position);
    source_meta.meta.forked_from_id = Some(ancestor_id);
    source_meta.meta.forked_from_ordinal_exclusive = Some(ancestor_position.end_ordinal_exclusive);
    let mut source_bytes = Vec::new();
    for line in &mut source_lines {
        line.ordinal = line.ordinal.map(|ordinal| ordinal + 1);
        serde_json::to_writer(&mut source_bytes, line)?;
        source_bytes.push(b'\n');
    }
    std::fs::write(source_path.as_path(), source_bytes)?;
    ...
    let expected_forked_from_ordinal_exclusive =
        RolloutRecorder::load_rollout_lines(source_path.as_path())
            ...
            .expect("paginated source has a next ordinal");
    assert_ne!(
        expected_forked_from_ordinal_exclusive, ancestor_position.end_ordinal_exclusive,
        "the source's inherited boundary differs from the selected source prefix"
    );
    let mut mcp = TestAppServer::builder()
        .with_codex_home(codex_home.path())
        .without_auto_env()
    ...
    let forked_thread_id = forked_thread.id.clone();
    let forked_path = forked_thread.path.expect("forked rollout path");
    let forked_meta = read_session_meta_line(forked_path.as_path()).await?.meta;
    assert_eq!(forked_meta.history_base, None);
    assert_eq!(
        forked_meta.forked_from_ordinal_exclusive,
        Some(expected_forked_from_ordinal_exclusive)
    );
    ...
    let turn_metadata: Value = serde_json::from_str(
        request_body["client_metadata"]["x-codex-turn-metadata"]
            .as_str()
            .expect("ephemeral fork turn metadata"),
    )?;
    assert_eq!(
        turn_metadata["forked_from_thread_id"].as_str(),
