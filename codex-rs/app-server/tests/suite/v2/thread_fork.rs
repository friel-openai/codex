...
async fn assert_thread_fork_at_named_boundary_keeps_only_terminal_prefix(
    ...
    let physical_items = forked_contents
        .lines()
        .map(serde_json::from_str::<RolloutLine>)
        .collect::<Result<Vec<_>, _>>()?;
    let RolloutItem::RolloutReference(reference) = &physical_items
        .get(1)
        .expect("forked rollout reference")
        .item
    else {
        panic!("forked rollout did not use the canonical rollout reference");
    };
...
async fn thread_fork_creates_reference_backed_paginated_thread() -> Result<()> {
    ...
    let physical = std::fs::read_to_string(forked_path.as_path())?;
    assert!(!physical.contains("Saved user message"));
    let meta = read_session_meta_line(forked_path.as_path()).await?;
    assert_eq!(meta.meta.history_base, None);
    let physical_items = physical
        .lines()
        .map(serde_json::from_str::<RolloutLine>)
        .collect::<Result<Vec<_>, _>>()?;
    let RolloutItem::RolloutReference(reference) = &physical_items
        ...
        panic!("paginated fork did not use the canonical rollout reference");
    };
    assert_eq!(
        reference.thread_id,
        Some(ThreadId::from_string(conversation_id.as_str())?)
    );
    ...
    let excluded_items = std::fs::read_to_string(excluded_turns_path.as_path())?
        .lines()
        .map(serde_json::from_str::<RolloutLine>)
        .collect::<Result<Vec<_>, _>>()?;
    assert!(matches!(
        excluded_items.get(1).map(|line| &line.item),
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    assert!(matches!(
        ...
        [
            ...
            RolloutLine { item: RolloutItem::RolloutReference(_), .. },
            RolloutLine {
    ...
    let nested_before_id = nested_before.id;
    ...
    let ThreadResumeResponse {
        thread: resumed_nested_before,
        ..
    } = resumed_app_server
        .request(|request_id| ClientRequest::ThreadResume {
            request_id,
            params: ThreadResumeParams {
                thread_id: nested_before_id,
                ..Default::default()
            },
        })
        .await?;
    assert!(resumed_nested_before.turns.is_empty());
