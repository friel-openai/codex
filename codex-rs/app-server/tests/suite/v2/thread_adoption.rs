...
const ADOPTER_DEVELOPER_INSTRUCTIONS: &str = "adopter-only developer instructions";
    ...
    if adoption_source == AdoptionSource::Stored {
        ...
        MockResponsesConfig::new(&server.uri())
            ...
            .with_root_config(&format!(
                r#"
developer_instructions = "{ADOPTER_DEVELOPER_INSTRUCTIONS}"
...
"#
    ...
    if adoption_source == AdoptionSource::Stored {
        let adopted_request = timeout(TIMEOUT, async {
            loop {
                if let Some(request) = adopted_work.requests().into_iter().find(|request| {
                    request
                        .body_json()
        ...
        .await
        .context("the mock did not capture the adopted worker request")?;
        assert!(
            !adopted_request.body_contains_text(ADOPTER_DEVELOPER_INSTRUCTIONS),
            "cold adoption must not copy the adopter's developer instructions"
        );
    }
    ...
    responses::mount_sse_once_match(
        ...
        move |request: &wiremock::Request| {
            request_has_thread_id(request, &expected_thread_id)
                && request_body_has_thread_id(request, &expected_thread_id)
                && request
                    .headers
                    .get("session-id")
                    .and_then(|value| value.to_str().ok())
...
fn request_body_has_thread_id(request: &wiremock::Request, expected: &str) -> bool {
