...
const LIST_RESTORED_ADOPTED_PROMPT: &str = "list the restored adopted durable worker";
const LIST_RESTORED_ADOPTED_CALL_ID: &str = "native-adoption-restored-list-agents-call";
...
async fn assert_native_thread_adoption_and_promotion(
    ...
    if adoption_source == AdoptionSource::DisabledAfterAdoption {
        ...
        let restored_children = list_threads(&restarted, 43, Some(parent_id.clone())).await?;
        assert_eq!(
            restored_children
                .data
                .iter()
        ...
        let _list_restored_adopted_call = mount_tool_response(
            &server,
            LIST_RESTORED_ADOPTED_PROMPT,
            "resp-native-list-restored-adopted-call",
            LIST_RESTORED_ADOPTED_CALL_ID,
            "list_agents",
            "{}",
        )
        .await;
        let list_restored_adopted_result = mount_final_response(
            &server,
            LIST_RESTORED_ADOPTED_CALL_ID,
            "resp-native-list-restored-adopted-return",
            "the restored adopted worker remains in the native agent registry",
        )
        .await;
        start_turn(&restarted, 44, &parent_id, LIST_RESTORED_ADOPTED_PROMPT).await?;
        wait_for_completed_turn(&mut restarted, &parent_id).await?;
        assert_listed_agent(
            &list_restored_adopted_result,
            LIST_RESTORED_ADOPTED_CALL_ID,
            "/root/adopted_worker",
            /*should_be_listed*/ true,
        )?;

        let restored = read_thread(&restarted, 45, &original_id).await?;
        assert_eq!(restored.id, original_id);
        assert_eq!(
            restored.parent_thread_id.as_deref(),
            Some(parent_id.as_str())
        );
        assert_eq!(restored.session_id, parent_session_id);
        assert!(matches!(restored.source, SessionSource::SubAgent(_)));
        restarted.shutdown().await?;
        return Ok(());
    }
