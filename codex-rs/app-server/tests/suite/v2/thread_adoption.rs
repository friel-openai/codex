...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn native_adopted_thread_remains_owned_after_restart_with_feature_disabled() -> Result<()> {
...
async fn assert_native_thread_adoption_and_promotion(
    ...
    if adoption_source == AdoptionSource::DisabledAfterAdoption {
        ...
        let resume_error = restarted
            ...
            .await?
            .expect_err("a parent-owned child must reject direct resume");
        assert!(
            resume_error.message.contains("is owned by")
                && resume_error.message.contains(parent_id.as_str()),
            "unexpected direct-resume error: {}",
            resume_error.message
        );

        let still_owned = read_thread(&restarted, 42, &original_id).await?;
        assert_eq!(
            still_owned.parent_thread_id.as_deref(),
            Some(parent_id.as_str())
        );
        assert_eq!(still_owned.session_id, parent_session_id);
        assert!(matches!(still_owned.source, SessionSource::SubAgent(_)));
        restarted.shutdown().await?;
        return Ok(());
    }
