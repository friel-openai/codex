...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn native_adopted_thread_requires_parent_resume_after_feature_is_disabled() -> Result<()> {
...
async fn assert_native_thread_adoption_and_promotion(
    ...
    if adoption_source == AdoptionSource::DisabledAfterAdoption {
        ...
        let original_thread_id = ThreadId::from_string(&original_id)?;
        assert_eq!(
            state_db
                ...
                .await?,
            vec![original_thread_id],
            "adoption must persist an open edge before the parent is cold-resumed"
        );
        MockResponsesConfig::new(&server.uri())
            .enable_feature(Feature::Collab)
        ...
        let direct_resume = restarted
            ...
            .await?;
        let direct_resume_error =
            direct_resume.expect_err("a cold parent-owned worker must not resume independently");
        assert!(
            direct_resume_error
                .message
                .contains("resume it through its parent or promote it first"),
            "unexpected direct-resume error: {direct_resume_error:?}"
        );

        let stored_after_rejection = read_thread(&restarted, 42, &original_id).await?;
        assert_eq!(stored_after_rejection.status, ThreadStatus::NotLoaded);
        assert_eq!(
            stored_after_rejection.parent_thread_id.as_deref(),
            Some(parent_id.as_str())
        );
        assert_eq!(stored_after_rejection.session_id, parent_session_id);
        assert!(matches!(
            stored_after_rejection.source,
            SessionSource::SubAgent(_)
        ));

        let resumed_parent: ThreadResumeResponse = request(
            &restarted,
            ClientRequest::ThreadResume {
        ...
        .await?;
        assert_eq!(resumed_parent.thread.id, parent_id);

        let _list_adopted_call = mount_tool_response(
            &server,
            LIST_ADOPTED_PROMPT,
            "resp-native-list-adopted-after-restart-call",
        ...
        let list_adopted_result = mount_final_response(
            &server,
            LIST_ADOPTED_CALL_ID,
            "resp-native-list-adopted-after-restart-return",
            "the adopted worker remains attached after the parent resumes",
        )
        .await;
        start_turn(&restarted, 44, &parent_id, LIST_ADOPTED_PROMPT).await?;
        wait_for_completed_turn(&mut restarted, &parent_id).await?;
        assert_listed_agent(
            &list_adopted_result,
            LIST_ADOPTED_CALL_ID,
            "/root/adopted_worker",
