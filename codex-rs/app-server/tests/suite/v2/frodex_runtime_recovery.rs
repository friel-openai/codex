...
async fn released_codex_repairs_alpha6_supervisor_history_across_restart() -> Result<()> {
    ...
    // Resume may append session metadata after the repaired historical bytes.
    let repaired_rollout = std::fs::read(rollout_path.as_path())?;
    let repaired_prefix = repaired_rollout
        .get(..original.len())
        .context("repair must preserve the original byte boundary")?
        .to_vec();
    assert_eq!(repaired_prefix.last(), Some(&b'\n'));
