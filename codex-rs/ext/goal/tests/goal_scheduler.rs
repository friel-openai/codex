...
#[tokio::test]
async fn scheduler_ownership_contends_and_hands_off() -> anyhow::Result<()> {
    ...
    let first_completion_gate = Arc::new(Semaphore::new(0));
    let first = GoalSchedulerHandle::start_with_options(
        Arc::clone(&state_db),
        gated_recording_activator(first_tx, Arc::clone(&first_completion_gate)),
        scheduler_options(),
    );
    let first_activation = tokio::time::timeout(Duration::from_secs(1), first_rx.recv())
        .await
        .expect("first scheduler should acquire ownership")
        .expect("first activation channel should remain open");
    assert_eq!(thread_id, first_activation.thread_id);

    let (second_tx, mut second_rx) = mpsc::unbounded_channel();
    let second = GoalSchedulerHandle::start_with_options(
        Arc::clone(&second_state_db),
        recording_terminal_activator(second_state_db, second_tx),
        scheduler_options(),
    );
    assert!(
        tokio::time::timeout(Duration::from_millis(100), second_rx.recv())
            .await
            .is_err(),
        "the second scheduler must wait while the first owns the lock"
    );

    first.stop();
    drop(first);

    let handoff_activation = tokio::time::timeout(Duration::from_secs(1), second_rx.recv())
        .await
        .expect("second scheduler should acquire ownership after handoff")
        .expect("second activation channel should remain open");
    assert_eq!(thread_id, handoff_activation.thread_id);

    tokio::time::timeout(Duration::from_secs(1), async {
        loop {
            if state_db
                .thread_goals()
                .get_active_goal_supervisor_schedule(thread_id)
                .await
                .expect("goal schedule lookup should succeed")
                .is_none()
            {
                break;
            }
    ...
    .await
    .expect("handoff activation should finish");
    first_completion_gate.add_permits(1);
    second.stop();

    assert!(
        sqlite_home.path().join(".goal-scheduler.lock").is_file(),
        "scheduler lock file should persist after ownership handoff"
    );
    Ok(())
}
