...
async fn run_scheduler(
    ...
    loop {
        match try_acquire_scheduler_ownership(state_db.codex_home()) {
            Ok(Some(_ownership)) => {
                debug!(
                    codex_home = %state_db.codex_home().display(),
...
fn try_acquire_scheduler_ownership(codex_home: &Path) -> io::Result<Option<File>> {
    // Lock the existing state-home directory rather than a SQLite file. This keeps scheduler
    // ownership independent from SQLite's byte-range locks and adds no persistent lock file.
    let file = File::open(codex_home)?;
    match file.try_lock() {
        Ok(()) => Ok(Some(file)),
