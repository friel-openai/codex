...
const GOAL_SCHEDULER_LOCK_FILE_NAME: &str = ".goal-scheduler.lock";
...
fn try_acquire_scheduler_ownership(sqlite_home: &Path) -> io::Result<Option<File>> {
    // Keep scheduler ownership independent from SQLite's byte-range locks. The lock file remains
    // in place after release so every process continues to coordinate through the same inode.
    let file = OpenOptions::new()
        .read(true)
        .write(true)
        .create(true)
        .truncate(false)
        .open(sqlite_home.join(GOAL_SCHEDULER_LOCK_FILE_NAME))?;
    match file.try_lock() {
        Ok(()) => Ok(Some(file)),
