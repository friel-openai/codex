...
async fn api_spawned_descendant_shares_the_root_mcp_server_process() -> Result<()> {
    ...
    let root_result = test
        .codex
        .call_mcp_tool(MCP_SERVER_NAME, "shared_counter", None, None)
        .await?;
...
async fn child_shutdown_cancels_unanswered_elicitation_and_recovers_root() -> Result<()> {
    ...
    let pending_call = tokio::spawn({
        ...
        async move {
            child_thread
                .call_mcp_tool(MCP_SERVER_NAME, "request_elicitation", None, None)
                .await
        }
    });
...
async fn crashed_shared_server_can_be_replaced_for_the_tree() -> Result<()> {
    ...
    child_thread
        .call_mcp_tool(MCP_SERVER_NAME, "crash", None, None)
        .await
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn root_refresh_replaces_the_preferred_connection_while_child_lease_drains() -> Result<()> {
