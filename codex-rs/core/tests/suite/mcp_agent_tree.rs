...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn adopted_root_rebinds_to_the_parent_agent_trees_mcp_process() -> Result<()> {
    const ADOPT: &str = "adopt the existing root into this agent tree";
    const CHILD_TASK: &str = "continue as the adopted root";
    const ORIGINAL_TURN: &str = "complete one turn before adoption";
    const SPAWN: &str = "adopt-root-with-shared-mcp";

    let server = start_mock_server().await;
    mount_sse_once_match(
        &server,
        |request: &Request| body_contains(request, ORIGINAL_TURN),
        sse(vec![
            ev_response_created("adoption-original-root"),
            ev_assistant_message("adoption-original-message", "ready for adoption"),
            ev_completed("adoption-original-root"),
        ]),
    )
    .await;
    ...
    let test = test_codex()
        ...
        .build(&server)
        .await?;
    let original_root = test
        .thread_manager
        .start_thread(StartThreadOptions::new(test.config.clone()))
        .await?;
    let original_root_id = original_root.session_configured.thread_id;

    let parent_before = test
        .codex
        .call_mcp_tool(
            MCP_SERVER_NAME,
            "shared_counter",
        ...
        .await?;
    let original_before = original_root
        .thread
        .call_mcp_tool(
            MCP_SERVER_NAME,
            "shared_counter",
        ...
        .await?;
    let parent_pid = parent_before
        .structured_content
        .as_ref()
        .and_then(|content| content.get("pid"))
        .and_then(Value::as_u64)
        .expect("parent counter response should include a PID");
    let original_pid = original_before
        .structured_content
        .as_ref()
        .and_then(|content| content.get("pid"))
        .and_then(Value::as_u64)
        .expect("original root counter response should include a PID");
    assert_ne!(parent_pid, original_pid);
    assert_eq!(
        parent_before.structured_content,
        Some(json!({ "count": 1, "pid": parent_pid }))
    );
    assert_eq!(
        original_before.structured_content,
        Some(json!({ "count": 1, "pid": original_pid }))
    );
    ...
    original_root
        ...
        .await?;
    wait_for_event(&original_root.thread, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;

    let original_root_id_for_spawn = original_root_id;
    mount_sse_once_match(
        &server,
        |request: &Request| body_contains(request, ADOPT) && !body_contains(request, SPAWN),
        sse(vec![
            ev_response_created("adoption-parent"),
            ev_function_call_with_namespace(
                SPAWN,
                MULTI_AGENT_V2_NAMESPACE,
                "spawn_agent",
                &serde_json::to_string(&json!({
                    "existing_thread_id": original_root_id_for_spawn,
                    "message": CHILD_TASK,
                    "task_name": "adopted_root",
                }))?,
            ),
            ev_completed("adoption-parent"),
        ]),
    )
    .await;
    mount_sse_once_match(
        &server,
        |request: &Request| body_contains(request, CHILD_TASK) && !body_contains(request, SPAWN),
        sse(vec![
            ev_response_created("adoption-child"),
            ev_assistant_message("adoption-child-message", "adopted"),
            ev_completed("adoption-child"),
        ]),
    )
    .await;
    ...
    let parent_final = mount_sse_once_match(
        ...
        sse(vec![
            ev_response_created("adoption-parent-final"),
            ev_assistant_message("adoption-parent-message", "adoption complete"),
            ev_completed("adoption-parent-final"),
        ]),
    )
    .await;

    test.submit_turn(ADOPT).await?;
    let spawn_output = parent_final
        .function_call_output_text(SPAWN)
        .expect("adoption should produce a spawn tool result");
    let spawn_output: Value = serde_json::from_str(&spawn_output)
        .with_context(|| format!("adoption returned {spawn_output:?}"))?;
    assert_eq!(
        spawn_output.get("task_name").and_then(Value::as_str),
        Some("/root/adopted_root"),
        "adoption should return the registered agent path"
    );
    let adopted_root = test.thread_manager.get_thread(original_root_id).await?;
    assert_eq!(
        adopted_root.config_snapshot().await.parent_thread_id,
        Some(test.session_configured.thread_id)
    );
    let adopted_result = adopted_root
        .call_mcp_tool(
            MCP_SERVER_NAME,
            "shared_counter",
            /*arguments*/ None,
        ...
        .await?;
    assert_eq!(
        adopted_result.structured_content,
        Some(json!({ "count": 2, "pid": parent_pid }))
    );
    wait_for_process_exit_after_grace(&original_pid.to_string())
        .await
        .context("the adopted root's original MCP process should retire")?;

    adopted_root.shutdown_and_wait().await?;
    assert!(
        process_is_alive(&parent_pid.to_string())?,
        "the parent's lease should keep the adopted tree's MCP process alive"
    );
    test.codex.shutdown_and_wait().await?;
    wait_for_process_exit_after_grace(&parent_pid.to_string())
        .await
        .context("the shared MCP process should exit after the last lease")
}
