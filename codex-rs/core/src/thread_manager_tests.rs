...
#[tokio::test]
async fn cold_child_resume_keeps_saved_locations_and_inherits_fresh_owner_config() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.codex_home = temp_dir.path().join("codex-home").abs();
    config.cwd = config.codex_home.clone();
    let parent_cwd = config.codex_home.join("parent-workspace");
    let child_cwd = config.codex_home.join("child-workspace");
    std::fs::create_dir_all(parent_cwd.as_path()).expect("create parent workspace");
    std::fs::create_dir_all(child_cwd.as_path()).expect("create child workspace");
    let fresh_owner_config = codex_protocol::protocol::EnvironmentConfig {
        allow_login_shell: false,
        workspace_roots: vec![PathUri::from_abs_path(&parent_cwd)],
        permission_profile: config.permissions.permission_profile_state().snapshot(),
        shell_environment_policy: Default::default(),
        windows_sandbox_level: WindowsSandboxLevel::from_config(&config),
        windows_sandbox_type: config.permissions.windows_sandbox_type,
    ...
    let parent_selection = TurnEnvironmentSelection {
        environment_id: codex_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
        cwd: PathUri::from_abs_path(&parent_cwd),
        workspace_roots: fresh_owner_config.workspace_roots.clone(),
        config: EnvironmentConfigState::Ready(fresh_owner_config.clone()),
    };
    let manager = ThreadManager::with_models_provider_and_home_for_tests(
        CodexAuth::from_api_key("dummy"),
        config.model_provider.clone(),
    ...
    options.environments = Some(vec![parent_selection]);
    let parent = manager
        .start_thread(options)
        .await
        .expect("start live parent");
    ...
    assert_eq!(
        inherited
            .primary()
            .expect("live parent environment")
            .config(),
        &fresh_owner_config
    );
    ...
    let expected_child_selection = TurnEnvironmentSelection {
        environment_id: codex_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
        cwd: PathUri::from_abs_path(&child_cwd),
        workspace_roots: vec![PathUri::from_abs_path(&child_cwd)],
        config: EnvironmentConfigState::Ready(fresh_owner_config.clone()),
    };
    let mut old_owner_config = fresh_owner_config.clone();
    old_owner_config.allow_login_shell = true;
    old_owner_config.workspace_roots = expected_child_selection.workspace_roots.clone();
    let settings = checkpoint_settings_item(
        &config,
        child_thread_id,
        child_cwd,
        TurnEnvironmentSelection {
            config: EnvironmentConfigState::Ready(old_owner_config),
            ..expected_child_selection.clone()
        },
    );
    let session_meta = SessionMetaLine {
        meta: SessionMeta {
    ...
    let rollout_path = config.codex_home.join("cold-child.jsonl").to_path_buf();
    std::fs::write(
        &rollout_path,
        format!(
            "{}\n",
    ...
    .expect("write cold child rollout");
    ...
    let resumed = manager
        ...
        .resume_thread_with_history_with_source(ResumeThreadWithHistoryOptions {
            ...
            initial_history: InitialHistory::Resumed(ResumedHistory {
                ...
                history: Arc::new(vec![RolloutItem::SessionMeta(session_meta), settings]),
                rollout_path: Some(rollout_path),
            }),
            ...
            inherited_environments: Some(inherited),
            inherited_instructions: None,
            inherited_exec_policy: None,
            client_mcp_extensions: Some(ClientMcpExtensions::default()),
            inherited_thread_state: InheritedThreadState::default(),
        })
        .await
        .expect("cold-resume child with fresh parent authority");
    assert_eq!(
        resumed.thread.environment_selections().await,
        vec![expected_child_selection.clone()]
    );
    let turn = resumed.thread.session.new_default_turn().await;
    let effective = turn
        .initial_environments
        .primary()
        .expect("ready child environment");
    assert_eq!(effective.cwd(), &expected_child_selection.cwd);
    assert_eq!(effective.config(), &fresh_owner_config);
    assert_eq!(
        effective.workspace_roots(),
        fresh_owner_config.workspace_roots.as_slice()
    );
    resumed
        .thread
        .shutdown_and_wait()
