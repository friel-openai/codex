...
mod tests {
    ...
    async fn handler_requests_immediate_context_refresh_and_refreshes_agents_md() {
        ...
        let EnvironmentConfigState::Ready(environment_config) = &mut selection.config else {
            panic!("test environment config should be ready");
        };
        environment_config.workspace_roots = selection.workspace_roots.clone();
    ...
    async fn handler_completes_workspace_switch_when_agents_md_refresh_fails() {
        ...
        environment_config.permission_profile =
            PermissionProfileSnapshot::legacy(PermissionProfile::default());
        let config_origin = current_environment.config_origin;
        let environment = Arc::clone(&current_environment.environment);
        let shell = current_environment.shell.clone();
        turn.environments.environments = vec![TurnEnvironmentState::Ready(TurnEnvironment::new(
