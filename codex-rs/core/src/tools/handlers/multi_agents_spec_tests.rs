...
#[test]
fn supervisor_snooze_delay_uses_integer_schema() {
    let ToolSpec::Function(ResponsesApiTool { parameters, .. }) = create_supervisor_snooze_tool()
    else {
        panic!("supervisor snooze should be a function tool");
    };
    ...
    assert_eq!(
        parameters
            .properties
            .as_ref()
            .and_then(|properties| properties.get("delay_seconds"))
