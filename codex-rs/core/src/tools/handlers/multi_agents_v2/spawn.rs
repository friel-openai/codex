...
async fn handle_agent_start(
    ...
    let plaintext_message = is_adoption
        || matches!(
            source,
            crate::tools::context::ToolCallSource::DirectPlaintextMessage
        );
    let context = AgentCommunicationContext::new(AgentCommunicationKind::Spawn, session.thread_id);
    let context = if plaintext_message {
        context.with_last_task_message_override(message.clone())
    } else {
        context
    };
    let communication = if is_adoption {
