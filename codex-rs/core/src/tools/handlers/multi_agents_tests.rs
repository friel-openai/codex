...
#[test]
fn goal_supervisor_checkin_spawns_hidden_full_history_helper_once() {
    run_large_stack_async(|| async {
        ...
        let helper_thread_id = helper_session.thread_id;
...
#[test]
#[serial]
fn materialized_ephemeral_parent_starts_goal_supervisor_checkin() {
    run_large_stack_async(|| async {
        struct EnvVarGuard {
            key: &'static str,
...
#[test]
fn goal_supervisor_plain_completion_reschedules_checkin() {
    run_large_stack_async(|| async {
...
fn multi_agent_v2_spawn_watchdog_role_is_rejected_in_goal_supervisor_mode() {
    run_large_stack_async(|| async {
        ...
        assert_eq!(
            err,
            FunctionCallError::RespondToModel("unknown agent_type 'watchdog'".to_string())
        );
    });
}
...
#[test]
fn supervisor_followup_task_parent_wakes_parent_and_finishes_helper() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, _state_db, _goal_id) =
            start_goal_supervisor_helper_for_test().await;
        let helper_thread_id = helper_session.thread_id;
...
#[test]
fn supervisor_followup_context_identifies_previous_supervisor_message() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, _state_db, _goal_id) =
            start_goal_supervisor_helper_for_test().await;
        let helper_turn = helper_session.new_default_turn().await;
...
#[test]
fn supervisor_checkin_does_not_spawn_duplicate_helper_while_one_is_active() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, state_db, goal_id) =
            start_goal_supervisor_helper_for_test().await;
        ...
        assert_ne!(
            manager
                .agent_control()
                .get_status(helper_session.thread_id)
                .await,
            AgentStatus::NotFound
        );
    });
...
#[test]
fn supervisor_checkin_spawns_while_user_visible_subagent_is_running() {
    run_large_stack_async(|| async {
...
#[test]
fn supervisor_checkin_honors_persisted_snooze_before_spawning_helper() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, parent_session, state_db, goal_id, protocol_goal) =
            start_supervised_goal_parent_for_test().await;
        state_db
...
#[test]
fn supervisor_helper_is_hidden_from_list_agents() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, _state_db, _goal_id) =
            start_goal_supervisor_helper_for_test().await;
        let parent_session = manager
        ...
        assert!(
            !result
                ...
                .any(|agent| agent.agent_name == "/root/goal_supervisor"
                    || agent.agent_name == helper_session.thread_id.to_string()),
...
#[test]
fn supervisor_send_message_parent_is_rejected_and_keeps_helper_active() {
    run_large_stack_async(|| async {
...
#[test]
fn supervisor_snooze_finishes_helper_and_persists_snooze() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, state_db, goal_id) =
            start_goal_supervisor_helper_for_test().await;
        let helper_thread_id = helper_session.thread_id;
        ...
        let (content, success) = expect_text_output(output);
        let result: serde_json::Value =
            serde_json::from_str(&content).expect("result should be json");
...
#[test]
fn supervisor_close_self_marks_goal_complete_notifies_parent_and_clears_snooze() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, state_db, goal_id) =
            start_goal_supervisor_helper_for_test().await;
        let helper_thread_id = helper_session.thread_id;
...
#[test]
fn supervisor_close_self_ignores_replaced_goal() {
    run_large_stack_async(|| async {
        ...
        let output = SupervisorSelfCloseHandler
            ...
            .await
            .expect(
                "stale supervisor helper should finish without completing the replacement goal",
            );
        let (content, success) = expect_text_output(output);
        let result: serde_json::Value =
            serde_json::from_str(&content).expect("result should be json");
...
#[test]
fn supervisor_snooze_does_not_snooze_replacement_goal() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, state_db, old_goal_id) =
            start_goal_supervisor_helper_for_test().await;
        let helper_thread_id = helper_session.thread_id;
...
#[test]
fn supervisor_compact_parent_context_submits_compaction_and_finishes_helper() {
    run_large_stack_async(|| async {
        let (manager, parent_thread_id, helper_session, _state_db, _goal_id) =
            start_goal_supervisor_helper_for_test().await;
        let helper_thread_id = helper_session.thread_id;
        ...
        assert!(manager.captured_ops().iter().any(|(thread_id, op)| {
            *thread_id == parent_thread_id && matches!(op, Op::Compact)
        }));
    });
}
...
fn multi_agent_v2_list_agents_filters_by_relative_path_prefix() {
    run_large_stack_async(|| async {
        ...
        let worker_path =
            AgentPath::from_string("/root/researcher/worker".to_string()).expect("path");
        session
            .services
            .agent_control
            .spawn_agent_with_metadata(
...
#[tokio::test]
async fn multi_agent_v2_spawn_agent_ignores_depth_limit() {
    let (mut session, mut turn) = make_session_and_context().await;
    let manager = thread_manager();
    let mut config = (*turn.config).clone();
    ...
    assert_eq!(result["nickname"], serde_json::Value::Null);
    assert_eq!(success, Some(true));
}
...
#[test]
fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    run_large_stack_async(|| async {
        let (mut session, turn) = make_session_and_context().await;
        let manager = thread_manager();
        session.services.agent_control = manager.agent_control();
...
#[test]
fn multi_agent_v2_close_agent_reaps_stale_task_name_target() {
    run_large_stack_async(|| async {
        let (mut session, mut turn) = make_session_and_context().await;
        let mut config = (*turn.config).clone();
