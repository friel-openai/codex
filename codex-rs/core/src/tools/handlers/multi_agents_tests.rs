...
#[tokio::test]
async fn build_agent_resume_config_clears_caller_prompt_identity() {
    let (_session, mut turn) = make_session_and_context().await;
    let mut base_config = (*turn.config).clone();
    base_config.base_instructions = Some("caller-base".to_string());
    base_config.developer_instructions = Some("caller-developer".to_string());
    base_config.personality = Some(codex_protocol::config_types::Personality::Friendly);
