...
const MIN_WATCHDOG_INTERVAL_SECS: u64 = 300;
const DEFAULT_WATCHDOG_INTERVAL_SECS: u64 = 300;
const DEFAULT_WATCHDOG_PROMPT: &str =
    "Watchdog timer — fork a helper to report status, next steps, and PLAN.md progress.";
...
fn summarize_agent(meta: &SubagentMetadata) -> serde_json::Value {
    let name = meta
        .label
        .clone()
...
fn summarize_agents(
    ...
    entries
        .iter()
        .filter(|entry| entry.parent_session_id == Some(caller_id))
        .map(summarize_agent)
        .collect()
}
...
fn attach_metadata(
    ...
    value["subagent_tool_metadata"] = json!({
        "subagents": subagents,
        "now_local": now_local,
    });
    value
...
impl ToolHandler for SubagentToolHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        let agents_summary = summarize_agents(&registry_entries, caller_id);
        ...
        let result: Result<ToolOutput, FunctionCallError> = async {
            match tool_name.as_str() {
                "subagent_spawn" => {
                    ...
                    let display_label = label.clone().unwrap_or_else(|| "<unlabeled>".to_string());
                    exec_logger = Some(
                        ExecEventLogger::new(
                            session.clone(),
                            turn.clone(),
                    ...
                    let mut agents = agents_summary.clone();
                    if metadata.parent_session_id == Some(caller_id) {
                        agents.push(summarize_agent(&metadata));
                    }

                    let response = attach_metadata(json!({
        ...
        "subagent_watchdog" => {
            let args: WatchdogArgs = serde_json::from_str(&arguments).map_err(|err| {
                FunctionCallError::RespondToModel(format!(
                    ...
                    let watchdog_prompt = load_watchdog_prompt().await;
                    ...
                    let response = attach_metadata(json!({
                        ...
                        "message": prompt,
                        "developer_prompt": watchdog_prompt,
                        "action": action,
                    }), &agents_summary, &now_local);
                ...
                "subagent_list" => {
                    let entries = registry_entries
                        ...
                        .collect::<Vec<_>>();
                    let payload = attach_metadata(json!({
                        "sessions": entries,
                    }), &agents_summary, &now_local);
                    exec_logger = Some(
                        ExecEventLogger::new(
                ...
                "subagent_logs" => {
                    ...
                    let response = attach_metadata(response, &agents_summary, &now_local);
