...
const MAX_AWAIT_TIMEOUT_MS: u64 = 8 * 60 * 1000;
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        let result: Result<ToolOutput, FunctionCallError> = async {
            match tool_name.as_str() {
                "subagent_spawn" => {
                    let args: SpawnArgs = serde_json::from_str(&arguments).map_err(|err| {
                ...
                "subagent_fork" => {
                    let args: ForkArgs = serde_json::from_str(&arguments).map_err(|err| {
                        FunctionCallError::RespondToModel(format!(
                ...
                "subagent_send_message" => {
                    ...
                    if agent_id == 0 && interrupt {
                        ...
                        let sender_agent_id = registry_by_agent
                            .values()
                ...
                "subagent_await" => {
                    ...
                    let timeout = Some(resolve_await_timeout(timeout_ms)?);

                    let target_session = if let Some(id) = agent_id {
                        Some(require_agent_session(&registry_by_agent, id)?)
                    } else {
                        registry_entries
                            .iter()
                            .filter(|m| m.parent_session_id == Some(caller_id))
                    ...
                    let Some(session_id) = target_session else {
                        return Err(FunctionCallError::RespondToModel(
                            "no active subagents to await; call subagent_await without agent_id unless you are blocked on a specific subagent"
                                .to_string(),
                        ));
                    };
                    ...
                    match manager
                        .await_inbox_and_completion(&session_id, timeout)
                        .await
                    {
                        Ok(result) => {
                        ...
                        Err(err) => Err(map_manager_error(err, agent_id)),
                    }
                }
                "subagent_prune" => {
...
fn resolve_await_timeout(timeout_ms: Option<u64>) -> Result<Duration, FunctionCallError> {
    let raw = timeout_ms.unwrap_or(MAX_AWAIT_TIMEOUT_MS);
    if raw == 0 {
        return Err(FunctionCallError::RespondToModel(
            "subagent_await timeout_ms must be greater than zero.".to_string(),
        ));
    }
    if raw > MAX_AWAIT_TIMEOUT_MS {
        return Err(FunctionCallError::RespondToModel(format!(
            "subagent_await timeout_ms ({raw} ms) exceeds the 8-minute limit ({MAX_AWAIT_TIMEOUT_MS} ms). Use shorter waits and poll children with exponential backoff (e.g., 30s -> 60s -> 120s)."
        )));
    }
...
mod tests {
    ...
    fn render_logs_page_trims_tail_for_backward() {
        ...
        // Header + last two assistant lines.
        assert_eq!(page.lines.len(), 3);
        assert!(page.lines[1].contains("three"));
        assert!(page.lines[2].contains("four"));
    ...
    fn render_logs_page_trims_head_for_forward() {
        ...
        // Header + first two assistant lines.
        assert_eq!(page.lines.len(), 3);
        assert!(page.lines[1].contains("one"));
        assert!(page.lines[2].contains("two"));
        assert!(!page.has_more_before);
        assert!(!page.has_more_after);
    }
    ...
    fn resolve_timeout_defaults_to_max() {
        ...
        assert_eq!(duration.as_millis() as u64, MAX_AWAIT_TIMEOUT_MS);
    }

    #[test]
    fn resolve_timeout_rejects_excessive_values() {
        let err = resolve_await_timeout(Some(MAX_AWAIT_TIMEOUT_MS + 1)).unwrap_err();
        match err {
            FunctionCallError::RespondToModel(msg) => {
                assert!(msg.contains("8-minute"), "unexpected message: {msg}");
            }
    ...
    #[test]
    fn require_agent_session_finds_any_agent() {
        let session_one = ConversationId::new();
        let mut registry = HashMap::new();
        registry.insert(9, metadata(session_one, Some(ConversationId::new()), 9));
        assert_eq!(
            require_agent_session(&registry, 9).expect("should find agent"),
            session_one
        );
    }

    #[test]
    fn require_agent_session_errors_for_unknown_agents() {
        let registry = HashMap::new();
        let err = require_agent_session(&registry, 42).unwrap_err();
        match err {
            FunctionCallError::RespondToModel(msg) => {
                assert!(msg.contains("agent_id 42"), "unexpected message: {msg}");
            }
            other => panic!("expected RespondToModel error, got {other:?}"),
        }
    }
