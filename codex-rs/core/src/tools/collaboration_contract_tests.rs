...
const PINNED_UPSTREAM: &str = "cd8dc1e9b6dd24d335f4fcb26546f36d045fd59a";
...
fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("src/tools/fixtures/collaboration_contract_cd8dc1e9.json")
}
...
fn canonical_handler_sources_without_internal_adapters_match_pinned_upstream() {
    for (name, actual, expected) in [
        (
            ...
            include_str!("fixtures/collaboration_source_cd8dc1e9/send_message.rs"),
        ),
        (
            "followup_task.rs",
            include_str!("handlers/multi_agents_v2/followup_task.rs"),
            include_str!("fixtures/collaboration_source_cd8dc1e9/followup_task.rs"),
        ),
        (
            "wait.rs",
            include_str!("handlers/multi_agents_v2/wait.rs"),
            include_str!("fixtures/collaboration_source_cd8dc1e9/wait.rs"),
        ),
        (
            "interrupt_agent.rs",
            include_str!("handlers/multi_agents_v2/interrupt_agent.rs"),
            include_str!("fixtures/collaboration_source_cd8dc1e9/interrupt_agent.rs"),
        ),
    ] {
