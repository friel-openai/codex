...
fn create_subagent_send_message_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Send follow-up questions or tasks to another agent you can see in `subagent_list`. \
            The target `agent_id` must be echoed exactly so Codex can reject stale lookups. Provide a new prompt to push the \
            agent back into `running`; omit the prompt if you only need to rename it or wake it without new work. \
            Set `interrupt=true` to preempt the agent before delivering the payload; interrupts \
            are only honored for non-root agents. Agents retain their existing sandbox; you may only request \
            downgrades to `read_only` or `workspace_write` when spawning or \
            forking. Each child stores only the latest 200 log events, so pair \
            this with `subagent_logs` for progress checks and remind the child to \
            summarize results before you call `subagent_await`."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_subagent_await_tool() -> ToolSpec {
    ...
    properties.insert(
        "agent_id".to_string(),
        JsonSchema::Number {
            description: Some("Numeric child agent id to await.".to_string()),
        },
    );
    properties.insert(
        "timeout_ms".to_string(),
        JsonSchema::Number {
            description: Some(
                "Optional timeout in milliseconds (max 480000 ms / 8 minutes). \
                Omit to use the maximum and prefer shorter waits with \
                exponential backoff (30s -> 60s -> 120s) so you can check on \
                children regularly."
                    .to_string(),
            ),
        },
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Drain the inbox for another agent and optionally \
            observe its terminal completion. `subagent_await` is the sole \
            delivery mechanism for cross-agent messages: each call returns a \
            `messages` array containing any queued items (with sender and \
            recipient ids), plus an optional `completion` object when the \
            child has reached a terminal state. Successful calls move the \
            agent’s status to `idle`, `failed`, or `canceled` when a \
            completion is present, but the agent remains listed until you \
            explicitly run `subagent_prune`. Provide `timeout_ms` (capped at \
            8 minutes / 480000 ms) to bound how long you wait—timeouts leave \
            the agent in its current status and return `timed_out=true` with \
            an empty `messages` array. Use shorter waits with exponential \
            backoff (30s -> 60s -> 120s, etc.) so you can check for progress \
            and send interrupts if an agent stalls."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
