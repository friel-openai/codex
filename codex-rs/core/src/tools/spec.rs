...
fn create_subagent_watchdog_tool() -> ToolSpec {
    ...
    properties.insert(
        ...
        JsonSchema::Number {
            description: Some(
                "Optional timer interval in seconds (minimum 300, default 300).".to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Required prompt used for the watchdog, provide the user's overarching goal and requirements; defaults to a status/progress prompt.".to_string(),
            ),
        },
    );
    properties.insert(
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Start, replace, or cancel a background watchdog (timer, like JS `setInterval`) that auto-forks a subagent with the given message (a) the target agent and all of its descendants go idle, or (b) any descendant has been running for the interval. Watchdogs are per-session timers and only consume a slot when they fire.".to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
