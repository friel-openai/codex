...
fn create_spawn_agent_tool() -> ToolSpec {
    ...
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(format!(
                "Optional agent type to spawn ({}). When spawn_mode=watchdog, leave agent_type unset (default).",
    ...
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Optional spawn_mode: spawn (default), fork (preserve history), or watchdog (monitor the current agent for idle intervals. Watchdog returns a handle you can close; the handle does not run the prompt itself. The prompt is used when spawning watchdog helper forks during idle intervals, and watchdog instructions are injected as developer guidance)."
                    .to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::Number {
            description: Some(
                "Optional watchdog interval in seconds. Only used when spawn_mode=watchdog; the watchdog should check in only after the owner agent has been idle for roughly this long. Must be greater than zero."
                    .to_string(),
            ),
        },
    );
...
fn create_send_input_tool() -> ToolSpec {
    ...
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Identifier of the agent to message. Omit (or use \"parent\") to message the parent/root agent when available."
                    .to_string(),
            ),
        },
    );
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description:
            "Send a message to an existing agent. When `id` is omitted and a parent agent exists, the parent is used."
                .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_compact_parent_context_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description:
            "Watchdog-only: request context compaction for the watchdog's parent thread when it is idle and appears stuck in a non-progress loop."
                .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
