...
use codex_thread_store::ForkSourceReservation;
...
pub(crate) struct ReferenceBackedHistory {
    pub(crate) history: InitialHistory,
    pub(crate) source_reservation: ForkSourceReservation,
}
...
pub(crate) fn full_history_from_frozen_segment(frozen: &FrozenRolloutSegment) -> InitialHistory {
    InitialHistory::Forked(vec![
        RolloutItem::SessionMeta(frozen.source_session_meta.clone()),
        RolloutItem::RolloutReference(frozen.reference.clone()),
    ])
}
...
impl ThreadManager {
    ...
    pub async fn spawn_subagent(
        ...
        let reference_backed_history = self
            .state
            ...
            .await?;
        options.initial_history = reference_backed_history.history;
    ...
    pub async fn resume_thread_with_history(
        ...
        Box::pin(self.reconcile_resumed_thread_ownership(&mut initial_history)).await?;
    ...
    async fn reconcile_resumed_thread_ownership(
        &self,
        initial_history: &mut InitialHistory,
    ) -> CodexResult<()> {
        let resumed_multi_agent_version = initial_history.get_multi_agent_version();
        let InitialHistory::Resumed(resumed) = initial_history else {
            return Ok(());
        };
        let Some(metadata) = Arc::make_mut(&mut resumed.history)
            .iter_mut()
            ...
            .await
        {
            ...
            // Root rollouts historically resumed without consulting the thread store. The
            // optional lookup only discovers a later persisted adoption, so preserve the old
            // behavior when no ownership is present and that overlay cannot be read.
            Err(err)
        ...
        let stored_has_ownership =
            stored.source.is_non_root_agent() || stored.parent_thread_id.is_some();
        let rollout_was_adopted_root = !rollout_has_ownership && stored_has_ownership;
        if resumed_multi_agent_version != Some(MultiAgentVersion::V2) && !rollout_was_adopted_root {
            return Ok(());
        }
        ...
        let parent_thread_id = stored.source.parent_thread_id().or(stored.parent_thread_id);
    ...
    async fn fork_thread_with_initial_history(
        ...
        let (history, fork_persistence) = if let Some(source_thread_id) = source_thread_id {
            ...
            let reference_backed_history = self
                ...
                .await?;
            (
                reference_backed_history.history,
                ForkPersistence::ReferenceBacked {
                    source_reservation: Arc::new(tokio::sync::Mutex::new(Some(
                        reference_backed_history.source_reservation,
                    ))),
...
impl ThreadManagerState {
    ...
    pub(crate) async fn snapshot_rollout_segment(
        &self,
        thread_id: ThreadId,
    ) -> CodexResult<FrozenRolloutSegment> {
    ...
    ) -> CodexResult<ReferenceBackedHistory> {
        let frozen = self.snapshot_rollout_segment(source_thread_id).await?;
        Ok(ReferenceBackedHistory {
            history: full_history_from_frozen_segment(&frozen),
            source_reservation: frozen.source_reservation,
        })
    }
    ...
    async fn reference_backed_snapshot_history(
        ...
        let source_items = materialize_recent_rollout_lines_from(
            codex_home,
            full_history_from_frozen_segment(&frozen)
                .get_rollout_items()
                .iter()
                .cloned()
        ...
        Ok(ReferenceBackedHistory {
            history: InitialHistory::Forked(history),
            source_reservation: frozen.source_reservation,
        })
    }
