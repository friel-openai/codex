...
impl ThreadManager {
    ...
    pub async fn spawn_subagent(
        ...
        options.initial_history = self
            ...
            .await?;
        self.start_thread_inner(options, Some(forked_from_thread_id), agent_control)
            .await
    }
            ...
            if let Some(metadata) = metadata {
                ...
                if let Some(stored) = stored {
                    ...
                    let stored_has_ownership =
                        stored_parent_thread_id.is_some() || stored.source.is_non_root_agent();
                    let rollout_parent_thread_id = metadata
                        .source
                        .parent_thread_id()
                        .or(metadata.parent_thread_id);
                    let rollout_has_ownership =
                        rollout_parent_thread_id.is_some() || metadata.source.is_non_root_agent();
                    if stored_has_ownership
                        && (!rollout_has_ownership
                            || rollout_parent_thread_id != stored_parent_thread_id)
                    {
                        let owner = stored_parent_thread_id
                            .map(|parent| parent.to_string())
                            .unwrap_or_else(|| "its owning agent tree".to_string());
                        return Err(CodexErr::InvalidRequest(format!(
                            "thread {} is owned by {owner}; resume it through its parent or promote it before resuming directly",
                            resumed.conversation_id
                        )));
                    }
                    if metadata.source != stored.source
                        || metadata.parent_thread_id != stored_parent_thread_id
                    ...
                    {
                        metadata.session_id = SessionId::from(resumed.conversation_id);
...
impl ThreadManagerState {
    ...
    pub(crate) async fn reference_backed_full_history(
        ...
        Ok(full_history_from_frozen_segment(frozen))
    }
    ...
    async fn reference_backed_snapshot_history(
        ...
        Ok(InitialHistory::Forked(history))
    }
        ...
        ends_mid_turn: !rollout_items[last_user_position + 1..]
            ...
            .any(|item| match item {
                RolloutItem::EventMsg(EventMsg::AgentMessage(message)) => {
                    matches!(message.phase, None | Some(MessagePhase::FinalAnswer))
                }
