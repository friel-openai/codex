...
impl ThreadManager {
    ...
    async fn fork_thread_with_initial_history(
        ...
        let history = match &fork_persistence {
            ForkPersistence::Referenced { .. } => {
                // The store already froze this paginated source and encoded its lineage in
                // `history_base`. Apply only the requested snapshot transformation here; wrapping
                // it in a second `RolloutReference` would duplicate the inherited history.
