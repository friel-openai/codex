...
pub struct CurrentAgentMembershipHandle {
    ...
    control: Option<LocalAgentControl>,
    ...
    /// Target plus persisted descendants still owned through Open or validated Closed edges.
...
impl CurrentAgentMembershipHandle {
    /// Exact owned subtree captured and fenced before archive or delete storage mutation.
    ...
    pub async fn unload_candidate_runtime_preserving_identity(
        ...
        let Some(lifecycle) = control.registered_agent_lifecycle(thread_id) else {
            return Ok(false);
        };
...
impl ThreadManager {
    ...
    /// Capture and fence the owned persisted and current subtree before archive or delete.
    ///
    ...
    pub async fn prepare_current_agent_membership_eviction(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            ...
            let incoming_edges = agent_graph_store
                .list_thread_spawn_edges_by_child_ids(&descendant_ids)
                .await
                .map_err(|err| {
                    CodexErr::Fatal(format!(
                        "failed to prepare thread-spawn ownership for archive or delete: {err}"
                    ))
                })?
                .into_iter()
                .map(|edge| (edge.child_thread_id, edge))
                .collect::<HashMap<_, _>>();
            let closed_ids = incoming_edges
                .values()
                .filter(|edge| edge.status == ThreadSpawnEdgeStatus::Closed)
                .map(|edge| edge.child_thread_id)
                .collect::<Vec<_>>();
            let closed_metadata =
                if let Some(state_db) = self.state.state_db().await {
                    state_db.get_threads_valid(&closed_ids).await.map_err(|err| {
                    CodexErr::Fatal(format!(
                        "failed to verify closed thread ownership for archive or delete: {err}"
                    ))
                })?
                } else {
                    HashMap::new()
                };
            // close_agent stops a child but retains its parent in SessionSource. Promotion and
            // adoption change that source. Closed edges alone cannot distinguish these cases.
            // The graph returns parents before children, so an excluded owner excludes its subtree.
            for descendant_id in descendant_ids {
                let Some(edge) = incoming_edges.get(&descendant_id) else {
                    continue;
                };
                if !seen_thread_ids.contains(&edge.parent_thread_id) {
                    continue;
                }
                let still_owned = match edge.status {
                    ThreadSpawnEdgeStatus::Open => true,
                    ThreadSpawnEdgeStatus::Closed => {
                        closed_metadata
                            .get(&descendant_id)
                            .and_then(|metadata| {
                                serde_json::from_str::<SessionSource>(&metadata.source).ok()
                            })
                            .and_then(|source| source.parent_thread_id())
                            == Some(edge.parent_thread_id)
                    }
                };
                if still_owned && seen_thread_ids.insert(descendant_id) {
    ...
    async fn agent_control_containing(
        &self,
        thread_id: ThreadId,
    ) -> Option<(ThreadId, LocalAgentControl)> {
        #[expect(
            clippy::needless_collect,
            reason = "collecting releases the thread-map read guard before querying each LocalAgentControl"
        )]
        let threads = self
            .state
            .threads
...
impl ThreadManagerState {
    ...
    fn retained_agent_controls(&self) -> Vec<(ThreadId, LocalAgentControl)> {
        let Ok(mut retained) = self.retained_agent_controls.lock() else {
            return Vec::new();
        };
