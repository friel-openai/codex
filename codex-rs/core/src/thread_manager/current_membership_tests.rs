...
#[test_case::test_case(false; "missing_rollouts")]
#[test_case::test_case(true; "corrupt_rollouts")]
#[tokio::test]
async fn archive_delete_capture_includes_only_indexed_owned_closed_descendants(
    corrupt_rollouts: bool,
) {
    ...
    let closed_owned = ThreadId::new();
    let nested_closed_owned = ThreadId::new();
    let owned_open_leaf = ThreadId::new();
    let promoted = ThreadId::new();
    let promoted_leaf = ThreadId::new();
    let adopted_out = ThreadId::new();
    let adopted_out_leaf = ThreadId::new();
    let destination = ThreadId::new();
    let corrupt_source = ThreadId::new();
    let corrupt_source_leaf = ThreadId::new();
    let missing_metadata = ThreadId::new();
    let source = |parent_thread_id, depth| {
    ...
    for (thread_id, source) in [
        (root, SessionSource::Cli),
        (open_child, source(root, 1)),
        (closed_owned, source(root, 1)),
        (nested_closed_owned, source(closed_owned, 2)),
        (owned_open_leaf, source(nested_closed_owned, 3)),
        (promoted, SessionSource::Cli),
        (promoted_leaf, source(promoted, 1)),
        (adopted_out, source(destination, 1)),
        (adopted_out_leaf, source(adopted_out, 2)),
        (destination, SessionSource::Cli),
        (corrupt_source, source(root, 1)),
        (corrupt_source_leaf, source(corrupt_source, 2)),
    ] {
        let rollout_path = home.path().join(format!("{thread_id}.jsonl"));
        if corrupt_rollouts {
            std::fs::write(&rollout_path, b"not a rollout record\n")
                .expect("write deliberately invalid rollout");
        }
        ...
        .build("openai");
        if thread_id == corrupt_source {
            metadata.source = "invalid indexed SessionSource".to_string();
        }
        state_db
    ...
    for (parent, child, status) in [
        (root, open_child, ThreadSpawnEdgeStatus::Open),
        (root, closed_owned, ThreadSpawnEdgeStatus::Closed),
        (
            closed_owned,
            nested_closed_owned,
        ...
        (root, promoted, ThreadSpawnEdgeStatus::Closed),
        (promoted, promoted_leaf, ThreadSpawnEdgeStatus::Open),
        (root, adopted_out, ThreadSpawnEdgeStatus::Closed),
        (adopted_out, adopted_out_leaf, ThreadSpawnEdgeStatus::Open),
        (root, corrupt_source, ThreadSpawnEdgeStatus::Closed),
        (
            corrupt_source,
            corrupt_source_leaf,
            ThreadSpawnEdgeStatus::Open,
        ),
        (root, missing_metadata, ThreadSpawnEdgeStatus::Closed),
    ] {
        graph
            .upsert_thread_spawn_edge(parent, child, status)
            .await
            .expect("indexed ownership edge");
    }

    assert_eq!(
        manager
            .list_open_agent_subtree_thread_ids(root)
            .await
            .expect("ordinary Open-only subtree")
            .into_iter()
            .collect::<HashSet<_>>(),
        HashSet::from([root, open_child]),
        "archive/delete ownership recovery must not broaden ordinary Open-only traversal",
    );
    let capture = manager
        .prepare_current_agent_membership_eviction(root)
        .await
        .expect("capture from indexed ownership despite unavailable rollout history");
    assert_eq!(
        capture
        ...
        "Closed edges remain owned only when indexed ThreadSpawn source names the exact parent; promotion, adoption, invalid source, and missing metadata stop traversal",
    );
    ...
    for path in rollout_paths {
        if corrupt_rollouts {
            assert_eq!(
                std::fs::read(path).expect("invalid rollout remains unchanged"),
                b"not a rollout record\n",
            );
        } else {
            assert!(
                !path.exists(),
                "capture must not materialize missing rollouts"
            );
        }
    }
}
