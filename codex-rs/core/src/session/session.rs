...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            if let ForkPersistence::ReferenceBacked { source_reservation } =
                &sess.fork_persistence
            {
                // Durable forks keep the source reserved until their RolloutReference is on disk.
                // Ephemeral forks already hold their complete model context and cannot resume.
                let should_materialize = if let Some(live_thread) = sess.live_thread() {
                ...
                source_reservation.lock().await.take();
            }
            {
