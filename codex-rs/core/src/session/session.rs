                ...
                _ => {
                    let live_thread = match &initial_history {
                        ...
                        | InitialHistory::Forked(_) => {
                            let reference_backed_paginated_subagent = is_paginated_subagent
                                && matches!(
                                    &initial_history,
                            ...
                            let subagent_history_start_ordinal =
                                if reference_backed_paginated_subagent {
                                    Some(initial_rollout_ordinal.checked_add(2).ok_or_else(
                                        || anyhow::anyhow!("inherited model context is too large"),
                                    )?)
                                } else {
                                    None
                                };
                            ...
                            if is_paginated_subagent
                                && !reference_backed_paginated_subagent
