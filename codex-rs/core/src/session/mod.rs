...
pub(crate) struct ForkStartupItems {
    ...
    /// Exclusive parent ordinal of this fork, not the source thread's own fork boundary.
...
impl ForkStartupItems {
    ...
    pub(crate) fn with_forked_from_ordinal_exclusive(mut self, cutoff: Option<u64>) -> Self {
        self.forked_from_ordinal_exclusive = cutoff;
        self
    }
}
...
impl Session {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        ...
        // A prepared fork persists references, but its already captured model context also
        // carries owner-scoped settings. Reuse that metadata without reading the lineage again.
        let settings_history = fork_startup_items
            .model_history_override
            .as_deref()
            .unwrap_or_else(|| conversation_history.get_rollout_items());
        let disabled_plugin_ids = disabled_plugin_ids.unwrap_or_else(|| {
            ...
            settings_owner
                .and_then(|thread_id| {
                    codex_history::latest_disabled_plugin_ids(settings_history, thread_id)
                })
                .map(<[String]>::to_vec)
                .unwrap_or_default()
        });
...
impl Session {
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        let invalidate_reviews = state.stage_annotated_history(
            items,
        ...
        if invalidate_reviews {
            std::mem::take(&mut state.history_reset).cancel();
        }
