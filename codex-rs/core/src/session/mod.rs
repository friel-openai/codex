...
pub(crate) enum ForkPersistence {
    Copied,
    ReferenceBacked {
        source_reservation: Arc<Mutex<Option<ForkSourceReservation>>>,
    },
}
...
impl Session {
            ...
            InitialHistory::Forked(mut rollout_items) => {
                ...
                match &self.fork_persistence {
                    ...
                    ForkPersistence::Copied => {
                        // Keep the compact inherited reference and the child's effective settings
                        // in one append so a cold resume cannot observe inherited settings as the
                        // latest value.
                    ...
                    ForkPersistence::ReferenceBacked { .. } => {
                        // The items after the reference may describe an interrupted in-flight
                        // turn. Persist the child's effective settings at the fork boundary,
                        // before that synthesized suffix.
                        let settings_index = persisted_rollout_items
                            .iter()
                            .position(|item| matches!(item, RolloutItem::RolloutReference(_)))
                            .map_or(persisted_rollout_items.len(), |index| index + 1);
                        persisted_rollout_items.insert(settings_index, thread_settings_applied);
                        self.persist_rollout_items(&persisted_rollout_items).await;
                    }
                }
    ...
    pub(crate) async fn record_user_prompt_and_emit_turn_item(
        ...
        let should_materialize = match self.live_thread() {
            Some(live_thread) => !live_thread.is_persistence_deferred().await,
            None => true,
        };
