...
async fn initial_rollout_ordinal(
    ...
    let lines = items
        ...
        .collect();
    materialize_rollout_lines_from(codex_home, lines)
        .await?
        .into_iter()
...
impl Session {
    ...
    async fn record_initial_history(
        ...
        match conversation_history {
            ...
            InitialHistory::Forked(mut rollout_items) => {
                ...
                // The source SessionMeta supplies fork configuration but belongs to the source
                // rollout. The child recorder writes its own canonical SessionMeta, then persists
                // the compact reference and child-local records.
                if !persisted_rollout_items.is_empty() {
    ...
    async fn materialize_forked_history(
        ...
        let lines = rollout_items
            ...
            .collect();
        Ok(materialize_rollout_lines_from(codex_home.as_path(), lines)
            .await?
            .into_iter()
            .map(|line| line.item)
            .collect())
