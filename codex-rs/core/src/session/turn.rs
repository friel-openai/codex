...
pub(crate) async fn run_turn(
        ...
        .await
    {
        ...
        if client_session.is_none() {
            // Guardian has not accepted this review input yet. Keep it pending across
            // compaction, but do not let compaction bypass the selected model's cooldown.
            sess.flush_rollout().await?;
            if !sess
                .wait_for_model_routing_retry(
                    turn_context.model_routing_retry_at,
                    &cancellation_token,
                )
                .await
            {
        ...
        run_auto_compact(
            ...
            client_session.get_or_insert_with(|| sess.services.model_client.new_session()),
    ...
    // Only speculate after hooks and deferred routing finalize tools and permissions.
    {
            ...
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                if refresh_turn_context {
                    let (refreshed_turn_context, refreshed_step_context) = loop {
                        ...
                        let refreshed = sess
                            .refresh_active_turn_context(&turn_context, &prepared.inputs)
                            .await;
                        let step = sess
                    ...
                    loop {
                        ...
                        if prepared
                            ...
                            != turn_context
                                ...
                                .collaboration_mode
                                .model()
                            || prepared.inputs.settings.reasoning_effort()
                                != turn_context.initial_settings.reasoning_effort()
                            || prepared.inputs.settings.selected().service_tier
...
async fn run_sampling_request(
    ...
    loop {
        ...
        if routing_profile_blocks_retry {
            return Err(SamplingRequestFailure {
                error: err,
        ...
        if interrupted_response.swap(false, Ordering::Relaxed) && !*interrupted_response_recorded {
            record_interrupted_response(sess.as_ref(), step_context.as_ref()).await;
            *interrupted_response_recorded = true;
        }
...
async fn record_interrupted_response(sess: &Session, step_context: &StepContext) {
...
async fn try_run_sampling_request(
    ...
    let can_recover_failed_stream = outcome.as_ref().err().is_some_and(|err| {
        err.retry_delay(1).is_some() || classify_model_routing_failure(err.details()).is_some()
    });
    if turn_context.model_profile.is_some() && can_recover_failed_stream {
        ...
        let checkpoint_ready = match active_response_item.take() {
            ...
            Some(item @ ResponseItem::FunctionCall { .. }) => {
                ...
                record_interrupted_tool_call(sess.as_ref(), step_context.as_ref(), item).await;
                interrupted_response.store(true, Ordering::Relaxed);
                true
            }
