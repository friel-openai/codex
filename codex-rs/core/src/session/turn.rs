...
pub(crate) async fn run_turn(
    ...
    loop {
        ...
        if execution_settings_refresh_requested {
            let previous_include_skills_usage_instructions =
            ...
            let refreshed_step_context = sess
                .capture_step_context(Arc::clone(&refreshed_turn_context))
                .await;
            if previous_include_skills_usage_instructions
                != refreshed_turn_context
                    .model_info
                    .include_skills_usage_instructions
                && let Some(skills_update) =
                    build_available_skills_context_update(refreshed_turn_context.as_ref())
            {
                sess.record_conversation_items(refreshed_turn_context.as_ref(), &[skills_update])
                    .await;
            }
            world_state = sess
            ...
            Ok((sampling_request_output, sampling_request_input)) => {
                ...
                if refresh_turn_context {
                    let refreshed_turn_context = sess
                        ...
                        .await;
                    let display_roots =
                        turn_diff_display_roots(refreshed_turn_context.as_ref()).await;
                    turn_diff_tracker
                        .lock()
                        .await
