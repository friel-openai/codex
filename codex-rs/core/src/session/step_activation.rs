...
/// Captures the task and atomic settings/environment version for context replacement.
/// A settings update, environment update, or task replacement invalidates preparation.
pub(super) struct TurnContextReplacement {
    turn: Arc<TurnContext>,
    task_done: Arc<tokio::sync::Notify>,
    pub(super) inputs: Arc<StepInputs>,
}
...
impl Session {
    pub(super) async fn prepare_turn_context_replacement(
        ...
        Ok(TurnContextReplacement {
            ...
            inputs: turn.next_step_input.load_full(),
        })
    }
    ...
    pub(super) async fn try_replace_active_turn_context(
        ...
        self.validate_active_step_settings(
            replacement,
            &replacement.next_step_input.load_full().settings,
