...
use super::ownership::persisted_thread_context_from_history;
use super::ownership::restore_cold_thread_config;
...
impl AgentControl {
    ...
    pub(super) async fn ensure_agent_loaded_locked(
        ...
        let history = load_agent_model_context(state, thread_id, stored_thread.history_mode)
            .await?
            .ok_or(CodexErr::ThreadNotFound(thread_id))?;
        let context = persisted_thread_context_from_history(&history);
        let config = restore_cold_thread_config(
            config,
            &stored_thread,
            context.as_ref(),
            registered_agent.agent_role.as_deref(),
        )
        .await?;
