...
use super::ownership::persisted_thread_resume_context_from_items;
...
impl AgentControl {
    ...
    pub(super) async fn ensure_agent_loaded_locked(
        ...
        // A resumed thread must not inherit prompt overrides from the thread that
        // happened to load it. Base instructions and dynamic tools are restored
        // from the rollout below; role instructions are restored from stored
        // metadata.
        ...
        config.cwd = stored_thread.cwd.clone().try_into().map_err(|err| {
            CodexErr::InvalidRequest(format!(
                "cannot restore the original working directory for agent {thread_id}: {err}"
            ))
        })?;
        ...
        let registered_parent_thread_id = match registered_agent.parent_thread_id {
            Some(parent_thread_id) => Some(parent_thread_id),
            None => state
                .list_live_thread_spawn_edges()
                .await
                .into_iter()
                .find_map(|(parent, child)| (child == thread_id).then_some(parent)),
        };
        let history = load_agent_model_context(state, thread_id, stored_thread.history_mode)
            .await?
            .ok_or(CodexErr::ThreadNotFound(thread_id))?;
        let mut resume_context = persisted_thread_resume_context_from_items(&history);
        resume_context.workspace_roots = resume_context.workspace_roots.filter(|roots| {
            !roots.is_empty()
                && roots
                    .iter()
        ...
        if (session_source != stored_source
            ...
            || registered_parent_thread_id
                .is_some_and(|parent| session_source.parent_thread_id() != Some(parent)))
            ...
            }) = stored_source
        {
            let canonical_source = SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                parent_thread_id: registered_parent_thread_id
                    .or(stored_parent_thread_id)
                    .unwrap_or(parent_thread_id),
                depth: registered_agent.thread_spawn_depth.unwrap_or(depth),
