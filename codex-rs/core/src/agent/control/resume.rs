...
impl LocalAgentControl {
    fn resumed_agent_is_ephemeral(&self, stored_thread: &StoredThread) -> bool {
        self.state
            .evicted_ephemeral(stored_thread.thread_id)
            .unwrap_or_else(|| {
                crate::goal_supervisor::is_goal_supervisor_helper_source(&stored_thread.source)
                    || matches!(
    ...
    async fn inherited_instructions_for_resume(
        &self,
        state: &ThreadManagerState,
        thread_id: ThreadId,
        session_source: &SessionSource,
        parent_thread_id: Option<ThreadId>,
    ) -> Option<SessionInstructions> {
        if let Some(instructions) = self.state.evicted_instructions(thread_id) {
            return Some(instructions);
        }
        if !session_source.is_non_root_agent() {
            return None;
        }
        if let Some(parent_thread_id) = session_source.parent_thread_id().or(parent_thread_id)
            && let Ok(parent) = state.get_thread(parent_thread_id).await
        {
            return Some(parent.session.inherited_instructions().await);
        }
    ...
    pub(super) async fn ensure_agent_loaded_locked(
        ...
        if let Ok(thread) = state.get_thread(thread_id).await {
            self.state.clear_evicted_instructions(thread_id);
        ...
        // Agent delivery inherits the sender's config, not the target's persistence policy.
        // Retain the target's own setting across eviction, including debug-materialized helpers.
        config.ephemeral = self.resumed_agent_is_ephemeral(&stored_thread);
        ...
        // Reserving a resident can unload a nested parent. Capture its applied instructions
        // and attachments before a later live lookup can lose them.
        let parent_thread_id = initial_history
            .get_resumed_parent_thread_id()
            .or(stored_parent_thread_id);
        let inherited_instructions = self
            .inherited_instructions_for_resume(state, thread_id, &session_source, parent_thread_id)
            .await;
        ...
        {
            Ok(reloaded_thread) => {
                self.state.clear_evicted_runtime_settings(thread_id);
                self.state.clear_evicted_instructions(thread_id);
    ...
    pub(super) async fn resume_agent_from_rollout_with_ownership(
        ...
        self.resume_agent_from_rollout_with_ownership_and_instructions(
            config,
            thread_id,
            session_source,
            ownership,
    ...
    pub(super) async fn resume_agent_from_rollout_with_ownership_and_instructions(
        &self,
        config: Config,
        thread_id: ThreadId,
        session_source: SessionSource,
    ...
    async fn resume_single_agent_from_rollout(
        ...
        let multi_agent_version = state
            ...
            .await;
        // Residency reservation can unload the parent before the new runtime is created.
        let inherited_instructions = match inherited_instructions {
            Some(instructions) => Some(instructions),
            None => {
                self.inherited_instructions_for_resume(
                    &state,
                    thread_id,
                    &session_source,
                    parent_thread_id,
        ...
        if let Some(lifecycle) = lifecycle {
            reservation.commit_with_lifecycle(agent_metadata.clone(), lifecycle);
        } else {
        ...
        if multi_agent_version != MultiAgentVersion::V2
            ...
            || crate::goal_supervisor::is_goal_supervisor_helper_source(&notification_source)
        {
        ...
        self.state
            .clear_evicted_instructions(resumed_thread.thread_id);
