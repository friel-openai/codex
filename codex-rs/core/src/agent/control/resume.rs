...
impl AgentControl {
    ...
    pub(super) async fn ensure_agent_loaded_locked(
        ...
        let registered_source_mismatch = match &session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                parent_thread_id,
                depth,
                ..
            }) => {
                registered_agent.parent_thread_id != Some(*parent_thread_id)
                    || registered_agent.thread_spawn_depth != Some(*depth)
            }
            _ => false,
        };
        if (session_source != stored_source
            || session_source.get_agent_path() != registered_agent.agent_path
            || registered_source_mismatch)
            ...
            }) = stored_source
        {
            let canonical_source = SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
                parent_thread_id: registered_agent
                    .parent_thread_id
