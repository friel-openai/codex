...
impl LocalAgentControl {
    /// Preserve the registered transition lock while archive or delete unloads a runtime.
    pub(crate) fn registered_agent_lifecycle(
        &self,
        thread_id: ThreadId,
    ) -> Option<Arc<AgentLifecycle>> {
        self.state.agent_lifecycle(thread_id)
    }
    ...
    /// and parent-mismatched edges are ownership boundaries.
    pub(crate) async fn current_only_descendant_parents_within(
        ...
        self.current_only_descendant_parents_with_seeded_ownership(
            ...
            agent_graph_store,
            /*include_closed_persisted*/ false,
        )
        .await
    }

    /// Include current-only children beneath persisted owners validated for archive or delete.
    ///
    /// The caller has verified that Closed persisted owners still name their exact parent in
    /// indexed SessionSource metadata. Ordinary current-member listing does not use this rule.
    pub(crate) async fn current_only_descendant_parents_for_eviction(
        &self,
        root_thread_id: ThreadId,
        persisted_owned_thread_ids: &HashSet<ThreadId>,
        agent_graph_store: Option<&dyn AgentGraphStore>,
    ...
    async fn current_only_descendant_parents_with_seeded_ownership(
        ...
        agent_graph_store: Option<&dyn AgentGraphStore>,
        include_closed_persisted: bool,
    ) -> CodexResult<HashMap<ThreadId, ThreadId>> {
        let mut remaining_parent_by_thread_id = if include_closed_persisted {
            // A stopped owner can be absent from the registry while its current-only children
            // remain registered. Indexed persisted owners seed reachability below that gap.
            self.state
                .live_agents()
                .into_iter()
                .filter_map(|metadata| Some((metadata.agent_id?, metadata.parent_thread_id?)))
                .collect()
        } else {
