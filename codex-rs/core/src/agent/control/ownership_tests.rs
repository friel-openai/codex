...
#[tokio::test]
async fn cold_root_config_preserves_original_turn_context_and_storage_metadata() {
    let mut config = crate::config::test_config().await;
    config.ephemeral = true;
    config.developer_instructions = Some("adopter-only developer instructions".to_string());
    ...
    assert_eq!(restored.approvals_reviewer, ApprovalsReviewer::User);
    assert_eq!(
