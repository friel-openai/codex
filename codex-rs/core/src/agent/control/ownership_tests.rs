...
async fn cold_root_config_preserves_original_provider_model_reasoning_and_permissions() {
    ...
    config.base_instructions = Some("adopter base".to_string());
    config.developer_instructions = Some("adopter developer".to_string());
    config.personality = Some(Personality::Pragmatic);
    config.approvals_reviewer = ApprovalsReviewer::User;
    config.model_reasoning_summary = Some(ReasoningSummary::Concise);
    let context = PersistedThreadContext {
    ...
    let restored = restore_cold_thread_config(
        config,
        &stored_thread,
        Some(&context),
        /*fallback_role*/ None,
    ...
    assert_eq!(restored.base_instructions, None);
...
async fn cold_root_config_rejects_an_unavailable_original_provider() {
    ...
    let error = restore_cold_thread_config(
        config,
        &stored_thread,
        /*context*/ None,
        /*fallback_role*/ None,
...
#[tokio::test]
async fn cold_thread_config_rejects_workspace_roots_from_another_cwd() {
    let config = crate::config::test_config().await;
    let stored_thread = stored_root(&config);
    let unrelated_root = config
        .codex_home
        .join("unrelated-workspace")
        .try_into()
        .expect("unrelated workspace root should be absolute");
    let context = PersistedThreadContext {
        workspace_roots: Some(vec![unrelated_root]),
        ..Default::default()
    };
    ...
    .await
    .expect("stored cwd should replace incompatible workspace roots");
    let expected_cwd = stored_thread
        .cwd
        .clone()
        .try_into()
        .expect("stored cwd should be absolute");

    assert!(!restored.workspace_roots_explicit);
    assert_eq!(restored.workspace_roots, vec![expected_cwd]);
}
