...
impl AgentControl {
    ...
    pub(crate) async fn adopt_agent_with_communication(
        ...
        let (original, lifecycle) = match thread {
            ...
            None => {
                let persisted_context = persisted_thread_context(&state, &stored_thread).await?;
                let original_config =
                    restore_cold_root_config(config, &stored_thread, persisted_context).await?;
...
pub(super) async fn restore_cold_root_config(
    ...
    persisted_context: Option<PersistedThreadContext>,
) -> CodexResult<Config> {
    config.ephemeral = false;
    config.developer_instructions = None;
    config.personality = None;
    config.approvals_reviewer = ApprovalsReviewer::default();
    ...
    let workspace_roots = persisted_context.as_ref().and_then(|context| {
        context
            .workspace_roots
            .as_ref()
            .map(|workspace_roots| workspace_roots.to_vec())
    });
    if let Some(context) = persisted_context {
        config.personality = context.personality;
        if let Some(approvals_reviewer) = context.approvals_reviewer {
            config.approvals_reviewer = approvals_reviewer;
        }
        config.model_reasoning_summary = Some(context.reasoning_summary);
    }
...
/// Restore the latest recorded turn context without importing another thread's configuration.
        ...
        load_agent_model_context(state, stored_thread.thread_id, stored_thread.history_mode)
            ...
            .find_map(|item| match item {
                RolloutItem::TurnContext(context) => Some(PersistedThreadContext {
                    ...
                    reasoning_summary: context.summary,
                }),
                RolloutItem::SessionMeta(_)
                | RolloutItem::RolloutReference(_)
                | RolloutItem::ResponseItem(_)
