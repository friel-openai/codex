...
struct OriginalRoot {
    ...
    /// Applied instructions captured before unloading a live root.
    instructions: Option<SessionInstructions>,
    /// Transition lock retained by callers waiting across an ownership transfer.
...
impl LocalAgentControl {
    ...
    pub(crate) async fn adopt_agent_with_communication(
        ...
        let (mut original, lifecycle) = match thread {
            Some(thread) => {
            ...
            None => {
                ...
                let original_control = LocalAgentControl::new(
                    Arc::downgrade(&state),
        ...
        let _transition = match lifecycle.as_ref() {
            Some((lifecycle, thread)) => {
                ...
                original.instructions = Some(thread.session.inherited_instructions().await);
                Some(transition)
            }
            None => None,
        };
    ...
    pub(crate) async fn promote_agent(&self, thread_id: ThreadId) -> CodexResult<ThreadId> {
        ...
        if !is_unloadable(thread.as_ref()).await || lifecycle.completion_watcher_active() {
            return Err(CodexErr::InvalidRequest(
        ...
        let original_instructions = thread.session.inherited_instructions().await;
        ...
        let root_control = LocalAgentControl::new(
            Arc::downgrade(&state),
            self.thread_id_generator.clone(),
            ...
            if state.get_thread(thread_id).await.is_err()
                && let Err(rollback_err) = self
                    .resume_agent_from_rollout_with_ownership_and_instructions(
                        ...
                        ResumedThreadOwnership::Transfer,
                        Some(original_instructions.clone()),
    ...
    async fn resume_root_from_rollout(
        ...
        if let Some(lifecycle) = lifecycle {
            self.state.remember_agent_lifecycle(thread_id, lifecycle);
        }
        let resumed = state
