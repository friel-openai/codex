...
impl AgentControl {
    ...
    pub(crate) async fn adopt_agent_with_communication(
        ...
        // Reject loaded subagents before the more expensive ancestry scan.
        if let Ok(thread) = state.get_thread(thread_id).await
            && thread.session_source.is_non_root_agent()
        {
            return Err(CodexErr::InvalidRequest(
        ...
        let (original, lifecycle) = match thread {
            ...
            None => {
                let context = persisted_thread_context(&state, &stored_thread).await?;
                let original_config = restore_cold_thread_config(
                    config,
                    &stored_thread,
                    context.as_ref(),
                    /*fallback_role*/ None,
...
/// Model-visible configuration persisted in the latest turn context.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
pub(super) struct PersistedThreadContext {
    ...
    pub(super) reasoning_summary: Option<ReasoningSummaryConfig>,
}

/// Reconstruct a persisted thread without importing another thread's runtime configuration.
pub(super) async fn restore_cold_thread_config(
    mut config: Config,
    stored_thread: &StoredThread,
    context: Option<&PersistedThreadContext>,
    fallback_role: Option<&str>,
) -> CodexResult<Config> {
    config.ephemeral = false;
    config.base_instructions = None;
    config.developer_instructions = None;
    if let Some(role) = stored_thread.agent_role.as_deref().or(fallback_role) {
        crate::agent::role::apply_role_to_config(&mut config, Some(role))
            .await
            .map_err(CodexErr::InvalidRequest)?;
    }
    config.personality = context.and_then(|context| context.personality);
    config.approvals_reviewer = context
        .and_then(|context| context.approvals_reviewer)
        .unwrap_or_default();
    config.model_reasoning_summary = context.and_then(|context| context.reasoning_summary);
    ...
    let workspace_roots = context
        .and_then(|context| context.workspace_roots.clone())
        .filter(|roots| {
            roots
                .iter()
                .any(|root| config.cwd.as_path().starts_with(root.as_path()))
        });
...
pub(super) fn persisted_thread_context_from_history(
    history: &[RolloutItem],
) -> Option<PersistedThreadContext> {
    history.iter().rev().find_map(|item| match item {
        RolloutItem::TurnContext(context) => Some(PersistedThreadContext {
            workspace_roots: context.workspace_roots.clone(),
            personality: context.personality,
            approvals_reviewer: context.approvals_reviewer,
            reasoning_summary: Some(context.summary),
        }),
        RolloutItem::SessionMeta(_)
        | RolloutItem::RolloutReference(_)
        | RolloutItem::ResponseItem(_)
...
/// Restore the latest recorded context without importing another thread's configuration.
pub(super) async fn persisted_thread_context(
    state: &ThreadManagerState,
    stored_thread: &StoredThread,
) -> CodexResult<Option<PersistedThreadContext>> {
    Ok(
        load_agent_model_context(state, stored_thread.thread_id, stored_thread.history_mode)
            .await?
            .as_deref()
            .and_then(persisted_thread_context_from_history),
    )
}
