...
#[derive(Default)]
pub(super) struct PersistedThreadResumeContext {
    pub(super) workspace_roots: Option<Vec<codex_utils_absolute_path::AbsolutePathBuf>>,
    pub(super) personality: Option<codex_protocol::config_types::Personality>,
    pub(super) approvals_reviewer: Option<codex_protocol::config_types::ApprovalsReviewer>,
    pub(super) reasoning_summary: Option<codex_protocol::config_types::ReasoningSummary>,
}
...
impl AgentControl {
    ...
    pub(crate) async fn adopt_agent_with_communication(
        ...
        let (original, lifecycle) = match thread {
            ...
            None => {
                let resume_context =
                    persisted_thread_resume_context(&state, &stored_thread).await?;
                let original_config =
                    restore_cold_root_config(config, &stored_thread, resume_context).await?;
...
pub(super) async fn restore_cold_root_config(
    ...
    resume_context: PersistedThreadResumeContext,
) -> CodexResult<Config> {
    config.ephemeral = false;
    config.base_instructions = None;
    config.developer_instructions = None;
    config.personality = resume_context.personality;
    config.approvals_reviewer = resume_context.approvals_reviewer.unwrap_or_default();
    config.model_reasoning_summary = resume_context.reasoning_summary;
    ...
    config.workspace_roots_explicit = resume_context.workspace_roots.is_some();
    config.workspace_roots = resume_context
        .workspace_roots
        .unwrap_or_else(|| vec![config.cwd.clone()]);
    config
        .permissions
...
pub(super) async fn persisted_thread_resume_context(
    state: &ThreadManagerState,
    stored_thread: &StoredThread,
) -> CodexResult<PersistedThreadResumeContext> {
    let mut context =
        load_agent_model_context(state, stored_thread.thread_id, stored_thread.history_mode)
            .await?
            .as_deref()
            .map(persisted_thread_resume_context_from_items)
            .unwrap_or_default();
    context.workspace_roots = context.workspace_roots.filter(|roots| {
        !roots.is_empty()
            && roots
                .iter()
                .any(|root| stored_thread.cwd.starts_with(root.as_path()))
    });
    Ok(context)
}

pub(super) fn persisted_thread_resume_context_from_items(
    items: &[RolloutItem],
) -> PersistedThreadResumeContext {
    ...
    for item in items.iter().rev() {
        match item {
            RolloutItem::TurnContext(context) => {
                ...
                personality.get_or_insert(context.personality);
                approvals_reviewer.get_or_insert(context.approvals_reviewer);
                reasoning_summary.get_or_insert(Some(context.summary));
            }
            RolloutItem::EventMsg(EventMsg::ThreadSettingsApplied(event)) => {
                personality.get_or_insert(event.thread_settings.personality);
                approvals_reviewer.get_or_insert(Some(event.thread_settings.approvals_reviewer));
                reasoning_summary.get_or_insert(event.thread_settings.reasoning_summary);
            }
            _ => {}
        }
        if saw_turn_context
    ...
    PersistedThreadResumeContext {
        ...
        approvals_reviewer: approvals_reviewer.flatten(),
        reasoning_summary: reasoning_summary.flatten(),
    }
}
