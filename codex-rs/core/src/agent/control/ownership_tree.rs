...
struct OwnedDescendant {
    ...
    original_control: LocalAgentControl,
    original_config: Config,
    /// Applied instructions captured before unloading a live descendant.
    original_instructions: Option<SessionInstructions>,
    /// Transition lock retained by callers waiting across the ownership transfer.
...
impl LocalAgentControl {
    ...
    pub(super) async fn snapshot_idle_owned_descendants(
            ...
            let (
                ...
                None => {
                    ...
                    (
                        ...
                        .await?,
                        self.state.evicted_instructions(thread_id),
                        false,
                    )
                }
            };
            let original_lifecycle = original_control.state.agent_lifecycle(thread_id);
        ...
        for index in lock_order {
            let descendant = &mut descendants[index];
            let Some(lifecycle) = descendant.original_lifecycle.as_ref() else {
    ...
    pub(super) async fn commit_owned_descendants(
        ...
        for (descendant, prepared_descendant) in tree.descendants.iter().zip(prepared.descendants) {
            ...
            prepared_descendant
                .reservation
                .commit_with_lifecycle(prepared_descendant.metadata, lifecycle);
        }
        for descendant in &tree.descendants {
            descendant
                .original_control
            ...
            if let Some(instructions) = &descendant.original_instructions {
                self.state
                    .remember_evicted_instructions(descendant.thread_id, instructions.clone());
            }
        }
        Ok(())
    }
        ...
        for descendant in &tree.descendants {
            ...
            if let Some(lifecycle) = &descendant.original_lifecycle {
                if descendant.original_visible_when_cold {
                    ...
                    lifecycle.clear_visible_when_cold();
                }
            }
