...
async fn legacy_subagent_followup_reloads_history_and_becomes_evictable_again() {
    ...
    let followup_items = child_thread
        .codex
        .session
        .input_queue
    ...
    let turn = child_thread.codex.session.new_default_turn().await;
    child_thread
        .codex
        .session
        .send_event(
...
async fn legacy_completion_and_followup_are_serialized_without_loss() {
    ...
    let followup_items = reloaded_child
        .codex
        .session
        .input_queue
        .drain_mailbox_input_items()
    ...
    let second_turn = reloaded_child.codex.session.new_default_turn().await;
    reloaded_child
        .codex
        .session
        .send_event(
...
async fn queued_followup_keeps_legacy_child_loaded_until_next_completion() {
    ...
    let second_turn = child_thread.codex.session.new_default_turn().await;
    child_thread
        .codex
        .session
        .send_event(
