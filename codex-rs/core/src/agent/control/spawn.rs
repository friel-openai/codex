    ...
    pub(crate) async fn restore_v2_agent_metadata(
        ...
        for thread_id in descendant_ids {
            ...
            let restore_result = async {
                // Cold registration needs identity only; legacy thread reads also scan rollouts.
                let indexed_identity =
                    state
                        ...
                        .and_then(|metadata| {
                            ...
                            Some((
                                agent_path,
                                metadata.agent_role.or_else(|| source.get_agent_role()),
                                metadata.agent_nickname.or_else(|| source.get_nickname()),
                            ))
                        });
