...
async fn make_cold_multi_agent_v1_child(
    ...
    let lifecycle = harness
        ...
        .expect("v1 child should remain registered");
    ...
    thread
        ...
        .persist_rollout_items(&[RolloutItem::EventMsg(EventMsg::ThreadSettingsApplied(
            ThreadSettingsAppliedEvent {
                thread_id: Some(thread.session.thread_id),
...
#[tokio::test]
async fn ensure_agent_loaded_preserves_evicted_parent_instructions() {
    check_v2_agent_reload(V2ReloadRoute::Generic).await;
}

/// Exercises both V2-specific loaders and the generic delivery loader.
#[derive(Clone, Copy)]
enum V2ReloadRoute {
    Sender,
    NestedParent,
...
async fn check_v2_agent_reload(route: V2ReloadRoute) {
        ...
        V2ReloadRoute::NestedParent | V2ReloadRoute::Generic => {
            ...
            *parent_thread.session.active_turn.lock().await = None;
            let _ = parent_thread
                ...
                .await;
            match route {
                V2ReloadRoute::NestedParent => harness
                    .manager
    ...
    if matches!(route, V2ReloadRoute::NestedParent | V2ReloadRoute::Generic) {
...
async fn ensure_agent_loaded_preserves_materialized_ephemeral_target() {
    ...
    let manager = harness.control.upgrade().expect("manager");
    assert!(
        harness
            .control
...
async fn multi_agent_v1_cold_delivery_reloads_and_preserves_turn_ancestry() -> anyhow::Result<()> {
    ...
    assert!(persisted.items.iter().any(|item| matches!(
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(message))
            if message.message.contains("cold v1 delivery")
    )));
    Ok(())
}
...
async fn cold_delivery_waits_for_completion_cleanup_before_reloading() -> anyhow::Result<()> {
    ...
    let lifecycle = harness
        ...
        .state
        .agent_lifecycle(child_thread_id)
        .expect("cold child should remain registered");
...
/// Distinguishes global instructions from the thread provider in fork requests.
struct TestUserInstructionsProvider(&'static str);

impl UserInstructionsProvider for TestUserInstructionsProvider {
...
#[tokio::test]
async fn full_history_agent_fork_uses_nonlocal_thread_store() {
    let (home, config) = test_config().await;
    ...
    parent
        .inject_response_items(vec![user_message("nonlocal source history")])
        .await
        .expect("record parent message");
    let spawn_call_id = "nonlocal-full-history-spawn";
    ...
    assert!(history_contains_text(
        child.session.clone_history().await.raw_items(),
        "nonlocal source history",
    ));
    child.flush_rollout().await.expect("flush child history");
    let stored_child = store
        ...
        .await
        .expect("read copied child history");
    let items = stored_child.history.expect("child history").items;
    ...
    assert!(
        ...
        "nonlocal stores must not depend on local rollout segments",
    );
...
#[tokio::test]
async fn failed_fork_flush_shuts_down_and_unregisters_only_the_child() {
    let harness = AgentControlHarness::new().await;
    let (parent_id, parent) = harness.start_thread().await;
    let child = harness
        .manager
        ...
        .await
        .expect("start child before final durability barrier");
    child
        .thread
        .session
        .live_thread()
        .expect("child persistence")
        .discard()
        .await
        .expect("remove writer to inject a durability-barrier failure");
    let expected_error = child
        .thread
        .flush_rollout()
        .await
        .expect_err("missing writer");
    let state = harness.control.upgrade().expect("live manager");
    let error = state
        .flush_fork_or_shutdown(&child)
        .await
        .expect_err("fork must report its failed durability barrier");
    assert_eq!(
        error.to_string(),
        CodexErr::from(expected_error).to_string()
    );
    timeout(Duration::from_secs(5), child.thread.wait_until_terminated())
        .await
        .expect("failed child runtime should terminate");
    assert_thread_not_loaded(&harness.manager, child.thread_id).await;
    assert_eq!(harness.manager.list_thread_ids().await, vec![parent_id]);
    assert!(Arc::ptr_eq(
        &harness
            .manager
    ...
    parent
        .flush_rollout()
        .await
        .expect("parent writer remains live");
    parent.shutdown_and_wait().await.expect("shutdown parent");
}
...
async fn self_contained_paginated_forks_work_without_state_db() {
    ...
    let (parent_thread_id, parent_thread) = harness.start_paginated_thread().await;
    parent_thread
        .inject_response_items(vec![user_message("paginated source without sqlite")])
        .await
        .expect("inject source message");
...
async fn full_history_fork_copies_paginated_history_base_lineage_across_resume() {
    ...
    let (prepared_child, _) = harness
        .manager
        .fork_prepared_thread(StartThreadOptions::new(harness.config.clone()), prepared)
        .await
    ...
    lineage_thread
        ...
        .record_conversation_items(
            turn_context.as_ref(),
            turn_context.model_info(),
            &[spawn_agent_call(spawn_call_id)],
        )
        .await;
    let full_history_child_id = harness
        .spawn_anonymous_child(
...
async fn spawn_agent_can_fork_parent_thread_history_with_sanitized_items() {
    ...
    // Spawn starts the child turn before returning, so the live baseline can already
    // have advanced beyond the inherited parent context.
    timeout(Duration::from_secs(5), async {
        loop {
            if child_thread
                .session
                ...
                .await
                .is_some_and(|context| context.turn_id.as_deref() == Some(child_turn_id.as_str()))
            {
                break;
            }
    ...
    .await
    .expect("child turn should establish its own reference context");
    ...
    let first_child_context_index = physical_items
        .iter()
        .position(|item| {
            matches!(item, RolloutItem::TurnContext(context)
                if context.turn_id.as_deref() == Some(child_turn_id.as_str()))
        })
        .expect("child rollout should persist the synchronized child turn context");
    let first_child_turn_index = physical_items
        .iter()
        .position(|item| {
            matches!(item, RolloutItem::EventMsg(EventMsg::TurnStarted(event))
                if event.turn_id == child_turn_id)
        })
        .expect("child rollout should persist its turn-start boundary");
    assert!(first_child_turn_index < first_child_context_index);
    let inherited_items = &physical_items[..first_child_context_index];
    // Context response items are persisted before their TurnContext, but after TurnStarted.
    let inherited_history_items = physical_items[..first_child_turn_index]
        .iter()
        .filter_map(|item| match item {
            RolloutItem::ResponseItem(envelope) => Some(envelope.item.clone()),
            _ => None,
        ...
        .collect::<Vec<_>>();
    assert_eq!(
        strip_response_item_ids(&inherited_history_items),
        strip_response_item_ids(&expected_history),
        "the complete inherited prefix before the child's turn must preserve sanitized instructions, final answers, and assignment while filtering parent-only chatter"
    );
    assert_eq!(
        strip_response_item_ids(
            history_items
                .get(..inherited_history_items.len())
                .expect("live child history should retain the complete inherited prefix")
        ),
        strip_response_item_ids(&expected_history),
        "the live inherited prefix must match the persisted sanitized prefix independently of child turn progress"
    );
    let inherited_reference_context = inherited_items.iter().rev().find_map(|item| match item {
        RolloutItem::TurnContext(context) => Some(context),
        _ => None,
    });
    assert_eq!(
        inherited_reference_context,
        Some(&parent_reference_context_item),
        "the persisted inherited baseline must precede any context from the child's own turn"
    );
    let latest_child_context = physical_items[first_child_context_index..]
        .iter()
        .rev()
        .find_map(|item| match item {
        ...
        .expect("child rollout should contain its latest turn context");
    assert_eq!(
        child_thread.session.reference_context_item().await.as_ref(),
        Some(latest_child_context),
        "the live child baseline must match the latest persisted child context, not the inherited parent baseline"
    );
...
async fn multi_agent_v2_completion_queues_message_for_direct_parent() {
    ...
    harness
        ...
        .notify_parent_of_terminal_turn(
            crate::agent::api::AgentTurnOutcome {
    ...
    .await
    .expect("captured terminal outcome should queue a direct-parent message");
...
async fn resume_agent_from_rollout_reads_archived_rollout_path() {
    ...
    let resumed_thread_id = harness
        .control
        .resume_agent_from_rollout(sender_config.clone(), child_thread_id, SessionSource::Exec)
        .await
    ...
    assert!(sender_config.ephemeral);
    assert!(!resumed_child.config_snapshot().await.ephemeral);
    assert!(resumed_child.session.services.live_thread.is_some());
    let mut persisted_marker = user_message("persist after ephemeral archived resume");
    // Recording fills missing identity fields; fix them so the disk comparison stays exact.
    persisted_marker.set_id(Some(ResponseItemId::with_suffix(
        "msg",
        "persisted-after-archive",
    )));
    persisted_marker.set_turn_id_if_missing("persisted-after-archive");
    persisted_marker.set_create_time_if_missing(123.into());
    // History classifies unannotated message content as unknown.
    let content = codex_context_fragments::to_annotated_content(&mut persisted_marker)
        .expect("message content");
    ...
    resumed_child
        .inject_response_items(vec![persisted_marker.clone()])
        .await
        .expect("resumed archived history should remain writable");
    ...
    let reopened_store = LocalThreadStore::new(
        LocalThreadStoreConfig::from_config(&harness.config),
        harness.state_db.clone(),
    );
    let persisted = reopened_store
        ...
        .await
        .expect("independent store should reopen archived history");
    let persisted_responses = persisted
        .items
        .iter()
        .filter_map(|item| match item {
    ...
    let persisted_markers = persisted_responses
        .iter()
        .copied()
        .filter(|item| history_contains_text([*item], "persist after ephemeral archived resume"))
        .collect::<Vec<_>>();
    assert_eq!(persisted_markers, vec![&persisted_marker]);
}
...
async fn goal_supervisor_helper_request_uses_parent_cache_key_and_mcp_snapshot_inner(
    ...
    config.cwd = workspace
        .try_into()
        .expect("test workspace should be an absolute path");
    let mcp_server_path = config.codex_home.join("fake_mcp_server.py");
    std::fs::write(
        &mcp_server_path,
        r#"import json
...
"#,
    ...
    let store = Arc::new(LocalThreadStore::new(
        LocalThreadStoreConfig {
            ...
            sqlite: codex_state::SqliteConfig::new_for_testing(config.codex_home.clone()),
    ...
    let manager = ThreadManager::new(
        ...
        Arc::new(codex_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(TestUserInstructionsProvider(USER_MARKER)),
        /*analytics_events_client*/ None,
        crate::thread_manager::passthrough_image_store(),
        store,
    ...
    let parent_instruction_text = parent_input
        .iter()
        .flat_map(|item| item["content"].as_array().into_iter().flatten())
        .filter_map(|content_item| content_item["text"].as_str())
        .collect::<Vec<_>>()
        .join("\n");
    for marker in [USER_MARKER, THREAD_MARKER, AGENTS_MARKER] {
        assert_eq!(
            parent_instruction_text.matches(marker).count(),
            1,
            "the parent request must contain each instruction provider exactly once: {marker}",
        );
    }
    assert!(
        parent_instruction_text
            .find(USER_MARKER)
            .expect("global marker")
            < parent_instruction_text
                .find(THREAD_MARKER)
        ...
        "instruction order must remain global UserInstructionsProvider, ThreadInstructionsProvider, then workspace AGENTS.md",
    );
    let expected_prompt_cache_key = parent_prompt_cache_key.to_string();
    assert_eq!(
    ...
    assert_eq!(
        ...
        parent_input,
        "goal supervisor helpers must preserve the exact parent request prefix, including both instruction providers and workspace AGENTS.md in their original order"
    );
    ...
    assert!(
        child_suffix.iter().all(|item| {
            item["content"]
                ...
                .flatten()
                .all(|content_item| {
                    content_item["text"].as_str().is_none_or(|text| {
                        [USER_MARKER, THREAD_MARKER, AGENTS_MARKER]
                            .into_iter()
                            .all(|marker| !text.contains(marker))
                    })
                })
        }),
        "global, thread, and workspace instructions must remain exclusively in the exact parent prefix"
    );
    assert!(
...
#[tokio::test]
async fn ownership_transfer_cold_reload_preserves_applied_instructions() {
    let server = start_mock_server().await;
    let _child_response = mount_sse_once(
        &server,
        sse(vec![
            ev_response_created("resp-ownership-transfer-child"),
            ev_completed("resp-ownership-transfer-child"),
        ]),
    )
    .await;
    let (home, mut config) = test_config().await;
    ...
    let harness = AgentControlHarness::new_with_config(home, config).await;
    let original_provider = Arc::new(TestThreadInstructionsProvider {
        text: "child applied thread instructions".into(),
        shared: false,
    });
    let original = harness
        ...
        .start_thread(StartThreadOptions {
            user_instructions: Some(LoadedUserInstructions {
                instructions: Some(Instructions {
                    text: "original applied user instructions".to_string(),
                    source: None,
                }),
                warnings: Vec::new(),
            }),
            thread_instructions_provider: Some(original_provider.clone()),
            ..StartThreadOptions::new(harness.config.clone())
        })
        .await
        .expect("start independent source");
    let original_control = original.thread.session.services.agent_control.clone();
    ...
    .await;
    wait_for_agent_status(&original_control, child.thread_id, |status| {
        matches!(status, AgentStatus::Completed(_))
    })
    .await;
    ...
    original.thread.session.mark_interrupted();
    wait_for_live_thread_spawn_children(&original_control, original.thread_id, &[child.thread_id])
        .await;

    // The child keeps its applied snapshot even when the source parent refreshes.
    *original_provider.text.write().unwrap() = "parent updated thread instructions";
    let environments = original
        .thread
        .session
        .services
        .turn_environments
    ...
    original
        ...
        .services
        .agents_md_manager
        .refresh(&harness.config, &environments)
        .await
        .0
        .expect("refresh source parent's thread instructions");
    let destination = harness
        .manager
        .start_thread(StartThreadOptions {
            user_instructions: Some(LoadedUserInstructions {
                instructions: Some(Instructions {
                    text: "destination user instructions".to_string(),
                    source: None,
                }),
                warnings: Vec::new(),
            }),
            thread_instructions_provider: Some(Arc::new(TestThreadInstructionsProvider {
                text: "destination thread instructions".into(),
                shared: false,
            })),
            ..StartThreadOptions::new(harness.config.clone())
        })
    ...
    let destination_control = destination.thread.session.services.agent_control.clone();
    let source = thread_spawn_source(
        destination.thread_id,
        &destination.thread.session_source,
        /*depth*/ 1,
        /*agent_role*/ None,
    ...
    let communication = InterAgentCommunication::new(
        AgentPath::root(),
        source.get_agent_path().unwrap(),
        Vec::new(),
        "continue the transferred task".to_string(),
        /*trigger_turn*/ false,
    );
    destination_control
        .adopt_agent_with_communication(
            ...
            AgentCommunicationContext::new(
                crate::agent_communication::AgentCommunicationKind::Spawn,
                destination.thread_id,
            ),
            source,
            TurnStartOptions {
                parent_turn_id: Some("adoption-parent-turn".to_string()),
                root_turn_id: Some("adoption-root-turn".to_string()),
                turn_trigger: Some("adoption-trigger".to_string()),
                cyber_access_program: Some(
                    codex_protocol::turn_input::CyberAccessProgram::Standard,
                ),
                ..Default::default()
            },
        )
        .await
        .expect("adopt source and its loaded descendant");

    let adopted = harness
        .manager
        .get_thread(original.thread_id)
        .await
        .unwrap();
    let adopted_instructions = adopted.session.inherited_instructions().await;
    assert_eq!(
    ...
    destination_control
        ...
        .await
        .expect("load transferred cold descendant");
    let reloaded = harness.manager.get_thread(child.thread_id).await.unwrap();
    let inherited = reloaded.session.inherited_instructions().await;
    assert_eq!(
        (
            inherited.user.map(|instructions| instructions.text),
            inherited.thread.map(|instructions| instructions.text),
        ),
        (
            Some("original applied user instructions".to_string()),
            Some("child applied thread instructions".to_string()),
        ),
    );
    ...
    assert!(
        destination_control
            .state
            .evicted_instructions(child.thread_id)
            .is_none()
    );
    assert!(harness.manager.captured_ops().iter().any(|(thread_id, operation)| {
        *thread_id == original.thread_id
            && matches!(operation, Op::InterAgentCommunication { start_options, .. }
                if start_options.turn_trigger.as_deref() == Some("adoption-trigger")
                    && start_options.cyber_access_program == Some(codex_protocol::turn_input::CyberAccessProgram::Standard)
                    && start_options.parent_turn_id.as_deref() == Some("adoption-parent-turn")
