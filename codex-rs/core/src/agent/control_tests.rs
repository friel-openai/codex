...
async fn ensure_agent_loaded_reloads_registered_unloaded_agent() {
    ...
    let original_cwd: AbsolutePathBuf = home
        .path()
        .join("original-workspace")
        .try_into()
        .expect("original cwd should be absolute");
    let additional_root: AbsolutePathBuf = home
        .path()
        .join("additional-root")
        .try_into()
        .expect("additional workspace root should be absolute");
    std::fs::create_dir_all(original_cwd.as_path()).expect("create original cwd");
    std::fs::create_dir_all(additional_root.as_path()).expect("create additional root");
    config.cwd = original_cwd.clone();
    config.workspace_roots = vec![original_cwd.clone(), additional_root.clone()];
    ...
    let adopter_cwd: AbsolutePathBuf = harness
        .config
        .codex_home
        .join("adopter-workspace")
        .try_into()
        .expect("adopter cwd should be absolute");
    std::fs::create_dir_all(adopter_cwd.as_path()).expect("create adopter cwd");
    let mut adopter_config = harness.config.clone();
    adopter_config.cwd = adopter_cwd.clone();
    adopter_config.workspace_roots = vec![adopter_cwd.clone()];
    adopter_config.workspace_roots_explicit = true;
    adopter_config
        .permissions
        .set_workspace_roots(adopter_config.workspace_roots.clone());
    adopter_config.personality = Some(Personality::Pragmatic);
    adopter_config.approvals_reviewer = ApprovalsReviewer::User;
    adopter_config.model_reasoning_summary = Some(ReasoningSummary::Concise);
    adopter_config.base_instructions = Some("adopter base instructions".to_string());
    adopter_config.developer_instructions = Some("adopter developer instructions".to_string());

    harness
        .control
        .ensure_agent_loaded(adopter_config, spawned_agent.thread_id)
        .await
    ...
    let restored = reloaded_thread.config().await;
    assert_eq!(restored.cwd, original_cwd);
    assert_eq!(
        restored.workspace_roots,
        vec![original_cwd.clone(), additional_root]
    );
    assert_eq!(restored.personality, Some(Personality::Friendly));
    assert_eq!(restored.approvals_reviewer, ApprovalsReviewer::AutoReview);
    assert_eq!(
        restored.model_reasoning_summary,
        Some(ReasoningSummary::Detailed)
    );
    assert_ne!(
        restored.developer_instructions.as_deref(),
        Some("adopter developer instructions")
    );
...
async fn resume_agent_from_rollout_registers_open_descendants_as_cold() {
    assert_resume_agent_from_rollout_registers_open_descendants_as_cold(
        /*enable_v2_before_resume*/ false,
    )
    .await;
}

#[tokio::test]
async fn legacy_resume_registers_open_descendants_when_v2_is_enabled_by_default() {
    assert_resume_agent_from_rollout_registers_open_descendants_as_cold(
        /*enable_v2_before_resume*/ true,
    )
    .await;
}

async fn assert_resume_agent_from_rollout_registers_open_descendants_as_cold(
    enable_v2_before_resume: bool,
) {
    let harness = AgentControlHarness::new_with_multi_agent_v1().await;
    let (parent_thread_id, parent_thread) = harness.start_thread().await;
    ...
    if enable_v2_before_resume {
        resume_config
            .features
            .enable(Feature::MultiAgentV2)
            .expect("test config should enable multi-agent V2");
        assert_eq!(
            resume_config.multi_agent_version_from_features(),
            MultiAgentVersion::V2
        );
    }
    let resumed_parent_thread_id = harness
        .control
        .resume_agent_from_rollout(resume_config, parent_thread_id, SessionSource::Exec)
        .await
