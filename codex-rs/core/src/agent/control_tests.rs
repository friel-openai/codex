...
async fn spawn_agent_full_history_fork_uses_compact_reference_and_materializes_parent_items() {
    ...
    assert_eq!(
        &history.raw_items()[..parent_history.len()],
        parent_history.as_slice(),
        "full-history forked child history should preserve the exact parent prefix"
    );
    assert_eq!(
        strip_response_item_ids(&history.raw_items()[parent_history.len()..]),
        strip_response_item_ids(expected_child_suffix.as_slice()),
        "child-specific context should begin only after the inherited prefix"
    );
    assert_eq!(
    ...
    assert!(
        no_hint_history.raw_items().starts_with(&parent_history),
        "empty child developer instructions should still preserve the exact parent prefix"
    );
    assert!(
        history_contains_text(
        ...
        "empty child developer instructions should not rewrite inherited developer context"
    );
    assert!(
...
-> anyhow::Result<()> {
    ...
    assert_matches!(
        ...
        [
            ...
            && world_state.state["context_window_guidance"] == NEW_GUIDANCE
    );
...
async fn spawn_agent_full_history_fork_preserves_compacted_parent_prefix() {
    ...
    assert!(
        ...
        "full-history forked child history should preserve compacted parent instructions"
    );
    assert!(
        history_contains_text(history.raw_items(), "Child developer instructions."),
        "full-history forked child should append child developer instructions after the inherited prefix"
    );
    assert!(
        history_contains_text(
            history.raw_items(),
