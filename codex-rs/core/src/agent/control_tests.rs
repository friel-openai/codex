...
async fn shared_mcp_process_id(thread: &CodexThread) -> u64 {
    thread
        .call_mcp_tool("retirement", "shared_counter", None, None)
        .await
...
async fn spawn_agent_fork_from_paginated_parent_persists_reference_backed_model_context() {
    ...
    let inherited_idless_context = child_model_history
        .iter()
        .find(|item| {
        ...
        .expect("model history should contain inherited response item");
    assert_eq!(
...
async fn spawn_agent_full_history_fork_uses_compact_reference_and_materializes_parent_items() {
    ...
    let child_mcp_runtime = child_thread.session.services.latest_mcp_runtime();
    let parent_mcp_runtime = parent_thread.session.services.latest_mcp_runtime();
    assert!(!Arc::ptr_eq(
        &child_mcp_runtime.manager_arc(),
        &parent_mcp_runtime.manager_arc(),
    ));
    ...
    let parent_mcp_tools = parent_mcp_runtime.manager().list_all_tools().await;
    let mut snapshot_tool_names = mcp_tool_snapshot.tools.keys().cloned().collect::<Vec<_>>();
    snapshot_tool_names.sort();
    let mut parent_tool_names = parent_mcp_tools
        .iter()
...
async fn failed_goal_supervisor_waits_for_one_persisted_retry_inner() -> anyhow::Result<()> {
    ...
    state_db
        .thread_goals()
        .set_thread_goal_supervisor_snoozed_until_ms(parent_thread_id, goal_id.as_str(), None)
        .await?;
...
"#,
    ...
    let mcp_runtime = parent.thread.session.services.latest_mcp_runtime();
    let mcp_connection_manager = mcp_runtime.manager();
    assert!(
        mcp_connection_manager
            .wait_for_server_ready("rmcp", Duration::from_secs(5))
            .await,
        "parent MCP server should become ready before forking"
    );
    let parent_mcp_tools = mcp_connection_manager.list_all_tools().await;
    assert!(
        parent_mcp_tools
            .iter()
