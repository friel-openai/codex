...
#[test]
fn adopt_agent_rejects_existing_subagent() {
    run_goal_supervisor_test(
        "adopt_agent_rejects_existing_subagent",
        adopt_agent_rejects_existing_subagent_inner(),
    );
}

async fn adopt_agent_rejects_existing_subagent_inner() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, _parent_thread) = Box::pin(harness.start_thread()).await;
    let child_thread_id = Box::pin(
    ...
    let err = Box::pin(harness.control.adopt_agent_with_communication(
        harness.config.clone(),
        child_thread_id,
        communication,
        context,
...
#[test]
fn ensure_agent_loaded_reloads_registered_unloaded_agent() {
    run_goal_supervisor_test(
        "ensure_agent_loaded_reloads_registered_unloaded_agent",
        ensure_agent_loaded_reloads_registered_unloaded_agent_inner(),
    );
}

async fn ensure_agent_loaded_reloads_registered_unloaded_agent_inner() {
    let (home, mut config) = test_config().await;
    let original_workspace = TempDir::new().expect("create original workspace");
    config.cwd = original_workspace
        .path()
        .to_path_buf()
        .try_into()
        .expect("original workspace should be absolute");
    ...
    let original_workspace_roots = config.workspace_roots.clone();
    ...
    let adopter_workspace = TempDir::new().expect("create adopter workspace");
    let mut adopter_config = harness.config.clone();
    adopter_config.cwd = adopter_workspace
        .path()
        .to_path_buf()
        .try_into()
        .expect("adopter workspace should be absolute");
    adopter_config.workspace_roots = vec![adopter_config.cwd.clone()];
    adopter_config
        .permissions
        .set_workspace_roots(adopter_config.workspace_roots.clone());
    adopter_config.ephemeral = true;
    harness
        .control
    ...
    assert!(!reloaded_config.ephemeral);
    assert_eq!(reloaded_config.cwd, original_cwd);
    assert_eq!(reloaded_config.workspace_roots, original_workspace_roots);
    assert!(reloaded_config.workspace_roots_explicit);
