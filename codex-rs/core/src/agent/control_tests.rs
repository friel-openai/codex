...
#[tokio::test]
async fn plaintext_inter_agent_communication_stores_raw_last_task_message() {
    let harness = AgentControlHarness::new().await;
    let (parent_thread_id, _) = harness.start_thread().await;
    let agent_path = AgentPath::try_from("/root/worker").expect("agent path");
    let spawned_agent = harness
        ...
        .await
        .expect("spawn_agent should succeed");
    let raw_task = "new plaintext task".to_string();
    let rendered_communication = InterAgentCommunication::new(
        AgentPath::root(),
        agent_path,
        Vec::new(),
        format!("structured envelope around {raw_task}"),
        /*trigger_turn*/ true,
    );
    ...
    harness
        ...
        .send_inter_agent_communication(
            spawned_agent.thread_id,
            rendered_communication,
            AgentCommunicationContext::new(AgentCommunicationKind::Followup, ThreadId::new())
                .with_last_task_message_override(raw_task.clone()),
            /*parent_turn_id*/ None,
        )
        .await
        .expect("send_inter_agent_communication should succeed");
...
async fn successful_supervisor_parent_compaction_is_recorded_inner() -> anyhow::Result<()> {
    ...
    let result = harness
        .control
        .compact_parent_for_goal_supervisor_helper(
            helper_thread_id,
            "supervisor-parent-compaction-turn".to_string(),
        )
        .await?;
    assert_matches!(
        result,
