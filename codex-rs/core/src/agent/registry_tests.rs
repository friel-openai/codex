...
#[test]
fn evicted_runtime_settings_retain_and_clear_ephemerality() {
    let registry = AgentRegistry::default();
    let thread_id = ThreadId::new();
    registry.register_root_thread(thread_id);
    assert_eq!(registry.evicted_ephemeral(thread_id), None);

    registry.save_evicted_runtime_settings(thread_id, Vec::new(), true);
    assert_eq!(registry.evicted_ephemeral(thread_id), Some(true));
    assert!(
        registry
            .evicted_environments(thread_id)
            .is_some_and(|environments| environments.is_empty())
    );

    registry.clear_evicted_runtime_settings(thread_id);
    assert_eq!(registry.evicted_ephemeral(thread_id), None);
    assert!(registry.evicted_environments(thread_id).is_none());

    registry.save_evicted_runtime_settings(thread_id, Vec::new(), false);
    assert_eq!(registry.evicted_ephemeral(thread_id), Some(false));
}
