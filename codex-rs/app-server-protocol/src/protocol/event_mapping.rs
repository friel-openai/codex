...
///
/// `encrypted_content` is opaque provider data. Messages containing an encrypted part are omitted
/// from this text projection so app-server clients cannot render ciphertext as plaintext.
...
pub struct InterAgentMessageDisplay {
    ...
    pub author: String,
    /// Plaintext message content after removing the inter-agent envelope, when present.
    pub content: String,
}
