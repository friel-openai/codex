...
mod tests {
    ...
    fn collab_spawn_end_preserves_receiver_agent_metadata() {
        // Regression guard for the May 2026 live watchdog TUI failure: core emitted the watchdog
        // nickname and role in `CollabAgentSpawnEndEvent`, but the app-server notification dropped
        // them, so the TUI rendered a bare thread id and could not classify the watchdog handle.
        ...
        let event = CollabAgentSpawnEndEvent {
            ...
            prompt: "watch the parent".to_string(),
