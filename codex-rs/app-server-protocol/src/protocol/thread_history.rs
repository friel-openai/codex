...
impl ThreadHistoryBuilder {
    ...
    fn handle_collab_agent_spawn_end(
        ...
        self.upsert_item_in_current_turn(ThreadItem::CollabAgentToolCall {
            ...
            sender_thread_id: payload.sender_thread_id.to_string(),
            receiver_thread_ids,
            new_agent_nickname: payload.new_agent_nickname.clone(),
            new_agent_role: payload.new_agent_role.clone(),
            prompt: Some(payload.prompt.clone()),
            model: Some(payload.model.clone()),
            reasoning_effort: Some(payload.reasoning_effort),
            agents_states,
...
mod tests {
    ...
    #[test]
    fn reconstructs_collab_spawn_end_item_with_model_and_agent_metadata() {
        // Regression guard for resumed watchdog transcripts: the rollout has the nickname and
        // role in `CollabAgentSpawnEndEvent`, so app-server replay must preserve both fields.
