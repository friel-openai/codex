...
mod tests {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        // Regression guard for resumed watchdog transcripts: rollout replay must preserve the
        // spawn nickname and role that the live app-server notification exposes, or resumed TUI
        // history regresses to a bare thread id while the live transcript shows `[watchdog]`.
