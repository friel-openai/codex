...
pub(super) async fn stage_legacy_lineage(
    ...
    for (index, (source, target)) in plan.sources.iter().zip(&plan.targets).enumerate() {
        ...
        let (start_ordinal, end_ordinal_exclusive) =
            replay_target(plan, &rollback_plan, index, &mut replay, &mut writer).await?;
    ...
    W: AsyncWrite + Unpin,
{
    ...
    let session_meta = codex_rollout::read_session_meta_line(source.path.as_path())
        .await
        .map_err(migration_error)?;
    let history_base = match source.predecessor.as_ref() {
    ...
    canonicalizer
        ...
        .await?;
    if let Some(LegacyLineagePredecessor::RolloutReference(reference)) = source.predecessor.as_ref()
    {
            ...
            .find(|dependency| dependency.successor_rollout_id == source.rollout_id)
        {
            ...
            let predecessor = plan.targets.get(index.wrapping_sub(1)).ok_or_else(|| {
                migration_error("lineage RolloutReference has no planned predecessor")
            })?;
            reference.rollout_path = predecessor.path.clone();
            reference.thread_id = Some(predecessor.thread_id);
            reference.rollout_id = Some(predecessor.rollout_id);
