...
static PROJECTION_CANDIDATES: LazyLock<Result<JsonlSpanScanner, regex::Error>> = LazyLock::new(
    || {
        JsonlSpanScanner::new(
            r#""type"[ \t\r]*:[ \t\r]*"(?:task_started|turn_started|task_complete|turn_complete|turn_aborted|item_completed)"|\\u"#,
        )
    },
);
...
pub(super) fn project_canonical_record(
    ...
    let changes = if needs_projection {
        project_rollout_line(&parse_paginated_rollout_line(bytes)?)
    } else {
