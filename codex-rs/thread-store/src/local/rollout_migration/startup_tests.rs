...
fn prepend_rollout_reference(path: &Path, reference: RolloutReferenceItem) {
    ...
    for line in original_lines {
        let mut line = codex_rollout::parse_rollout_line(line).expect("parse local record");
...
async fn native_projection_failure_uses_retained_source_without_waiting_for_migration_job() {
    ...
    let mut unsupported_record =
        codex_rollout::parse_rollout_line(source.lines().last().expect("last source record"))
            .expect("parse last source record");
    // Duplicate ordinals are tolerated; an unrepresentable next ordinal still fails projection.
    unsupported_record.ordinal = Some(u64::MAX);
    let unsupported_record =
        serde_json::to_string(&unsupported_record).expect("serialize unsupported ordinal");
    fs::write(&path, format!("{source}{unsupported_record}\n"))
        .expect("append unsupported ordinal");
