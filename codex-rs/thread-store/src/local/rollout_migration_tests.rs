...
fn set_paginated_subagent_history_start(path: &Path, boundary: u64) {
    ...
    let mut first = codex_rollout::parse_rollout_line(lines.next().expect("session metadata line"))
...
fn token_count_with_float_rate_limit(used_percent: f64) -> RolloutItem {
...
async fn lineage_migration_plan_authenticates_and_orders_segmented_legacy_sources() {
    ...
    write_legacy_segment(
        ...
        segment_ids[0],
        vec![
            token_count_with_float_rate_limit(12.5),
            user_message("oldest"),
        ],
    );
...
async fn legacy_fork_reuses_compressed_native_parent_with_decoded_byte_boundary() {
    ...
    let end = write_paginated_segment(
        ...
        /*start_ordinal*/ 0,
        vec![
            token_count_with_float_rate_limit(87.25),
            turn_started("parent"),
...
async fn old_durable_native_parent_journals_keep_their_recorded_targets() {
    for phase in [
        ...
        store
            .recover_legacy_lineage(&journal_path, &mut limiter)
            .await
...
async fn lineage_migration_stages_one_contiguous_paginated_ordinal_space() {
    ...
    let mut saw_float_token_count = false;
    for (index, target) in staged.iter().enumerate() {
        ...
        while let Some(raw) = reader.next_line().await.expect("read staged target") {
            let line = codex_rollout::parse_rollout_line(raw.as_str()).expect("parse staged line");
            assert_eq!(line.ordinal, Some(next_ordinal));
            next_ordinal += 1;
            if let RolloutItem::EventMsg(EventMsg::TokenCount(event)) = &line.item
                && event
                    .rate_limits
                    .as_ref()
    ...
    assert!(
        saw_float_token_count,
        "staged lineage must preserve arbitrary-precision token_count values"
    );
...
async fn migration_rewrites_segmented_paginated_references_as_native_history_base() {
    ...
    let predecessor_end = write_paginated_segment(
        ...
        vec![
            token_count_with_float_rate_limit(87.25),
            user_message("native migration predecessor"),
        ],
    );
    ...
    assert_eq!(json.matches("\"used_percent\":87.25").count(), 1);
    let mut materialized_ordinals = materialized
        .iter()
        .filter_map(|line| line.ordinal)
        .collect::<Vec<_>>();
    materialized_ordinals.sort_unstable();
    assert_eq!(materialized_ordinals, vec![1, 2, 3, 4]);
    assert_eq!(history_base.end_ordinal_exclusive, 3);
...
async fn migration_accepts_paginated_numeric_token_count_records() {
    ...
    assert_eq!(
        serde_json::to_value(
            codex_rollout::parse_rollout_line(&token_count)
...
fn write_token_count_ordinal_reuse_fixture(home: &Path) -> (ThreadId, PathBuf, PathBuf) {
    ...
    let token = codex_rollout::decode_rollout_line(json!({
...
async fn lineage_migration_stages_cross_thread_history_base_without_copying_parent() {
    ...
    let child_head = codex_rollout::parse_rollout_line(
...
async fn migration_keeps_late_completions_in_their_original_turn() {
    ...
    let migrated_mcp = read_rollout(&path)
        .into_iter()
        .find_map(|line| match line.item {
            RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
                turn_id,
    ...
    assert_eq!(
        ...
        Some((
            "old".to_string(),
            Some(codex_protocol::items::McpAppUi {
                resource_uri: "ui://app/resource".to_string(),
                preferred_model_display_mode: codex_protocol::items::McpAppDisplayMode::Fullscreen,
            }),
        ))
    );
}
...
async fn migration_rolls_back_pre_compaction_turns_from_sqlite_history() {
    ...
    checkpoint.guardian_history = Some(
        serde_json::from_value(json!([input_response_message("user", "old question")]))
    ...
    assert_eq!(
        checkpoint.guardian_history.map(|history| history.0),
        Some(Vec::new())
    );
...
#[tokio::test]
async fn migration_preserves_guardian_checkpoint_before_rolled_back_turn() {
    let home = TempDir::new().expect("create Codex home");
    let thread_id = ThreadId::new();
    let guardian_history = vec![input_response_message("user", "surviving question")];
    let RolloutItem::Compacted(mut checkpoint) = compacted(guardian_history.clone()) else {
        unreachable!("compacted helper always creates a compaction checkpoint");
    };
    checkpoint.guardian_history =
        Some(serde_json::from_value(json!(guardian_history)).expect("Guardian checkpoint fixture"));
    let path = write_rollout(
        home.path(),
        thread_id,
        SessionSource::Cli,
        vec![
            started("surviving-turn"),
            user_message("surviving question"),
            completed("surviving-turn"),
            RolloutItem::Compacted(checkpoint),
            started("removed-turn"),
            user_message("removed question"),
    ...
    store
        .migrate_rollouts(apply_options())
        .await
        .expect("migrate rollback after surviving checkpoint");
...
#[tokio::test]
async fn migration_preserves_guardian_prefix_when_rollback_exhausts_raw_boundaries() {
    for with_message_ids in [true, false] {
        for (checkpoint_turn_count, rollback_counts) in
            [(1, vec![1]), (2, vec![2]), (2, vec![1, 1])]
        {
            let home = TempDir::new().expect("create Codex home");
            let thread_id = ThreadId::new();
            let messages = (0..=checkpoint_turn_count)
                .map(|index| {
                    let mut item = input_response_message("user", &format!("question-{index}"));
                    if with_message_ids {
                        let ResponseItem::Message { id, .. } = &mut item else {
                            unreachable!("input_response_message creates a message");
                        };
                    ...
                    item.set_turn_id_if_missing(&format!("turn-{index}"));
                    item
                })
                .collect::<Vec<_>>();
            ...
            let guardian_items = messages
                .iter()
                .enumerate()
                .flat_map(|(index, item)| [item.clone(), tool_call(&format!("tool-{index}"))])
                .collect::<Vec<_>>();
            let surviving_guardian = guardian_items[..2].to_vec();
            let RolloutItem::Compacted(mut initial_checkpoint) =
                compacted(messages[..checkpoint_turn_count].to_vec())
            else {
                unreachable!("compacted helper creates a checkpoint");
            };
            initial_checkpoint.guardian_history = Some(
                serde_json::from_value(json!(guardian_items[..checkpoint_turn_count * 2]))
                    .expect("Guardian checkpoint fixture"),
            );
            let RolloutItem::Compacted(mut latest_checkpoint) = compacted(messages.clone()) else {
                unreachable!("compacted helper creates a checkpoint");
            };
            latest_checkpoint.window_number = Some(2);
            latest_checkpoint.guardian_history = Some(
                serde_json::from_value(json!(guardian_items)).expect("Guardian checkpoint fixture"),
            );
            // Earlier questions exist only in checkpoints, not in the raw turn boundary stack.
            let removed_turn_id = format!("turn-{checkpoint_turn_count}");
            let mut items = vec![
                RolloutItem::Compacted(initial_checkpoint),
                started(&removed_turn_id),
                if with_message_ids {
                    rollout_response_item(messages[checkpoint_turn_count].clone())
                } else {
                    // No raw ResponseItem is available for fingerprint matching in this case.
                    user_message(&format!("question-{checkpoint_turn_count}"))
                },
                completed(&removed_turn_id),
                RolloutItem::Compacted(latest_checkpoint),
            ];
            items.extend(rollback_counts.iter().map(|num_turns| {
                RolloutItem::EventMsg(EventMsg::ThreadRolledBack(ThreadRolledBackEvent {
                    num_turns: *num_turns,
                }))
            }));
            ...
            store
                .migrate_rollouts(apply_options())
                .await
                .expect("migrate rollback with an earlier checkpoint-only turn");
            ...
            let checkpoint = read_rollout(&path)
                ...
                .expect("latest migrated checkpoint");
            assert_eq!(checkpoint.window_number, Some(2));
            assert_eq!(
                checkpoint.replacement_history,
                Some(vec![messages[0].clone().into()]),
                "with_message_ids={with_message_ids}, rollback_counts={rollback_counts:?}",
            );
            assert_eq!(
                checkpoint.guardian_history.map(|history| history.0),
                Some(surviving_guardian),
                "with_message_ids={with_message_ids}, rollback_counts={rollback_counts:?}",
            );
        }
    }
...
#[tokio::test]
async fn migration_preserves_guardian_prefix_after_rollback_of_empty_model_checkpoint() {
    let home = TempDir::new().expect("create Codex home");
    let thread_id = ThreadId::new();
    let mut surviving = input_response_message("user", "surviving question");
    let mut removed = input_response_message("user", "removed steer");
    surviving.set_turn_id_if_missing("shared-turn");
    removed.set_turn_id_if_missing("shared-turn");
    let tool_call = |call_id: &str| {
        let mut item = serde_json::from_value::<ResponseItem>(json!({
            "type": "function_call", "call_id": call_id,
            "name": "exec_command", "arguments": "{}"
        }))
        .expect("tool call fixture");
    ...
    let surviving_guardian = vec![surviving.clone(), tool_call("surviving-tool")];
    let guardian_items = surviving_guardian
        .iter()
        .cloned()
        .chain([removed, tool_call("removed-tool")])
        .collect::<Vec<_>>();
    let RolloutItem::Compacted(mut checkpoint) = compacted(Vec::new()) else {
        unreachable!("compacted helper creates a checkpoint");
    };
    // Token-budget eviction can empty model history without emptying Guardian history.
    checkpoint.guardian_history =
        Some(serde_json::from_value(json!(guardian_items)).expect("Guardian checkpoint fixture"));
    let mut items = vec![started("shared-turn")];
    items.extend(guardian_items.into_iter().map(rollout_response_item));
    items.extend([
    ...
    store
        .migrate_rollouts(apply_options())
        .await
        .expect("migrate rollback with ID-less steers and empty model history");
...
#[test]
fn native_rollback_matches_idless_guardian_boundary_after_item_completed_pairing() {
    let thread_id = ThreadId::new();
    let mut surviving = input_response_message("user", "surviving question");
    let mut removed = input_response_message("user", "removed steer");
    surviving.set_turn_id_if_missing("shared-turn");
    ...
    let surviving_tool = tool_call("surviving-tool");
    let removed_tool = tool_call("removed-tool");
    let surviving_guardian = vec![surviving.clone(), surviving_tool.clone()];
    let RolloutItem::Compacted(mut checkpoint) = compacted(Vec::new()) else {
        unreachable!("compacted helper creates a checkpoint");
    };
    checkpoint.guardian_history = Some(
    ...
    // ItemCompleted supplies IDs after the raw responses, but Guardian retains the ID-less items.
    let lines = vec![
        started("shared-turn"),
        rollout_response_item(surviving),
        completed_user_message(
            thread_id,
            "shared-turn",
            "surviving-id",
        ...
        rollout_response_item(surviving_tool),
        rollout_response_item(removed),
        completed_user_message(thread_id, "shared-turn", "removed-id", "removed steer"),
        rollout_response_item(removed_tool),
        completed("shared-turn"),
        RolloutItem::Compacted(checkpoint),
        RolloutItem::EventMsg(EventMsg::ThreadRolledBack(ThreadRolledBackEvent {
            num_turns: 1,
    ...
    let checkpoint_index = lines
        .iter()
        .position(|line| matches!(line.item, RolloutItem::Compacted(_)))
        .expect("checkpoint index");
    let mut planner = super::rollback_plan::RollbackPlanner::new();
    for line in &lines {
        planner
            .observe_paginated(line)
            .expect("observe native paired response");
    }

    let line = planner
        .finish()
        .apply(checkpoint_index, lines[checkpoint_index].clone())
        .expect("rewrite checkpoint after native rollback")
        .expect("preserve checkpoint");
    let RolloutItem::Compacted(checkpoint) = line.item else {
        panic!("expected rewritten checkpoint");
    };
    assert_eq!(checkpoint.replacement_history, Some(Vec::new()));
    assert_eq!(
        checkpoint.guardian_history.map(|history| history.0),
