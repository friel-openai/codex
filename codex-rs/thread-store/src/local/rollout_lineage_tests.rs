...
use super::LineageRepresentation;
...
async fn normalizes_rollout_references_and_same_thread_rotations() {
    ...
    assert_eq!(rotated.segments[0].end_ordinal_exclusive, Some(4));
...
#[tokio::test]
async fn referenced_subagent_uses_persisted_local_history_boundary() {
    let home = TempDir::new().expect("temp dir");
    let store = LocalThreadStore::new(test_config(home.path()), /*state_db*/ None);
    let parent = ThreadId::default();
    let child = ThreadId::default();
    ...
    let child_path = write_referenced_rollout_with_local_boundary(
        home.path(),
        child,
        parent,
        parent_path.clone(),
        /*reference_ordinal*/ 4,
        /*local_history_start_ordinal*/ 5,
        /*next_ordinal*/ 7,
    );
    ...
    let lineage = store
        .resolve_rollout_lineage(child)
        .await
        .expect("resolve referenced subagent");
...
async fn history_base_cutoff_survives_parent_rotation() {
    ...
    write_referenced_rollout_at(
        ...
        /*reference_ordinal*/ 6,
        /*local_history_start_ordinal*/ None,
        /*next_ordinal*/ 8,
    );
    let child_path = write_rollout(
        home.path(),
...
fn write_referenced_rollout_with_local_boundary(
    home: &Path,
    thread_id: ThreadId,
    referenced_thread_id: ThreadId,
    referenced_path: std::path::PathBuf,
    reference_ordinal: u64,
    local_history_start_ordinal: u64,
    next_ordinal: u64,
) -> std::path::PathBuf {
    let directory = home.join("sessions/2026/07/16");
    fs::create_dir_all(directory.as_path()).expect("create rollout directory");
    let path = directory.join(format!("rollout-2026-07-18T00-00-00-{thread_id}.jsonl"));
    write_referenced_rollout_at(
        path.as_path(),
        thread_id,
        referenced_thread_id,
        referenced_path,
        reference_ordinal,
        Some(local_history_start_ordinal),
        next_ordinal,
    );
    path
}
...
fn write_referenced_rollout_at(
    ...
    referenced_path: std::path::PathBuf,
    reference_ordinal: u64,
    local_history_start_ordinal: Option<u64>,
    next_ordinal: u64,
) {
    let mut lines = vec![
        rollout_line(
            ...
            RolloutItem::SessionMeta(SessionMetaLine {
                meta: SessionMeta {
                    ...
                    id: thread_id,
                    history_mode: ThreadHistoryMode::Paginated,
                    subagent_history_start_ordinal: local_history_start_ordinal,
                    ..SessionMeta::default()
                },
                git: None,
            }),
