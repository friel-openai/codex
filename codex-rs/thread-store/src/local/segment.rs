...
pub(super) async fn freeze_thread_segment(
    ...
    let live_entry = store.live_recorders.lock().await.get(&thread_id).cloned();
    if let Some(live_entry) = live_entry.as_ref() {
        live_entry
            .recorder
            .persist()
            .await
    ...
    let recorded_path = match live_entry.as_ref() {
        Some(entry) => entry.recorder.rollout_path().to_path_buf(),
    ...
    if params.is_snapshot() {
        ...
        return Ok(FrozenRolloutSegment {
            reference: RolloutReferenceItem {
                ...
                max_depth: MAX_ROLLOUT_REFERENCE_DEPTH,
...
fn stabilize_rollout_reference<'a>(
    store: &'a LocalThreadStore,
    reference: RolloutReferenceItem,
    active_references: &'a mut HashSet<(ThreadId, Option<SegmentId>)>,
    depth: usize,
) -> Pin<Box<dyn Future<Output = ThreadStoreResult<RolloutReferenceItem>> + Send + 'a>> {
    Box::pin(async move {
        if depth >= MAX_ROLLOUT_REFERENCE_DEPTH {
            return Err(ThreadStoreError::Internal {
                message: format!(
        ...
        let result = async {
            let resolved_path =
                codex_rollout::resolve_rollout_reference_path(&store.config.codex_home, &reference)
                    .await
                    .map_err(thread_store_io_error)?;
            let (mut lines, loaded_thread_id, parse_errors) =
            ...
            let mut nested_reference_changed = false;
            for line in lines.iter_mut().skip(1) {
                let RolloutItem::RolloutReference(nested_reference) = &mut line.item else {
                    continue;
                };
                let stabilized = stabilize_rollout_reference(
                    store,
                    nested_reference.clone(),
                    active_references,
                    depth + 1,
                )
                .await?;
                nested_reference_changed |=
                    !rollout_references_equal(nested_reference, &stabilized);
                *nested_reference = stabilized;
            }

            if !nested_reference_changed
            ...
            let segment_id = snapshot_segment_id(lines.as_slice())?;
            let immutable_path = immutable_segment_path(
                store.config.codex_home.as_path(),
                thread_id,
                Some(segment_id),
            ...
            install_snapshot_segment(lines.as_slice(), immutable_path.as_path(), segment_id)
                .await?;
            Ok(RolloutReferenceItem {
                rollout_path: immutable_path,
                thread_id: Some(thread_id),
                rollout_timestamp: reference.rollout_timestamp,
                segment_id: Some(segment_id),
...
async fn validate_source_rollout(
    ...
    Ok((source_meta, next_rollout_ordinal, existing_reference, lines))
}
...
async fn install_snapshot_segment(
    ...
    let write_result = async {
        let mut temporary_file = fs::OpenOptions::new()
            ...
            .await?;
        for encoded in
            canonical_snapshot_lines(lines, Some(segment_id)).map_err(io::Error::other)?
        {
