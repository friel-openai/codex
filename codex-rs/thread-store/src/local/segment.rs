...
use crate::ForkSourceReservation;
...
pub(super) async fn freeze_thread_segment(
    ...
    let source_reservation =
        ForkSourceReservation::new(store.live_writer_locks.reserve_lifecycle(thread_id).await);
    ...
    if params.is_snapshot() {
        if live_entry.is_some() && matches!(history_mode, ThreadHistoryMode::Paginated) {
