...
pub(super) async fn rotate_thread_segment(
    ...
    let mut session_meta = old_meta.meta.clone();
    session_meta.agent_nickname = params.source.get_nickname();
    session_meta.agent_role = params.source.get_agent_role();
    session_meta.agent_path = params.source.get_agent_path().map(Into::into);
    session_meta.source = params.source;
    ...
    let entry = live_recorders
        .get_mut(&thread_id)
        .ok_or(ThreadStoreError::ThreadNotFound { thread_id })?;
    let current_path = entry.recorder.rollout_path().to_path_buf();
    if current_path != old_rollout_path {
        return Err(ThreadStoreError::Conflict {
            message: format!("live writer for thread {thread_id} changed during segment rotation"),
        });
    }
    entry.recorder = new_recorder;
    Ok(())
}
