...
impl RolloutLineage {
    ...
    pub(super) fn segment_index_for_cutoff(
        &self,
        thread_id: ThreadId,
        ...
        self.segments
            .iter()
            .position(|segment| {
                segment.thread_id == thread_id && segment.start_ordinal == end_ordinal_exclusive
            })
            .or_else(|| {
                end_ordinal_exclusive.checked_sub(1).and_then(|ordinal| {
                    self.segments.iter().rposition(|segment| {
                        segment.thread_id == thread_id && segment.contains_ordinal(ordinal)
                    })
                })
            })
    }
...
impl RolloutLineageSegment {
    ...
    pub(super) fn end_position(&self) -> Option<HistoryPosition> {
...
#[expect(
    ...
    reason = "recursive lineage resolution carries immutable traversal state"
)]
fn resolve_path<'a>(
    ...
    graph_depth: usize,
    representation: LineageRepresentation,
    active_paths: &'a mut HashSet<PathBuf>,
) -> Pin<Box<dyn Future<Output = ThreadStoreResult<Vec<RolloutLineageSegment>>> + Send + 'a>> {
    Box::pin(async move {
        let writer_guard = match representation {
            LineageRepresentation::Existing => None,
            LineageRepresentation::PlainForReference => {
                Some(store.live_writer_locks.lock(expected_thread_id).await)
            }
        };
        let rollout_path = codex_rollout::existing_rollout_path(rollout_path.as_path())
            .await
        ...
        if !active_paths.insert(rollout_path.clone()) {
        ...
        let current = async {
            ...
            let file_len = tokio::fs::metadata(rollout_path.as_path())
                .await
                .map_err(lineage_io_error)?
                .len();
            Ok::<_, ThreadStoreError>((head, file_len))
        }
        .await;
        drop(writer_guard);
        let (head, file_len) = match current {
            Ok(current) => current,
            Err(err) => {
                active_paths.remove(&rollout_path);
                return Err(err);
            }
        };

        let result = async {
            let mut segments = if let Some((reference_ordinal, reference)) =
                head.leading_reference.as_ref()
            {
                // `RolloutReferenceItem::max_depth` bounds recent replay. Paginated reads expose
                // complete history, so only the global graph bound applies here.
                ...
                .await
                .map_err(lineage_io_error)?;
                let filter_texts = inherited_filter_texts
                    .clone()
                    .or_else(|| reference.compacted_replacement_history_filter_texts.clone());
                let mut referenced_segments = resolve_path(
                    store,
                ...
                if let Some(nth_user_message) = reference.nth_user_message {
                    trim_before_nth_user_message(&mut referenced_segments, nth_user_message)
                        .await?;
                } else {
                    trim_to_ordinal(
                        &mut referenced_segments,
                        referenced_thread_id,
                        *reference_ordinal,
                    )
                    .await?;
                ...
                referenced_segments
            } else if let Some(history_base) = head.session_meta.meta.history_base {
                if graph_depth >= codex_rollout::MAX_ROLLOUT_REFERENCE_DEPTH {
                    let detail = format!(
                        "rollout reference graph exceeds maximum depth of {}",
                        codex_rollout::MAX_ROLLOUT_REFERENCE_DEPTH
...
async fn read_rollout_head(path: &Path) -> ThreadStoreResult<RolloutHead> {
    ...
    let default_first_local_ordinal = match session_meta.meta.history_base {
        Some(base) => base
    ...
    let (leading_reference, first_local_ordinal) = match next {
        ...
        None => (None, default_first_local_ordinal),
    };
    Ok(RolloutHead {
        session_meta,
        leading_reference,
...
async fn trim_to_history_position(
    ...
    let file_len = tokio::fs::metadata(segment.rollout_path.as_path())
        .await
        .map_err(lineage_io_error)?
        .len();
    let is_plain = segment.rollout_path == codex_rollout::plain_rollout_path(&segment.rollout_path);
    if is_plain && end.end_byte_offset > file_len {
        return Err(malformed_lineage(
            end.thread_id,
            "cutoff byte offset is past the source rollout",
        ));
    }
    segment.end_byte_offset = if is_plain {
        Some(end.end_byte_offset)
    } else {
        byte_offset_for_ordinal(segment.rollout_path.as_path(), end.end_ordinal_exclusive).await?
    };
    Ok(())
}
...
async fn trim_before_nth_user_message(
    ...
    let mut canonical_boundaries = Vec::new();
    let mut response_boundaries = Vec::new();
    let mut active_turn_start = None;
    let mut active_response_boundary = None;
    let mut active_has_canonical_boundary = false;
    let mut pending_legacy_response_boundary = None;
    ...
    for (segment_index, segment) in segments.iter().enumerate() {
        ...
        for line in lines {
            ...
            let preceding_legacy_response_boundary = pending_legacy_response_boundary.take();
            ...
            match line.item {
                RolloutItem::EventMsg(EventMsg::TurnStarted(_)) => {
                    ...
                    active_response_boundary = None;
                    active_has_canonical_boundary = false;
                }
                RolloutItem::EventMsg(EventMsg::UserMessage(_)) => {
                    saw_canonical_user_message = true;
                    let boundary = active_turn_start
                        .or(preceding_legacy_response_boundary)
                        .unwrap_or(boundary);
                    if canonical_boundaries.last() != Some(&boundary) {
                        canonical_boundaries.push(boundary);
                    }
                ...
                RolloutItem::ResponseItem(item) if item.is_user_message() => {
                    if response_boundaries.last() != Some(&boundary) {
                        response_boundaries.push(boundary);
                    }
                    if active_turn_start.is_some() {
                        active_response_boundary = Some(boundary);
                    } else if !response_follows_unframed_canonical_boundary {
                        pending_legacy_response_boundary = Some(boundary);
                    }
                }
                RolloutItem::EventMsg(EventMsg::TurnComplete(_) | EventMsg::TurnAborted(_)) => {
                    if !active_has_canonical_boundary
                        && let Some(boundary) = active_response_boundary
                        && canonical_boundaries.last() != Some(&boundary)
                    {
                        canonical_boundaries.push(boundary);
                    }
                ...
                RolloutItem::EventMsg(EventMsg::ThreadRolledBack(rollback)) => {
                    ...
                    canonical_boundaries.truncate(canonical_boundaries.len().saturating_sub(count));
    ...
    let boundaries = if saw_canonical_user_message {
