...
impl LineageResolution {
    fn root(representation: LineageRepresentation) -> Self {
        Self::at_depth(representation, 0)
    }

    fn at_depth(representation: LineageRepresentation, graph_depth: usize) -> Self {
        Self {
            graph_depth,
            representation,
        }
    ...
    fn descend(self) -> Self {
        Self {
            graph_depth: self.graph_depth + 1,
            ..self
        }
    }
}
...
impl LocalThreadStore {
    ...
    async fn resolve_rollout_lineage_with_representation(
        ...
        let segments = resolve_path(
            ...
            LineageResolution::root(representation),
            &mut active_paths,
        )
        .await?;
    ...
    pub(super) async fn resolve_rollout_lineage_at(
        ...
        let segments = resolve_path(
            ...
            Some(end),
            /*inherited_filter_texts*/ None,
            LineageResolution::root(LineageRepresentation::Existing),
            &mut active_paths,
        )
        .await?;
        Ok(RolloutLineage { segments })
...
fn resolve_path<'a>(
    ...
    Box::pin(async move {
        let writer_guard = match resolution.representation {
        ...
        let rollout_path = match resolution.representation {
        ...
        let result = async {
                ...
                head.leading_reference.as_ref()
            {
                ...
                if resolution.graph_depth >= codex_rollout::MAX_ROLLOUT_REFERENCE_DEPTH {
                    let detail = format!(
