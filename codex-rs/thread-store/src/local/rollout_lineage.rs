...
impl LocalThreadStore {
    ...
    /// Resolves the same canonical lineage while ensuring every referenced physical rollout has
    /// a stable plain representation suitable for a byte-addressed fork boundary.
    pub(super) async fn resolve_rollout_lineage_for_reference(
        ...
        for segment in lineage.segments.iter_mut().rev() {
            let _writer_guard = self.live_writer_locks.lock(segment.thread_id).await;
            let existing_path =
                codex_rollout::existing_rollout_path(segment.rollout_path.as_path())
                    .await
                    .ok_or_else(|| {
                        malformed_lineage(segment.thread_id, "missing referenced rollout")
                    })?;
            let scoped_path = super::helpers::scoped_rollout_path(
                self.config.codex_home.clone(),
                existing_path.as_path(),
                "Codex home",
            )?;
            segment.rollout_path =
                codex_rollout::materialize_rollout_for_reference(scoped_path.as_path())
                    .await
            ...
            segment.end_byte_offset = match segment.end_ordinal_exclusive {
                Some(end_ordinal_exclusive) => {
                    byte_offset_for_ordinal(segment.rollout_path.as_path(), end_ordinal_exclusive)
                        .await?
                }
                None => Some(
                    tokio::fs::metadata(segment.rollout_path.as_path())
                        .await
...
fn resolve_path<'a>(
    ...
    Box::pin(async move {
        ...
        let result = async {
            ...
            let (rollout_path, file_len) =
                existing_rollout_path_and_len(rollout_path.as_path(), expected_thread_id).await?;
...
async fn existing_rollout_path_and_len(
    path: &Path,
    thread_id: ThreadId,
) -> ThreadStoreResult<(PathBuf, u64)> {
    let mut last_not_found = None;
    for _ in 0..3 {
        let existing_path = codex_rollout::existing_rollout_path(path)
            .await
            .ok_or_else(|| malformed_lineage(thread_id, "missing source rollout"))?;
        match tokio::fs::metadata(existing_path.as_path()).await {
            Ok(metadata) => return Ok((existing_path, metadata.len())),
            Err(err) if err.kind() == io::ErrorKind::NotFound => {
                last_not_found = Some(err);
                tokio::task::yield_now().await;
            }
            Err(err) => return Err(lineage_io_error(err)),
        }
    }
    Err(lineage_io_error(last_not_found.unwrap_or_else(|| {
        io::Error::new(io::ErrorKind::NotFound, "missing source rollout")
    })))
}
    ...
    let (leading_reference, first_local_ordinal) = match next {
        ...
        None => (
            ...
            session_meta
                .meta
                .history_base
                .map_or(1, |base| base.end_ordinal_exclusive.saturating_add(1)),
        ),
    };
    Ok(RolloutHead {
        session_meta,
