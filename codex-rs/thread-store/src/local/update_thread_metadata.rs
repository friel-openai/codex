...
mod tests {
    ...
    async fn sqlite_ownership_metadata_survives_rollout_history_hydration() {
        ...
        store
            .update_thread_metadata(UpdateThreadMetadataParams {
                thread_id,
                patch: ThreadMetadataPatch {
                    name: Some(Some("Adopted thread".to_string())),
                    ..Default::default()
                },
                include_archived: false,
            })
            .await
            .expect("reconcile rollout metadata after adoption");
    ...
    #[test]
    fn sqlite_failures_block_for_required_ownership_updates() {
        assert!(sqlite_write_failure_should_block(&ThreadMetadataPatch {
    ...
    #[tokio::test]
    async fn required_ownership_update_fails_without_sqlite() {
        let home = TempDir::new().expect("temp dir");
        let uuid = Uuid::from_u128(323);
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T20-30-00", uuid).expect("session file");
        let store = LocalThreadStore::new(test_config(home.path()), /*state_db*/ None);
        ...
        let err = store
            ...
            .await
            .expect_err("required ownership update without sqlite should fail");

        assert!(err.to_string().contains("sqlite state db unavailable"));
    }
