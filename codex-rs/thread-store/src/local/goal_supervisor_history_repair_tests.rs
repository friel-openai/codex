...
fn repairs_exact_top_level_delivery() {
    let mut lines = vec![
        ...
        delivery(1, true),
        poisoned_message(2, "synthetic supervisor instruction"),
    ];
...
fn same_rollout_repairs_plaintext_and_preserves_fernet() {
    let lines = vec![
        session_meta(AFFECTED_CLI_VERSION),
        delivery(1, true),
        poisoned_message(2, genuine_synthetic_fernet_token()),
        delivery(3, true),
        poisoned_message(4, "synthetic poisoned supervisor instruction"),
    ];
...
fn unknown_non_fernet_payload_fails_closed() {
    let lines = vec![
        session_meta(AFFECTED_CLI_VERSION),
        delivery(1, true),
        poisoned_message(2, "synthetic-opaque-format-v1:0123456789abcdef"),
    ];
    let source = encode_jsonl(&lines);
...
fn repairs_nested_compaction_without_new_provenance_field() {
    let compacted = compacted_message(1, "synthetic compacted instruction");
...
fn selection_repairs_only_consumed_top_level_message() {
    ...
    let mut lines = vec![
        session_meta(AFFECTED_CLI_VERSION),
        delivery(1, true),
        poisoned_message_with_id(2, "selected supervisor instruction", selected_id),
        delivery(3, true),
        poisoned_message_with_id(4, "unknown-opaque-format", excluded_id),
    ];
...
fn duplicate_selected_message_id_fails_before_mutation() {
    ...
    let mut lines = vec![
        session_meta(AFFECTED_CLI_VERSION),
        delivery(1, true),
        poisoned_message_with_id(2, "first supervisor instruction", duplicate_id),
        delivery(3, true),
        poisoned_message_with_id(4, "second supervisor instruction", duplicate_id),
    ];
    let before = serde_json::to_value(&lines).expect("serialize lines");
...
fn duplicate_selected_fernet_message_id_remains_protected() {
    ...
    let mut lines = vec![
        session_meta(AFFECTED_CLI_VERSION),
        delivery(1, true),
        poisoned_message_with_id(2, genuine_synthetic_fernet_token(), duplicate_id),
        compacted_message_with_id(3, genuine_synthetic_fernet_token(), duplicate_id),
    ];
    let before = serde_json::to_value(&lines).expect("serialize lines");
...
fn delivery_serialization_has_no_new_content_encoding_field() {
    let serialized = serde_json::to_value(delivery(1, true)).expect("serialize delivery");
...
fn inherited_provenance_is_same_thread_openai_only() {
    let mut lines = vec![
        ...
        delivery(1, true),
        poisoned_message(2, "synthetic inherited instruction"),
    ];
    let count =
    ...
    meta.meta.id = ThreadId::from_u128(0x22);
    assert_eq!(
...
fn invalid_jsonl_is_preserved_and_breaks_delivery_adjacency() {
    let lines = [
        session_meta(AFFECTED_CLI_VERSION),
        delivery(1, true),
        poisoned_message(2, "synthetic nonadjacent instruction"),
    ];
...
fn rewrite_preserves_unterminated_tail_and_projection() {
    let lines = vec![
        session_meta(AFFECTED_CLI_VERSION),
        delivery(1, true),
        poisoned_message(2, "synthetic unterminated instruction"),
    ];
...
fn non_openai_and_nonaffected_candidates_fail_closed() {
    ...
    for meta in [non_openai, session_meta("0.148.0-alpha.7+frodex.0")] {
        let source = encode_jsonl(&[
            meta,
            delivery(1, true),
            poisoned_message(2, "synthetic natural language instruction"),
        ]);
...
fn supplied_history_rejects_natural_language_and_preserves_fernet() {
    let natural_language = [
        delivery(0, true).item,
        poisoned_message(1, "synthetic supplied instruction").item,
    ];
    let fernet = [
        delivery(0, true).item,
        poisoned_message(1, genuine_synthetic_fernet_token()).item,
    ];
