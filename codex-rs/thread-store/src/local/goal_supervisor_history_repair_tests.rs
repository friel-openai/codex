...
fn rewrite_preserves_unterminated_tail_and_projection() {
    ...
    let RolloutItem::ResponseItem(envelope) = &mut lines[2].item else {
        panic!("response envelope");
    };
    let metadata = envelope.metadata.get_or_insert_default();
    metadata.client_authored = true;
    metadata.history_truncation_token_limit = Some(512);
    let mut expected = lines[2].clone();
    let RolloutItem::ResponseItem(envelope) = &mut expected.item else {
        panic!("expected response envelope");
    };
    let ResponseItem::AgentMessage { content, .. } = &mut envelope.item else {
        panic!("expected goal-supervisor message");
    };
    *content = vec![AgentMessageInputContent::InputText {
        text: "Message Type: NEW_TASK\nTask name: /root\nSender: /root/goal_supervisor\nPayload:\nsynthetic unterminated instruction".to_string(),
    }];
    ...
    let repaired = codex_rollout::parse_rollout_line_bytes(
        rewritten
            .rsplit(|byte| *byte == b'\n')
            .next()
            .expect("unterminated response record"),
    )
    .expect("decode repaired response envelope");
    assert_eq!(
        serde_json::to_value(repaired).expect("serialize repaired record"),
        serde_json::to_value(expected).expect("serialize expected record"),
        "repair must preserve envelope metadata, sender, recipient, turn ID, timestamp, and ordinal",
    );
}
