...
mod tests {
    ...
    async fn rotate_thread_segment_keeps_live_thread_out_of_archived_sessions() {
        ...
        let store = Arc::new(LocalThreadStore::new(config, Some(runtime.clone())));
        let thread_id = ThreadId::default();
        let selected_capability_root: SelectedCapabilityRoot = serde_json::from_value(
            serde_json::json!({
                "id": "demo@1",
                "location": {
                    "type": "environment",
                    "environmentId": "build",
                    "path": "file:///plugins/demo"
                }
            }),
        )
        .expect("selected capability root");
        let mut create_params = create_thread_params(thread_id);
        create_params.session_id = ThreadId::default().into();
        create_params.parent_thread_id = Some(ThreadId::default());
        create_params.thread_source = Some(ThreadSource::Subagent);
        create_params.selected_capability_roots = vec![selected_capability_root.clone()];
        create_params.multi_agent_version = Some(MultiAgentVersion::V1);
        ...
        let (old_items, _, _) = RolloutRecorder::load_rollout_items(live_rollout_path.as_path())
            .await
            .expect("load pre-rotation rollout items");
        let old_session_meta = old_items
            .iter()
            ...
            .expect("pre-rotation session metadata");
        ...
        let rotated_session_meta = items
            .iter()
            .find_map(|item| match item {
                RolloutItem::SessionMeta(meta) => Some(meta.meta.clone()),
                RolloutItem::RolloutReference(_)
            ...
            .expect("rotated session metadata");
        let mut expected_session_meta = old_session_meta;
        expected_session_meta.segment_id = rotated_session_meta.segment_id;
        assert_eq!(
            serde_json::to_value(rotated_session_meta).expect("serialize rotated session metadata"),
            serde_json::to_value(expected_session_meta)
                .expect("serialize expected session metadata")
        );
    ...
    #[tokio::test]
    async fn rotate_thread_segment_keeps_live_rollout_readable_until_staged_segment_is_ready() {
        let home = TempDir::new().expect("temp dir");
        ...
        let live_rollout_exists = tokio::fs::try_exists(live_rollout_path.as_path())
            .await
            .expect("live rollout path should be checkable");
        let live_rollout_items = RolloutRecorder::load_rollout_items(live_rollout_path.as_path())
            .await
            .expect("live rollout should remain readable before staged install");
        resume.notify_one();
        let rotation_result = rotation.await.expect("rotation task should finish");
        live_writer::clear_segment_rotation_commit_hook();
        rotation_result.expect("rotate thread segment");

        assert!(live_rollout_exists);
        assert!(live_rollout_items.0.iter().any(|item| {
            matches!(
                item,
                RolloutItem::EventMsg(EventMsg::UserMessage(event))
                    if event.message == "before rotation"
            )
        }));
    }
