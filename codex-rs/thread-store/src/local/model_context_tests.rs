...
async fn fork_context_excludes_items_after_frozen_cutoff() {
    ...
    let thread_id = ThreadId::from_string(&uuid.to_string()).expect("thread id");
    let mut frozen_message = user_message("frozen message");
    let RolloutItem::ResponseItem(envelope) = &mut frozen_message else {
        unreachable!("user_message should return a response envelope");
    };
    ...
    let path = write_ordinaled_paginated_rollout(
        ...
        "2025-01-03T13-00-06",
        uuid,
        [turn_started("frozen-turn"), frozen_message.clone()],
    );
    let history_base =
    ...
    let context = load_for_fork(lineage.clone(), Some(history_base))
        .await
        .expect("load frozen fork context");
    let full_context = load_full_for_fork(lineage, Some(history_base))
        .await
        .expect("load complete frozen fork history");
    ...
    let expected = serde_json::to_value(expected).expect("serialize expected fork context");
    for context in [context, full_context] {
        assert_eq!(
