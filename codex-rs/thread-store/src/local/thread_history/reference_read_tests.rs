...
use super::super::super::LocalThreadStore;
...
#[tokio::test]
async fn concatenated_referenced_item_pages_equal_bounded_logical_projection() {
    ...
    let logical_lines =
        codex_rollout::materialize_recent_rollout_lines(home.path(), child_path.as_path())
            .await
...
#[tokio::test]
async fn logical_reference_read_does_not_open_segments_beyond_recent_window() {
    let home = TempDir::new().expect("temp dir");
    let thread_id = ThreadId::default();
    let deepest_segment_id = SegmentId::new();
    let deepest_path = rotated_segment_path(home.path(), thread_id, deepest_segment_id, "deepest");
    tokio::fs::create_dir_all(deepest_path.parent().expect("deepest parent"))
        .await
        .expect("create deepest parent");
    tokio::fs::write(deepest_path.as_path(), "malformed rollout\n")
        .await
        .expect("write malformed deepest segment");

    let old_segment_id = SegmentId::new();
    let old_path = rotated_segment_path(home.path(), thread_id, old_segment_id, "old");
    write_lines(
        old_path.as_path(),
        vec![
            line(1, session_meta(thread_id, old_segment_id)),
            line(
                2,
                rollout_reference(deepest_path, thread_id, deepest_segment_id),
            ),
            line(3, turn_started("old")),
        ],
    )
    .await;

    let middle_segment_id = SegmentId::new();
    let middle_path = rotated_segment_path(home.path(), thread_id, middle_segment_id, "middle");
    write_lines(
        middle_path.as_path(),
        vec![
            line(4, session_meta(thread_id, middle_segment_id)),
            line(5, rollout_reference(old_path, thread_id, old_segment_id)),
            line(6, turn_started("middle")),
        ],
    )
    .await;

    let live_segment_id = SegmentId::new();
    let live_path = home.path().join(format!(
        "sessions/2026/07/16/rollout-2026-07-16T00-00-00-{thread_id}.jsonl"
    ));
    write_lines(
        live_path.as_path(),
        vec![
            line(7, session_meta(thread_id, live_segment_id)),
            line(
                8,
                rollout_reference(middle_path, thread_id, middle_segment_id),
            ),
            line(9, turn_started("current")),
        ],
    )
    .await;
    ...
    let logical_lines =
        super::logical_lines_if_referenced(&store, thread_id, /*include_archived*/ false)
            .await
            .expect("read bounded logical history")
            .expect("referenced history");

    let turn_ids = logical_lines
        .iter()
        ...
        .collect::<Vec<_>>();
    assert_eq!(turn_ids, vec!["old", "middle", "current"]);
}
...
fn rotated_segment_path(
    ...
    home.join(codex_rollout::ROTATED_ROLLOUT_SEGMENTS_SUBDIR)
        ...
        .join(format!("rollout-{name}-{thread_id}.jsonl"))
}
