...
pub(super) async fn prepare(
    ...
    // History repair also takes lifecycle before maintenance. Taking maintenance first can
    // deadlock behind a queued archive writer: repair retains a lifecycle reader while waiting
    // for maintenance, and the queued writer prevents recovery from acquiring another reader.
    let lifecycle = store
        .live_writer_locks
