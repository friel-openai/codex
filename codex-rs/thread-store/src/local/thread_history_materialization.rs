...
pub(super) async fn materialize_to_sqlite(
    ...
    let next_rollout_ordinal = projection_state
        .as_ref()
    ...
    let initial_ordinal = session_meta.history_base.map_or_else(
        || {
            lines
                .first()
                .and_then(|record| record.line.ordinal)
                .unwrap_or(0)
        },
        |base| base.end_ordinal_exclusive,
    );
