...
fn legacy_meta_line(thread_id: ThreadId, ordinal: u64) -> RolloutLine {
    meta_line_with_segment(thread_id, None, ordinal)
}
...
async fn nth_user_message_excludes_corresponding_turn_started_and_suffix() -> io::Result<()> {
    ...
    write_rollout(
        source_path.as_path(),
        &[
            meta_line(source_thread, source_segment, 0),
            agent_line("inherited", 1),
            turn_started_line("retained-turn", 2),
            user_line("retained user", 3),
            agent_line("retained answer", 4),
            turn_complete_line("retained-turn", 5),
            turn_started_line("turn-at-boundary", 6),
            user_line("fork boundary", 7),
            agent_line("after boundary", 8),
        ],
    )?;
    ...
    let mut reference_line = reference_line(source_path, source_thread, source_segment, 12);
    ...
    write_rollout(
        root_path.as_path(),
        &[
            meta_line(root_thread, root_segment, 11),
            reference_line,
            agent_line("local", 13),
        ],
    )?;
...
async fn nth_user_message_uses_real_turn_events_instead_of_contextual_user_items() -> io::Result<()>
{
    ...
    write_rollout(
        ...
        &[
            meta_line(source_thread, source_segment, 0),
            user_line("<environment_context>context</environment_context>", 1),
            turn_started_line("retained-turn", 2),
            user_event_line("retained user", 3),
            user_line("retained user", 4),
            turn_complete_line("retained-turn", 5),
            turn_started_line("turn-at-boundary", 6),
            user_event_line("fork boundary", 7),
            user_line("fork boundary", 8),
        ],
    )?;
    ...
    let root_segment = SegmentId::new();
    let root_path = home.path().join("root.jsonl");
    let mut reference_line = reference_line(source_path, source_thread, source_segment, 10);
    let RolloutItem::RolloutReference(reference) = &mut reference_line.item else {
        unreachable!();
    };
    reference.nth_user_message = Some(1);
    write_rollout(
        root_path.as_path(),
        &[meta_line(root_thread, root_segment, 9), reference_line],
    )?;
...
async fn thread_summary_uses_inherited_reference_preview() -> io::Result<()> {
    ...
    write_rollout(
        ...
        &[
            meta_line(source_thread, source_segment, 0),
            user_event_line("inherited preview", 1),
        ],
    )?;
    ...
    write_rollout(
        child_path.as_path(),
        &[
            meta_line(child_thread, child_segment, 2),
...
async fn resolver_rejects_missing_and_mismatched_segments() -> io::Result<()> {
    ...
    write_rollout(
        ...
        &[meta_line(ThreadId::new(), SegmentId::new(), 0)],
    )?;
...
async fn resolver_uses_validated_rotated_compressed_segment() -> io::Result<()> {
    ...
    write_rollout(
        plain_path.as_path(),
        &[
            meta_line(thread_id, segment_id, 0),
            agent_line("compressed", 1),
        ],
    )?;
...
async fn legacy_reference_accepts_matching_recorded_path() -> io::Result<()> {
    ...
    write_rollout(
        ...
        &[legacy_meta_line(thread_id, 0), agent_line("legacy", 1)],
    )?;
    let RolloutItem::RolloutReference(reference) =
        legacy_reference_line(recorded_path.clone(), thread_id, 2).item
    else {
        unreachable!();
    };
...
async fn legacy_reference_uses_initial_after_stable_path_is_replaced() -> io::Result<()> {
    ...
    write_rollout(
        stable_path.as_path(),
        &[
            meta_line(thread_id, SegmentId::new(), 0),
            agent_line("new", 1),
        ],
    )?;
    ...
    let RolloutItem::RolloutReference(reference) =
        legacy_reference_line(stable_path, thread_id, 2).item
    else {
        unreachable!();
    };
    assert_eq!(
...
async fn legacy_reference_rejects_replaced_stable_path_without_initial() -> io::Result<()> {
    ...
    let RolloutItem::RolloutReference(reference) =
        legacy_reference_line(stable_path, thread_id, 1).item
    else {
        unreachable!();
    };
...
async fn legacy_reference_resolves_archived_initial_segment() -> io::Result<()> {
    ...
    write_rollout(
        ...
        &[legacy_meta_line(thread_id, 0), agent_line("archived", 1)],
    )?;
...
async fn legacy_reference_rejects_ambiguous_initial_segments() -> io::Result<()> {
    ...
    for file_name in [
        ...
        write_rollout(
            ...
            &[legacy_meta_line(thread_id, 0)],
        )?;
    }
    let RolloutItem::RolloutReference(reference) =
        legacy_reference_line(PathBuf::new(), thread_id, 1).item
    else {
        unreachable!();
    };
...
async fn materialization_rejects_reference_cycles() -> io::Result<()> {
    ...
    write_rollout(
        path_a.as_path(),
        &[
            meta_line(thread_a, segment_a, 0),
            reference_line(path_b.clone(), thread_b, segment_b, 1),
        ],
    )?;
    write_rollout(
        path_b.as_path(),
        &[
            meta_line(thread_b, segment_b, 0),
            reference_line(path_a.clone(), thread_a, segment_a, 1),
        ],
    )?;
...
async fn materialization_rejects_legacy_reference_cycles() -> io::Result<()> {
    ...
    write_rollout(
        path_a.as_path(),
        &[
            legacy_meta_line(thread_a, 0),
            legacy_reference_line(path_b.clone(), thread_b, 1),
        ],
    )?;
    write_rollout(
        path_b.as_path(),
        &[
            legacy_meta_line(thread_b, 0),
            legacy_reference_line(path_a.clone(), thread_a, 1),
        ],
    )?;
...
async fn recent_materialization_bounds_existing_deep_reference_chains() -> io::Result<()> {
    ...
    let deepest_meta = serde_json::to_string(&meta_line(thread_id, deepest_segment, 0))?;
    fs::write(
        deepest_path.as_path(),
        format!("{deepest_meta}\n{{malformed rollout line\n"),
    )?;

    let mut deepest_reference = reference_line(deepest_path.clone(), thread_id, deepest_segment, 1);
    ...
    write_rollout(
        old_path.as_path(),
        &[
            meta_line(thread_id, old_segment, 2),
            deepest_reference,
            agent_line("old", 3),
        ],
    )?;

    let mut old_reference = reference_line(old_path.clone(), thread_id, old_segment, 4);
    ...
    let mut middle_reference = reference_line(middle_path.clone(), thread_id, middle_segment, 7);
    ...
    write_rollout(
        ...
        &[
            meta_line(thread_id, current_segment, 8),
            middle_reference,
            agent_line("current", 9),
        ],
    )?;
    ...
    let bounded_window =
        materialize_bounded_rollout_lines(home.path(), current_path.as_path(), 2).await?;
    assert_eq!(
    ...
    let mut fork_reference = reference_line(current_path.clone(), thread_id, current_segment, 10);
    ...
    write_rollout(
        fork_path.as_path(),
        &[meta_line(fork_thread, fork_segment, 11), fork_reference],
    )?;
...
async fn materialization_accepts_reference_chain_longer_than_legacy_limit() -> io::Result<()> {
    ...
    for index in 0..identities.len() {
        ...
        let mut lines = vec![meta_line(thread_id, segment_id, 0)];
