...
fn item_completed_user_line(turn_id: &str, message: &str, ordinal: u64) -> RolloutLine {
    RolloutLine {
        timestamp: "2026-07-13T00:00:01Z".to_string(),
        ordinal: Some(ordinal),
...
#[test]
fn nth_user_message_uses_paginated_item_completed_instead_of_contextual_user_items() {
    let mut lines = vec![
        turn_started_line("retained-turn", /*ordinal*/ 1),
        user_line(
            "<environment_context>context</environment_context>",
            /*ordinal*/ 2,
        ),
        item_completed_user_line("retained-turn", "retained user", /*ordinal*/ 3),
        turn_complete_line("retained-turn", /*ordinal*/ 4),
        turn_started_line("turn-at-boundary", /*ordinal*/ 5),
        user_line(
            "<environment_context>context</environment_context>",
            /*ordinal*/ 6,
        ),
        item_completed_user_line("turn-at-boundary", "fork boundary", /*ordinal*/ 7),
    ];

    super::truncate_before_nth_user_message(&mut lines, /*nth_user_message*/ 1);

    assert!(lines.iter().any(|line| matches!(
        &line.item,
        RolloutItem::EventMsg(EventMsg::ItemCompleted(event))
            if event.turn_id == "retained-turn"
    )));
    assert!(lines.iter().all(|line| !matches!(
        &line.item,
        RolloutItem::EventMsg(EventMsg::TurnStarted(event))
