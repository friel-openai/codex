...
fn truncate_before_nth_user_message(lines: &mut Vec<RolloutLine>, nth_user_message: usize) {
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        match &line.item {
            ...
            RolloutItem::ResponseItem(item) if item.is_user_message() => {
                let position = active_turn_start.unwrap_or(index);
            ...
            RolloutItem::EventMsg(EventMsg::ThreadRolledBack(rollback)) => {
                ...
                canonical_user_positions
                    .truncate(canonical_user_positions.len().saturating_sub(count));
    ...
    let user_positions = if saw_canonical_user_message {
        canonical_user_positions
    } else {
        response_user_positions
    };
