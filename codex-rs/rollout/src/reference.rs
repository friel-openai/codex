...
pub async fn materialize_rollout_lines(
    ...
    materialize_rollout_lines_from(codex_home, lines).await
}
...
async fn validated_candidate(
    ...
    let lines = match load_strict_rollout_lines(path.as_path()).await {
        Ok(lines) => lines,
        Err(err) if err.kind() == io::ErrorKind::NotFound => return Ok(None),
        Err(err) => return Err(err),
    };
    let meta = canonical_session_meta(&lines)?;
