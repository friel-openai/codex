...
impl MaterializationPolicy {
    fn ordinary_reference_limit(
        ...
        match self {
            Self::Complete => None,
            Self::RecentSegments if is_fork_boundary_reference(reference, current_thread_id) => {
                None
            }
...
pub async fn materialize_rollout_lines(
    ...
    materialize_rollout_lines_from_with_policy(codex_home, lines, MaterializationPolicy::Complete)
        .await
}
