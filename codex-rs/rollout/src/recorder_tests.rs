...
async fn load_rollout_items_normalizes_legacy_sleep_without_weakening_errors() -> std::io::Result<()>
{
    ...
    // RolloutLine deliberately does not implement Deserialize. Canonical decoding below is the
    // only supported path because it preserves arbitrary-precision token counts.
