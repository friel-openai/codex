...
async fn open_sqlite(
    ...
    if matches!(db, DbKind::State)
        && let Some(repair) = frodex_migration_33_collision_repair(migrator)
    {
        repair_frodex_migration_33_collision(&pool, repair).await?;
    }
    let started = Instant::now();
...
struct FrodexMigration33CollisionRepair {
    migration_33_description: String,
    migration_33_checksum: Vec<u8>,
    migration_33_sql: String,
    migration_34_description: String,
    migration_34_checksum: Vec<u8>,
}

fn frodex_migration_33_collision_repair(
    migrator: &Migrator,
) -> Option<FrodexMigration33CollisionRepair> {
    let migration_33 = migrator.iter().find(|migration| migration.version == 33)?;
    let migration_34 = migrator.iter().find(|migration| migration.version == 34)?;
    let migration_33_description = migration_33.description.to_string();
    let migration_33_checksum = migration_33.checksum.to_vec();
    let migration_33_sql = migration_33.sql.to_string();
    let migration_34_description = migration_34.description.to_string();
    let migration_34_checksum = migration_34.checksum.to_vec();
    if migration_33_description != "thread goal stopped statuses"
        || migration_34_description != "thread goal supervisor state"
    {
        return None;
    }
    Some(FrodexMigration33CollisionRepair {
        migration_33_description,
        migration_33_checksum,
        migration_33_sql,
        migration_34_description,
        migration_34_checksum,
    })
}

async fn repair_frodex_migration_33_collision(
    pool: &SqlitePool,
    repair: FrodexMigration33CollisionRepair,
) -> anyhow::Result<()> {
    let FrodexMigration33CollisionRepair {
        migration_33_description,
        migration_33_checksum,
        migration_33_sql,
        migration_34_description,
    ...
    if applied_33_description != "thread goal supervisor state"
        || applied_33_checksum.as_slice() != migration_34_checksum.as_slice()
        || !applied_33_success
    {
        return Ok(());
    }
    ...
    .bind(34_i64)
    .bind(migration_34_description.as_str())
    .bind(true)
    .bind(migration_34_checksum.as_slice())
    .bind(0_i64)
    .execute(pool)
    .await
