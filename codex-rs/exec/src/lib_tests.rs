...
#[tokio::test]
async fn thread_fork_params_include_permission_profile_from_config() {
    let codex_home = tempdir().expect("create temp codex home");
    let cwd = tempdir().expect("create temp cwd");
    let config = ConfigBuilder::default()
        .codex_home(codex_home.path().to_path_buf())
...
#[test]
fn session_configured_from_thread_fork_response_preserves_permission_profile() {
    let permission_profile = codex_protocol::models::PermissionProfile::from_legacy_sandbox_policy(
        &codex_protocol::protocol::SandboxPolicy::new_workspace_write_policy(),
    );
