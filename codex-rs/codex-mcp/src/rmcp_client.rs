...
pub(crate) struct ManagedClient {
    ...
    #[cfg(test)]
    pub(crate) managed_oauth_credentials_for_test: Option<Option<StoredOAuthTokens>>,
}

impl ManagedClient {
    pub(crate) async fn managed_oauth_credentials(&self) -> Option<Option<StoredOAuthTokens>> {
        #[cfg(test)]
        if let Some(credentials) = self.managed_oauth_credentials_for_test.as_ref() {
            return Some(credentials.clone());
        }
        self.client.managed_oauth_credentials().await
    }
...
impl AsyncManagedClient {
    ...
    pub(crate) async fn capture_ready_client_and_tools(
        ...
        Some((Arc::clone(&managed_client), tools))
    }
    ...
    pub(crate) async fn reconnect_failed_startup(&self, route: Arc<McpSessionRoute>) {
        ...
        if startup_reconnect.current_client().is_none()
            && matches!(
