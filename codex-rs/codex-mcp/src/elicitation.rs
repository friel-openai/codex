...
impl ElicitationRequestManager {
    ...
    pub(crate) fn make_sender(
        ...
        let send_event: SendEvent = Arc::new(move |event| {
            let tx_event = tx_event.clone();
            async move {
                tx_event
                    .send(event)
                    .await
                    .map_err(|error| anyhow!("failed to deliver MCP elicitation request: {error}"))
            }
            .boxed()
        });
...
/// Removes a pending responder when its transport request is cancelled or disconnected.
