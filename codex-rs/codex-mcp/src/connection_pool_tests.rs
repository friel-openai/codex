...
use crate::server::McpServerConnectionIdentity as McpConnectionIdentity;
...
fn stored_oauth_tokens(access_token: &str) -> StoredOAuthTokens {
    ...
    .expect("valid test OAuth credentials")
}
...
async fn test_managed_client_with_oauth(
    label: &str,
    credentials: StoredOAuthTokens,
) -> anyhow::Result<ManagedClient> {
    let mut client = test_managed_client(label).await?;
    client.managed_oauth_credentials_for_test = Some(Some(credentials));
    Ok(client)
}
...
#[tokio::test]
async fn child_acquisition_rekeys_connection_after_live_oauth_refresh() -> anyhow::Result<()> {
    let pool = McpConnectionPool::default();
    let root_route = route();
    let child_route = route();
    let sibling_route = route();
    let old_identity = identity("server", "/one")
        .with_oauth_credentials_for_test(Some(stored_oauth_tokens("old-token")));
    let desired_identity = identity("server", "/one")
        .with_oauth_credentials_for_test(Some(stored_oauth_tokens("refreshed-token")));
    let root_client =
        test_managed_client_with_oauth("root", stored_oauth_tokens("refreshed-token")).await?;
    let creations = Arc::new(AtomicUsize::new(0));
    let root_creations = Arc::clone(&creations);
    let root = pool.acquire(
        ...
        move |request_router| {
            ...
            ready_client(request_router, root_client.clone())
        },
    );

    let child_creations = Arc::clone(&creations);
    let child = pool
        .acquire_named(
            "test".to_string(),
        ...
        .await;
    let sibling_creations = Arc::clone(&creations);
    let sibling = pool
        .acquire_named(
            "test".to_string(),
            desired_identity,
            ...
            &sibling_route,
            move |request_router| {
                sibling_creations.fetch_add(1, Ordering::SeqCst);
                client(request_router)
            },
        )
        .await;

    assert!(root.ptr_eq(&child));
    assert!(root.ptr_eq(&sibling));
    assert_eq!(creations.load(Ordering::SeqCst), 1);
    Ok(())
}

#[tokio::test]
async fn compatible_acquisition_rechecks_generation_before_rekey() -> anyhow::Result<()> {
    let pool = McpConnectionPool::default();
    let root_route = route();
    let child_route = route();
    let old_identity = identity("server", "/one")
    ...
    let root = pool.acquire(
        ...
        McpConnectionPoolMode::Reuse,
        &root_route,
        move |request_router| ready_client(request_router, root_client.clone()),
    );
    let observed = root
        .reusable_connection(&old_identity, &desired_identity)
        .await
        .expect("the original generation should pass live OAuth validation");

    let replacement_client =
        test_managed_client_with_oauth("replacement", stored_oauth_tokens("refreshed-token"))
            .await?;
    let original_connection_id = root.connection_id();
    drop(root.inner.slot.replace(Arc::new(move |request_router| {
        ready_client(request_router, replacement_client.clone())
    })));
    let replacement_connection_id = root.connection_id();
    assert_ne!(replacement_connection_id, original_connection_id);

    assert!(
        !root
            .inner
            .slot
            .try_acquire_if_current(&observed.connection, &child_route)
    );
    assert_eq!(root.active_lease_count(), 1);
    assert!(!root.has_registered_route(&child_route));

    let child_creations = Arc::new(AtomicUsize::new(0));
    let child = pool
        .acquire_named(
            "test".to_string(),
            desired_identity,
            ...
            &child_route,
            {
                let child_creations = Arc::clone(&child_creations);
                move |request_router| {
                    child_creations.fetch_add(1, Ordering::SeqCst);
                    client(request_router)
                }
    ...
    assert!(root.ptr_eq(&child));
    assert_eq!(child.connection_id(), replacement_connection_id);
    assert_eq!(child_creations.load(Ordering::SeqCst), 0);
    Ok(())
}

#[tokio::test]
async fn reconciliation_converges_on_competing_exact_oauth_connection() -> anyhow::Result<()> {
    let pool = McpConnectionPool::default();
    let root_route = route();
    let competing_route = route();
    let refreshed_parent_route = route();
    let child_route = route();
    let old_identity = identity("server", "/one")
        .with_oauth_credentials_for_test(Some(stored_oauth_tokens("old-token")));
    let desired_identity = identity("server", "/one")
    ...
    let competing_client = test_managed_client("competing").await?;
    let competing = pool
        .acquire_named(
            "test".to_string(),
            desired_identity.clone(),
            McpConnectionPoolMode::Replace,
            &competing_route,
            move |request_router| ready_client(request_router, competing_client.clone()),
        )
        .await;

    assert!(!root.ptr_eq(&competing));
    let observed = root
        .reusable_connection(&old_identity, &desired_identity)
        .await
        .expect("the refreshed root connection should be reusable");
    let reconciled = pool
        .adopt_reconciled_connection(
            "test",
            &root,
            desired_identity.clone(),
            &refreshed_parent_route,
            &observed,
        )
        .expect("reconciliation should adopt the competing exact connection");
    assert!(reconciled.ptr_eq(&competing));
    assert!(!reconciled.ptr_eq(&root));
    ...
    let child = pool
        ...
        .await;
    assert!(child.ptr_eq(&competing));
    Ok(())
}

#[tokio::test]
async fn rejected_compatible_oauth_probe_leaves_no_route_or_lease() -> anyhow::Result<()> {
    let pool = McpConnectionPool::default();
    let root_route = route();
    let child_route = route();
    let old_identity = identity("server", "/one")
        .with_oauth_credentials_for_test(Some(stored_oauth_tokens("old-token")));
    let desired_identity = identity("server", "/one")
        .with_oauth_credentials_for_test(Some(stored_oauth_tokens("new-token")));
    let root_client =
        test_managed_client_with_oauth("root", stored_oauth_tokens("old-token")).await?;
    let root = pool.acquire(
        old_identity,
        McpConnectionPoolMode::Reuse,
        &root_route,
    ...
    assert!(!root.ptr_eq(&child));
    assert_eq!(root.active_lease_count(), 1);
    assert!(root.has_registered_route(&root_route));
    assert!(!root.has_registered_route(&child_route));
    Ok(())
}
...
fn authorization_and_client_capabilities_are_part_of_connection_identity() {
    ...
    let form_capability = ElicitationCapability {
...
async fn captured_binding_retains_its_exact_generation_across_replacement() -> anyhow::Result<()> {
    ...
    let (binding, _) = old
        .capture_ready_client_and_tools(Arc::clone(&session_route), /*catalog_override*/ None)
        .await
