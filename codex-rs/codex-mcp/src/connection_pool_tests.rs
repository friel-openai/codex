...
fn identity_with(
    ...
    let runtime_auth_provider =
        has_runtime_auth.then(codex_model_provider::unauthenticated_auth_provider);
...
struct TestAuth(&'static str);

impl AuthProvider for TestAuth {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        headers.insert(
            http::header::AUTHORIZATION,
            HeaderValue::from_str(self.0).expect("test authorization should be valid"),
        );
    }
}

fn identity_with_auth(provider: &SharedAuthProvider) -> McpConnectionIdentity {
    McpConnectionIdentity::new(
        "test".to_string(),
        Some(config("server")),
        OAuthCredentialsStoreMode::default(),
...
fn authorization_and_client_capabilities_are_part_of_connection_identity() {
    ...
    for incompatible in [
        ...
        identity_with(
            ...
            "/one",
            &Ok(None),
            ConnectorRuntimeContextKey::workspace(None, None),
            false,
            &default_capability,
            false,
        ),
    ] {
        assert_ne!(baseline, incompatible);
    }
}

#[test]
fn effective_runtime_credentials_are_part_of_connection_identity() {
    let first: SharedAuthProvider = Arc::new(TestAuth("Bearer first"));
    let second: SharedAuthProvider = Arc::new(TestAuth("Bearer second"));

    assert_ne!(identity_with_auth(&first), identity_with_auth(&second));
}
...
async fn queued_operation_converges_on_the_replacement_generation() -> anyhow::Result<()> {
    ...
    let queued = tokio::spawn(async move {
        queued_lease
            .run(queued_route, |client| async move {
                Arc::as_ptr(&client.connection) as usize
            })
            .await
    });
    tokio::task::yield_now().await;
