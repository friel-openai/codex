...
impl ConnectionSlot {
    ...
    async fn reusable_connection(
        &self,
        current_identity: &McpServerConnectionIdentity,
        desired_identity: &McpServerConnectionIdentity,
        ...
        loop {
            ...
            if !Arc::ptr_eq(&self.current(), &connection) {
                continue;
            }
            if client.client.is_closed().await {
                return None;
            }
            let reusable = match desired_identity.oauth_credentials() {
                Err(_) => true,
                Ok(desired_credentials) => match client.managed_oauth_credentials().await {
                    Some(live_credentials) => &live_credentials == desired_credentials,
                    None => current_identity
                        .oauth_credentials()
                        .is_ok_and(|startup_credentials| {
                            startup_credentials == desired_credentials
                        }),
                },
            };
            if Arc::ptr_eq(&self.current(), &connection) {
                return reusable.then_some(connection);
            }
        }
    }

    fn try_acquire_if_current(
        &self,
        expected: &Arc<SharedConnection>,
        route: &Arc<McpSessionRoute>,
    ) -> bool {
        let mut routes = self
            .routes
        ...
        let state = self
            ...
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        if !Arc::ptr_eq(&state.connection, expected) {
            return false;
        }
        let mut active = self.active_leases.load(Ordering::Acquire);
        loop {
            ...
            match self.active_leases.compare_exchange_weak(
                ...
                Ok(_) => {
                    ...
                    expected.client.register_route(route);
                    return true;
                }
                Err(current) => active = current,
            }
...
/// The physical connection generation whose live credentials passed compatibility validation.
pub(crate) struct McpReusableConnection {
    connection: Arc<SharedConnection>,
}
...
impl McpConnectionLease {
    ...
    pub(crate) async fn reusable_connection(
        &self,
        current_identity: &McpServerConnectionIdentity,
        desired_identity: &McpServerConnectionIdentity,
    ) -> Option<McpReusableConnection> {
        self.inner
            .slot
            .reusable_connection(current_identity, desired_identity)
            .await
            .map(|connection| McpReusableConnection { connection })
    }
    ...
    pub(crate) async fn capture_ready_client_and_tools(
        ...
        self.run(route.clone(), move |client| async move {
            ...
            let (managed, tools) = client
                .capture_ready_client_and_tools(catalog_override, Arc::clone(&route))
                .await?;
            Some((
    ...
    pub(crate) async fn listed_tools(
        ...
        self.run(route, move |client| async move {
            if let Some((connection_id, tools)) = catalog_override
    ...
    pub(crate) async fn reconnect_failed_startup(&self, route: Arc<McpSessionRoute>) {
        ...
        if let Ok(connection) = self.current() {
            connection.client.reconnect_failed_startup(route).await;
        }
    }
    ...
    pub(crate) fn cancel_pending_startup_if_sole_route(&self, route: &Arc<McpSessionRoute>) {
        let connection = self.inner.slot.current();
        if !connection.client.startup_complete.load(Ordering::Acquire)
            && !self.inner.slot.has_live_route_other_than(route)
        {
    ...
    pub(crate) fn active_lease_count(&self) -> usize {
        self.inner.slot.active_leases.load(Ordering::Acquire)
    }

    #[cfg(test)]
    pub(crate) fn has_registered_route(&self, route: &Arc<McpSessionRoute>) -> bool {
        self.inner
            .slot
            .routes
            .lock()
            ...
            .iter()
            .filter_map(Weak::upgrade)
            .any(|registered| Arc::ptr_eq(&registered, route))
    }

    #[cfg(test)]
    pub(crate) fn async_client(&self) -> Arc<AsyncManagedClient> {
        Arc::clone(&self.inner.slot.current().client)
    }

    #[cfg(test)]
    pub(crate) fn register_route(&self, route: &Arc<McpSessionRoute>) {
        self.inner.slot.register_route(route);
    }
...
impl McpConnectionPool {
    ...
    pub(crate) fn acquire(
        ...
        if let Some(slot) = entries
            .iter()
            .rev()
            .find(|entry| entry.server_name == "test" && entry.identity == identity)
    ...
    pub(crate) fn acquire_named<'a>(
        ...
        route: &Arc<McpSessionRoute>,
        create: impl Fn(McpConnectionRequestRouter) -> AsyncManagedClient + Send + Sync + 'static,
    ) -> std::pin::Pin<Box<dyn Future<Output = McpConnectionLease> + Send + 'a>> {
        let factory: ConnectionFactory = Arc::new(create);
        ...
        let immediate = {
            ...
            if mode == McpConnectionPoolMode::Reuse {
                entries
                    .iter()
                    .rev()
                    .find(|entry| entry.server_name == server_name && entry.identity == identity)
                    .and_then(|entry| entry.slot.upgrade())
                    .filter(|slot| slot.try_acquire(&route))
                    .map(McpConnectionLease::new)
            } else {
                None
            }
        };
        if let Some(lease) = immediate {
            return Box::pin(std::future::ready(lease));
        }
        let has_compatible = mode == McpConnectionPoolMode::Reuse && {
            let entries = self
                .entries
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
            entries.iter().any(|entry| {
                entry.server_name == server_name
                    && entry.identity.has_same_connection_config(&identity)
            })
        };
        if !has_compatible {
            return Box::pin(Self::create_or_acquire_exact(
        ...
        Box::pin(self.acquire_named_with_factory(server_name, identity, mode, route, factory))
    }

    /// Creates a physical client behind a fresh task poll boundary while keeping insertion atomic.
    ///
    /// `AsyncManagedClient::new` has a large debug-build stack frame. Calling it from the nested
    /// session-initialization future can overflow Tokio's worker stack, so the scheduler polls this
    /// task independently. The task owns the pool lock, performs the final exact-entry check, and
    /// constructs at most one replacement.
    async fn create_or_acquire_exact(
        entries: Arc<Mutex<Vec<PoolEntry>>>,
        server_name: String,
        identity: McpServerConnectionIdentity,
        mode: McpConnectionPoolMode,
        ...
        match task.await {
            Ok(lease) => lease,
            Err(error) => panic!("MCP connection pool acquisition task failed: {error}"),
        }
    }

    async fn acquire_named_with_factory(
        &self,
        server_name: String,
        identity: McpServerConnectionIdentity,
        mode: McpConnectionPoolMode,
        ...
        'acquire: loop {
            let compatible = {
                ...
                {
                    if mode == McpConnectionPoolMode::Replace {
                        drop(slot.replace(Arc::clone(&factory)));
                    }
                    return McpConnectionLease::new(slot);
                }
                    ...
                    entries
                        ...
                        .filter_map(|entry| {
                            entry
                                .slot
                                .upgrade()
                                .map(|slot| (entry.identity.clone(), slot))
                        })
                        .collect::<Vec<_>>()
                } else {
                    Vec::new()
            ...
            for (current_identity, compatible_slot) in compatible {
                let Some(observed_connection) = compatible_slot
                    .reusable_connection(&current_identity, &identity)
                    .await
                else {
                    continue;
                };
                ...
                if let Some(exact_slot) = entries
                    .iter()
                    .rev()
                    .find(|entry| entry.server_name == server_name && entry.identity == identity)
                    .and_then(|entry| entry.slot.upgrade())
                    .filter(|slot| slot.try_acquire(&route))
                {
                    return McpConnectionLease::new(exact_slot);
                }
                let Some(index) = entries.iter().position(|entry| {
                    entry.server_name == server_name
                        && entry
                            .slot
                            .upgrade()
                            .is_some_and(|slot| Arc::ptr_eq(&slot, &compatible_slot))
                }) else {
                    continue;
                };
                if !compatible_slot.try_acquire_if_current(&observed_connection, &route) {
                    continue 'acquire;
                }
                let mut entry = entries.remove(index);
                entry.identity = identity.clone();
                entries.push(entry);
                return McpConnectionLease::new(compatible_slot);
            }
            ...
            return Self::create_or_acquire_exact(
                Arc::clone(&self.entries),
                server_name.clone(),
                identity.clone(),
                mode,
    ...
    pub(crate) fn adopt_reconciled_connection(
        &self,
        server_name: &str,
        previous: &McpConnectionLease,
        identity: McpServerConnectionIdentity,
        route: &Arc<McpSessionRoute>,
        observed: &McpReusableConnection,
    ) -> Option<McpConnectionLease> {
        let previous_slot = &previous.inner.slot;
        let mut entries = self
            .entries
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        ...
        if let Some(exact_slot) = entries
            ...
            .and_then(|entry| entry.slot.upgrade())
        {
            if Arc::ptr_eq(&exact_slot, previous_slot) {
                return previous_slot
                    .register_route_if_current(&observed.connection, route)
                    .then(|| previous.clone());
            }
            if exact_slot.try_acquire(route) {
                return Some(McpConnectionLease::new(exact_slot));
            }
        }
        if !previous_slot.register_route_if_current(&observed.connection, route) {
            return None;
        }
        if let Some(index) = entries.iter().position(|entry| {
            entry.server_name == server_name
                && entry
                    .slot
                    .upgrade()
                    .is_some_and(|slot| Arc::ptr_eq(&slot, previous_slot))
        }) {
            let mut entry = entries.remove(index);
            ...
            entries.push(PoolEntry {
                ...
                identity,
                slot: Arc::downgrade(previous_slot),
            });
        }
        Some(previous.clone())
    }
